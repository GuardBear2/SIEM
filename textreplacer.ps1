...
# Directories to ignore (will be skipped completely)
$ignoreDirs = @(".github")
...
foreach ($dir in $dirs) {
    # Skip directories in the ignore list or if they're in an ignored path
    $shouldSkip = $false
    foreach ($ignoreDir in $ignoreDirs) {
        if ($dir.Name -eq $ignoreDir -or $dir.FullName -like "*\$ignoreDir\*") {
            Write-Host "Skipping ignored directory:" $dir.FullName
            $shouldSkip = $true
            break
        }
    }
...
foreach ($file in $files) {
    ...
    foreach ($ignoreDir in $ignoreDirs) {
        if ($file.FullName -like "*\$ignoreDir\*") {
            Write-Host "Skipping file in ignored directory:" $file.FullName
            $shouldSkip = $true
            break
        }
    }

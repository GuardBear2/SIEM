...
from guardbear_testing.constants.paths.logs import GUARDBEAR_API_LOG_FILE_PATH, GUARDBEAR_API_JSON_LOG_FILE_PATH
from guardbear_testing.constants.api import GUARDBEAR_API_PORT, CONFIGURATION_TYPES
from guardbear_testing.modules.api.configuration import get_configuration, append_configuration, delete_configuration_file
from guardbear_testing.modules.api.patterns import API_STARTED_MSG
from guardbear_testing.tools.monitors import file_monitor
from guardbear_testing.utils.callbacks import generate_callback


@pytest.fixture
def add_configuration(test_configuration: list[dict], request: pytest.FixtureRequest) -> None:
    """Add configuration to the GuardBear API configuration files.
...
def wait_for_api_start(test_configuration: dict) -> None:
    ...
    if test_configuration is not None:
        if test_configuration.get('blocks') is not None:
            ...
            port = test_configuration['blocks'].get('port', GUARDBEAR_API_PORT)

    file_to_monitor = GUARDBEAR_API_JSON_LOG_FILE_PATH if logs_format == 'json' else GUARDBEAR_API_LOG_FILE_PATH

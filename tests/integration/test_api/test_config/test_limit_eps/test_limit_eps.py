...
from guardbear_testing.constants.api import CONFIGURATION_TYPES, MANAGER_CONFIGURATION_ROUTE
from guardbear_testing.constants.daemons import API_DAEMONS_REQUIREMENTS
from guardbear_testing.modules.api.utils import login, get_base_url
from guardbear_testing.utils.configuration import get_test_cases_data, load_configuration_template, get_guardbear_conf
...
def test_limit_eps(test_configuration, test_metadata, set_guardbear_configuration, add_configuration,
    ...
    guardbear_config = " ".join(get_guardbear_conf())
    assert f"<maximum>{test_metadata['expected_max_value']}</maximum>" in guardbear_config
    assert f"<timeframe>{test_metadata['expected_timeframe_value']}</timeframe>" in guardbear_config

...
brief: These tests will check if the 'drop_privileges' setting of the API is working properly.
       This setting allows the user who starts the 'guardbear-apid' daemon to be different from
       the 'root' user. The GuardBear API is an open source 'RESTful' API that allows for interaction
       with the GuardBear manager from a web browser, command line tool like 'cURL' or any script
...
references:
    ...
    - https://documentation.guardbear.com/current/user-manual/api/configuration.html#drop-privileges
...
from guardbear_testing.constants.paths.api import GUARDBEAR_API_SCRIPT
from guardbear_testing.utils.configuration import get_test_cases_data, load_configuration_template
from guardbear_testing.utils.services import search_process_by_command
...
def test_drop_privileges(test_configuration, test_metadata, add_configuration, truncate_monitored_files,
    ...
    description: Check if 'drop_privileges' affects the user of the API process. In this test, the 'PID' of the API
                 process is obtained. After that, it gets the user ('root' or 'guardbear') and checks if it matches the
    ...
    test_phases:
        ...
        - test:
            - Search guardbear-apid process and verify that it is present
    ...
    assertions:
        - Verify that when 'drop_privileges' is enabled the user who has started the 'guardbear-apid' daemon is 'guardbear'.
        - Verify that when 'drop_privileges' is disabled the user who has started the 'guardbear-apid' daemon is 'root'.
    ...
    expected_output:
        - PID of the 'guardbear-apid' process.
        - guardbear (if drop_privileges is enabled)
        - root (if drop_privileges is disabled)
    """
    expected_user = test_metadata['expected_user']

    # Get guardbear-apid process info
    api_process = search_process_by_command(GUARDBEAR_API_SCRIPT)
    assert api_process is not None, f"The process {GUARDBEAR_API_SCRIPT} could not be found"

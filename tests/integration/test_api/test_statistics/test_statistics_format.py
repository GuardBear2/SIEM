...
brief: These tests will check if the stats can be obtained from the API and if they follow the expected schema.
       The GuardBear API is an open source 'RESTful' API that allows for interaction with the GuardBear manager from
...
references:
    - https://documentation.guardbear.com/current/user-manual/api/reference.html (Get GuardBear daemon stats)
    - https://documentation.guardbear.com/current/user-manual/api/reference.html (Get GuardBear daemon stats from an agent)
...
from guardbear_testing import DATA_PATH
from guardbear_testing.constants.api import DAEMONS_STATS_ROUTE
from guardbear_testing.modules.api.utils import get_base_url, login, validate_statistics
...
@pytest.mark.parametrize('test_configuration,test_metadata', zip(test1_configuration, test1_metadata),
                         ids=test1_cases_ids)
def test_manager_statistics_format(test_configuration, test_metadata, load_guardbear_basic_configuration,
                                   set_guardbear_configuration, daemons_handler):
    """

...
brief: These tests will check if the response_postprocessing middleware of the API handled by the 'guardbear-apid' daemon is
       working properly. The GuardBear API is an open source 'RESTful' API that allows the interaction with the GuardBear
...
from guardbear_testing.constants.api import GUARDBEAR_API_PROTOCOL
from guardbear_testing.modules.api.utils import login, get_base_url, set_authorization_header
from guardbear_testing.utils.configuration import get_test_cases_data
...
def test_response_postprocessing(test_configuration, test_metadata, truncate_monitored_files, daemons_handler,
    ...
    if use_login_token:
        ...
        session.mount(f"{GUARDBEAR_API_PROTOCOL}://", adapter)

...
from guardbear_testing.constants.aws import RANDOM_ACCOUNT_ID
from guardbear_testing.constants.paths.aws import AWS_SERVICES_DB_PATH, S3_CLOUDTRAIL_DB_PATH
from guardbear_testing.modules.aws.utils import path_exist
from guardbear_testing.utils.db_queries.aws_db import (get_multiple_service_db_row, table_exists_or_has_values,
...
def test_regions(
        test_configuration, metadata, load_guardbear_basic_configuration,  create_test_bucket, manage_bucket_files,
        set_guardbear_configuration, clean_s3_cloudtrail_db, configure_local_internal_options_function,
        truncate_monitored_files, restart_guardbear_function, file_monitoring
):
    """
    description: Only the logs for the specified region are processed.
    test_phases:
        - setup:
            - Load GuardBear light configuration.
            - Apply ossec.conf configuration changes according to the configuration template and use case.
            - Apply custom settings in local_internal_options.conf.
            - Truncate guardbear logs.
            - Restart guardbear-manager service to apply configuration changes.
        - test:
    ...
    parameters:
        ...
        - load_guardbear_basic_configuration:
            type: fixture
            brief: Load basic guardbear configuration.
        - set_guardbear_configuration:
            type: fixture
        ...
        - truncate_monitored_files:
            type: fixture
            brief: Truncate guardbear logs.
        - restart_guardbear_daemon_function:
            type: fixture
            brief: Restart the guardbear service.
        - file_monitoring:
            type: fixture
...
def test_cloudwatch_regions(
        test_configuration, metadata, load_guardbear_basic_configuration, create_test_log_group, create_test_log_stream,
        manage_log_group_events, set_guardbear_configuration, clean_aws_services_db,
        configure_local_internal_options_function, truncate_monitored_files, restart_guardbear_function, file_monitoring
):
    """
    description: Only the logs for the specified region are processed.
    test_phases:
...
def test_inspector_regions(
        test_configuration, metadata, load_guardbear_basic_configuration,
        set_guardbear_configuration, clean_aws_services_db, configure_local_internal_options_function,
        truncate_monitored_files, restart_guardbear_function, file_monitoring
):
    """
    description: Only the logs for the specified region are processed.

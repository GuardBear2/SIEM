...
from guardbear_testing import session_parameters
from guardbear_testing.constants import platforms
from guardbear_testing.constants.platforms import WINDOWS
from guardbear_testing.constants.daemons import GUARDBEAR_MANAGER, API_DAEMONS_REQUIREMENTS
from guardbear_testing.constants.paths import ROOT_PREFIX
from guardbear_testing.constants.paths.api import RBAC_DATABASE_PATH
from guardbear_testing.constants.paths.logs import ACTIVE_RESPONSE_LOG_PATH, GUARDBEAR_LOG_PATH, ALERTS_JSON_PATH, \
                                               GUARDBEAR_API_LOG_FILE_PATH, GUARDBEAR_API_JSON_LOG_FILE_PATH
from guardbear_testing.constants.paths.configurations import GUARDBEAR_CLIENT_KEYS_PATH
from guardbear_testing.logger import logger
from guardbear_testing.tools import socket_controller
from guardbear_testing.tools.monitors import queue_monitor
from guardbear_testing.tools.monitors.file_monitor import FileMonitor
from guardbear_testing.tools.simulators.agent_simulator import create_agents, connect
from guardbear_testing.tools.simulators.authd_simulator import AuthdSimulator
from guardbear_testing.tools.simulators.remoted_simulator import RemotedSimulator
from guardbear_testing.utils import configuration, database, file, mocking, services
from guardbear_testing.utils.file import remove_file, truncate_file
from guardbear_testing.utils.manage_agents import remove_agents
import guardbear_testing.utils.configuration as guardbear_configuration
from guardbear_testing.utils.services import control_service
...
def load_guardbear_basic_configuration():
    ...
    backup_ossec_configuration = configuration.get_guardbear_conf()

    # Write new configuration
    configuration.write_guardbear_conf(minimal_configuration)

    yield

    # Restore the ossec.conf backup
    configuration.write_guardbear_conf(backup_ossec_configuration)


@pytest.fixture()
def backup_guardbear_configuration() -> None:
    """Backup guardbear configuration. Saves the initial configuration and restores it after the test."""
    # Save configuration
    backup_config = configuration.get_guardbear_conf()

    yield

    # Restore configuration
    configuration.write_guardbear_conf(backup_config)


@pytest.fixture()
def set_guardbear_configuration(test_configuration: dict) -> None:
    """Set guardbear configuration
    ...
    test_config = configuration.set_section_guardbear_conf(test_configuration.get('sections'))

    # Set new configuration
    configuration.write_guardbear_conf(test_config)
...
def configure_local_internal_options_function(request):
    ...
    backup_local_internal_options = guardbear_configuration.get_local_internal_options_dict()

    logger.debug(f"Set local_internal_option to {str(local_internal_options)}")
    guardbear_configuration.set_local_internal_options_dict(local_internal_options)

    yield

    logger.debug(f"Restore local_internal_option to {str(backup_local_internal_options)}")
    guardbear_configuration.set_local_internal_options_dict(backup_local_internal_options)


@pytest.fixture()
def restart_guardbear_function(request):
    ...
    if len(daemons) == 0:
        ...
        # Stop a list daemons in order (as GuardBear does)
        daemons.reverse()
        for daemon in daemons:
...
def file_monitoring(request):
    ...
    if hasattr(request.module, 'file_to_monitor'):
        ...
        file_to_monitor = GUARDBEAR_LOG_PATH
...
def truncate_monitored_files_implementation() -> None:
    ...
    if services.get_service() == GUARDBEAR_MANAGER:
        log_files = [GUARDBEAR_LOG_PATH, ALERTS_JSON_PATH, GUARDBEAR_API_LOG_FILE_PATH,
                     GUARDBEAR_API_JSON_LOG_FILE_PATH, GUARDBEAR_CLIENT_KEYS_PATH]
    else:
        log_files = [GUARDBEAR_LOG_PATH]
...
def daemons_handler_implementation(request: pytest.FixtureRequest) -> None:
    """Helper function to handle GuardBear daemons.
    ...
    The  `daemons_handler_configuration` should be a dictionary with the following keys:
        daemons (list, optional): List with every daemon to be used by the module. In case of empty a ValueError
            will be raised
        all_daemons (boolean): Configure to restart all guardbear services. Default `False`.
    ...
    if config := getattr(request.module, 'daemons_handler_configuration', None):
        ...
        if 'all_daemons' in config:
            logger.debug(f"GuardBear control set to {config['all_daemons']}")
        ...
        logger.debug("GuardBear control set to 'all_daemons'")
        all_daemons = True

    try:
        if all_daemons:
            logger.debug('Restarting guardbear using guardbear-control')
    ...
    if all_daemons:
        logger.debug('Stopping guardbear using guardbear-control')
...
@pytest.fixture(scope='module')
def restart_guardbear_daemon_after_finishing_module(daemon: str = None) -> None:
    """Restart a GuardBear daemons and clears the guardbear log after the test module finishes execution.
    Args:
        daemon (str): provide which daemon to restart. If None, all daemons will be restarted.
    """
    yield
    file.truncate_file(GUARDBEAR_LOG_PATH)
    services.control_service("restart", daemon=daemon)
...
def configure_local_internal_options_handler(request: pytest.FixtureRequest, test_metadata) -> None:
    ...
    The `local_internal_options` is a dict with keys and values as the GuardBear `local_internal_options` format.
...
def configure_sockets_environment_implementation(request: pytest.FixtureRequest) -> None:
    ...
    # Stop guardbear-service and ensure all daemons are stopped
    services.control_service('stop')
...
def mock_agent_module():
    """Fixture to create a mocked agent in guardbear databases"""
...
def autostart_simulators(request: pytest.FixtureRequest) -> None:
    """
    Fixture for starting simulators in guardbear-agent executions.

    This fixture starts both Authd and Remoted simulators only in the cases where the service is not
    GUARDBEAR_MANAGER, and when the test function is not already using the simulator fixture, if it does
    use one of them, only start the remaining simulator.

    This is required so all guardbear-agent instances are being tested with the guardbear-manager connection
    being mocked.
    """
    create_authd = 'authd_simulator' not in request.fixturenames
    create_remoted = 'remoted_simulator' not in request.fixturenames

    if services.get_service() is not GUARDBEAR_MANAGER:

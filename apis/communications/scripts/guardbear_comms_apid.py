...
from guardbear.core.batcher.mux_demux import MuxDemuxManager, MuxDemuxQueue
from guardbear.core.cluster.utils import print_version
from guardbear.core.config.client import CentralizedConfig
from guardbear.core.config.models.comms_api import CommsAPIConfig
...
MAIN_PROCESS = 'guardbear-comms-apid'
...
def get_gunicorn_options(pid: int, log_config_dict: dict, config: CommsAPIConfig) -> dict:
    ...
    pidfile = common.GUARDBEAR_RUN / f'{MAIN_PROCESS}-{pid}.pid'
...
if __name__ == '__main__':
    ...
    try:
        ...
        logger.error(f'Error when trying to start the GuardBear Communications API. {e}')
        exit_code = 1
    except Exception as e:
        logger.error(f'Internal error when trying to start the GuardBear Communications API. {e}')
        exit_code = 1
    finally:

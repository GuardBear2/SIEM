...
from guardbear.core import common, pyDaemonModule, utils
from guardbear.core.authentication import load_jwt_keys
from guardbear.core.cluster.utils import print_version
from guardbear.core.common import GUARDBEAR_SERVER_YML
from guardbear.core.config.client import CentralizedConfig
from guardbear.core.config.models.central_config import ManagementAPIConfig
from guardbear.core.config.models.ssl_config import APISSLConfig
...
API_MAIN_PROCESS = 'guardbear-server-management-apid'
API_LOCAL_REQUEST_PROCESS = 'guardbear-server-management-apid_exec'
API_AUTHENTICATION_PROCESS = 'guardbear-server-management-apid_auth'
API_SECURITY_EVENTS_PROCESS = 'guardbear-server-management-apid_events'
...
def configure_ssl(params: dict, config: APISSLConfig):
    ...
    try:
        ...
        # Check and assign ownership to guardbear user for the api.key and api.crt files
        utils.assign_guardbear_ownership(config.key)
        utils.assign_guardbear_ownership(config.cert)
        ...
        if exc.errno == 22:
            ...
            msg = (
                f'GuardBear API SSL ERROR. Please, ensure '
                f'if path to certificates is correct in the configuration '
                f'file {GUARDBEAR_SERVER_YML}'
            )
            print(msg)
            logger.error(msg)
            raise exc from exc
...
def start(params: dict, config: ManagementAPIConfig):
    """Run the GuardBear API.

    If another GuardBear API is running, this function will fail because uvicorn server will
    ...
    app.add_api(
        'spec.yaml',
        arguments={'title': 'GuardBear API', 'protocol': 'https', 'host': config.host, 'port': config.port},
    ...
    app.add_middleware(GuardBearAccessLoggerMiddleware, MiddlewarePosition.BEFORE_EXCEPTION)
...
def add_debug2_log_level_and_error():
    """Add a new debug level used by guardbear api and framework."""
    logging.DEBUG2 = 6
...
if __name__ == '__main__':
    ...
    try:
        ...
        print(f'Error when trying to start the GuardBear API. {api_log_error}')
        sys.exit(1)

    # Configure and create the guardbear-api logger
    ...
    if not args.root:
        ...
        if management_config.drop_privileges:
            os.setgid(common.guardbear_gid())
            os.setuid(common.guardbear_uid())
    else:
    ...
    try:
        ...
        print(f'Error when trying to start the GuardBear API. {e}')
        sys.exit(1)
    except Exception as e:
        print(f'Internal error when trying to start the GuardBear API. {e}')
        sys.exit(1)
    finally:

...
if [[ "$OS" == "Darwin" ]]; then
    if [ "${GUARDBEAR_HOME}" != "/Library/Ossec" ]; then
    ...
    if [ "${GUARDBEAR_HOME}" != "/var/ossec" ]; then
...
if [[ "$OS" == "Darwin" ]]; then
    installer -pkg ./var/upgrade/guardbear-agent* -target / >> ./logs/upgrade.log 2>&1
elif [[ "$OS" == "Linux" ]]; then
    if find ./var/upgrade/ -mindepth 1 -maxdepth 1 -type f -name "*.rpm" | read; then
        if command -v rpm >/dev/null 2>&1; then
            rpm -Uvh ./var/upgrade/guardbear-agent* >> ./logs/upgrade.log 2>&1
        else
        ...
        if command -v dpkg >/dev/null 2>&1; then
            dpkg -i ./var/upgrade/guardbear-agent* >> ./logs/upgrade.log 2>&1
        else
        ...
        if command -v apk >/dev/null 2>&1; then
            apk add --allow-untrusted --force ./var/upgrade/guardbear-agent* >> ./logs/upgrade.log 2>&1
        else
...
echo "$(date +"%Y/%m/%d %H:%M:%S") - Checking for GuardBear Agent control script." >> ./logs/upgrade.log

if [ -f "./bin/guardbear-control" ]; then
    echo "$(date +"%Y/%m/%d %H:%M:%S") - Checking if GuardBear Agent is running." >> ./logs/upgrade.log
    if ./bin/guardbear-control status >/dev/null 2>&1; then
        echo "$(date +"%Y/%m/%d %H:%M:%S") - GuardBear Agent is already running." >> ./logs/upgrade.log
    else
        echo "$(date +"%Y/%m/%d %H:%M:%S") - Starting GuardBear Agent." >> ./logs/upgrade.log
        ./bin/guardbear-control start >> ./logs/upgrade.log 2>&1
    fi
elif [ -f "./bin/ossec-control" ]; then
    echo "$(date +"%Y/%m/%d %H:%M:%S") - Upgrade failed: guardbear-control not found. Attempting to restart using ossec-control." >> ./logs/upgrade.log
    ./bin/ossec-control restart >> ./logs/upgrade.log 2>&1
    echo -ne "2" > ./var/upgrade/upgrade_result
    rm -f $LOCK
    exit 1
else
    echo "$(date +"%Y/%m/%d %H:%M:%S") - Upgrade failed: Neither guardbear-control nor ossec-control were found." >> ./logs/upgrade.log
    echo -ne "2" > ./var/upgrade/upgrade_result
    rm -f $LOCK
    exit 1
fi
...
while [ "$status" != "connected" -a $COUNTER -gt 0 ]; do
    . ./var/run/guardbear-agentd.state >> ./logs/upgrade.log 2>&1

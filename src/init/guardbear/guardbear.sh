...
# Install functions for GuardBear
# GuardBear.com (https://github.com/guardbear)
...
InstallSELinuxPolicyPackage(){
    ...
    if command -v semodule > /dev/null && command -v getenforce > /dev/null; then
        if [ -f selinux/guardbear.pp ]; then
            if [ $(getenforce) != "Disabled" ]; then
                cp selinux/guardbear.pp /tmp && semodule -i /tmp/guardbear.pp
                rm -f /tmp/guardbear.pp
                semodule -e guardbear
            fi
        fi
    fi
}
...
GuardBearUpgrade()
{
    ...
    if [ -f "$PREINSTALLEDDIR/var/db/global.db" ]; then
        ...
        if [ -f "$PREINSTALLEDDIR/queue/db/global.db" ]; then
            ...
            chown guardbear:guardbear $PREINSTALLEDDIR/queue/db/global.db
    ...
    # Remove existing SQLite databases for GuardBear DB, only if upgrading from 3.2..3.6
    ...
    if [ -d $PREINSTALLEDDIR/logs/ossec ]; then
        if [ "$(ls -A $PREINSTALLEDDIR/logs/ossec)" ]; then
            mv -f $PREINSTALLEDDIR/logs/ossec/* $PREINSTALLEDDIR/logs/guardbear
        fi
        rm -rf $PREINSTALLEDDIR/logs/ossec
    fi

    # Remove deprecated GuardBear tools
    ...
    if (grep "^ossec:" /etc/group > /dev/null 2>&1) || (dscl . -read /Groups/ossec > /dev/null 2>&1)  ; then
        if [ "X$1" = "Xserver" ]; then
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -user root -print0 | xargs -0 chown root:guardbear
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -print0 | xargs -0 chown guardbear:guardbear
        else
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -user root -exec chown root:guardbear {} \;
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -exec chown guardbear:guardbear {} \;
        fi
    fi

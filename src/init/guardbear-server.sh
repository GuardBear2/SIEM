...
# guardbear-control        This shell script takes care of starting
...
USE_JSON=false
DAEMONS="guardbear-clusterd guardbear-modulesd guardbear-monitord guardbear-logcollector guardbear-remoted guardbear-syscheckd guardbear-analysisd guardbear-maild guardbear-execd guardbear-db guardbear-authd guardbear-agentlessd guardbear-integratord guardbear-dbd guardbear-csyslogd guardbear-apid guardbear-comms-apid"
OP_DAEMONS="guardbear-clusterd guardbear-maild guardbear-agentlessd guardbear-integratord guardbear-dbd guardbear-csyslogd"
...
start_service()
{
    ...
    if [ $USE_JSON = false ]; then
        echo "Starting GuardBear $VERSION..."
    fi

    TEST=$(${DIR}/bin/guardbear-logtest-legacy -t  2>&1 | grep "ERROR")
    if [ ! -z "$TEST" ]; then
        if [ $USE_JSON = true ]; then
        ...
        touch ${DIR}/var/run/guardbear-analysisd.failed
        exit 1;
    fi
    ...
    for i in ${SDAEMONS}; do
        ## If guardbear-maild is disabled, don't try to start it.
        if [ X"$i" = "Xguardbear-maild" ]; then
             grep "<email_notification>no<" ${DIR}/etc/ossec.conf >/dev/null 2>&1
             if [ $? = 0 ]; then
                 continue
             fi
        fi
        ## If guardbear-clusterd is disabled, don't try to start it.
        if [ X"$i" = "Xguardbear-clusterd" ]; then
        ...
        ## If guardbear-authd is disabled, don't try to start it.
        if [ X"$i" = "Xguardbear-authd" ]; then
...
pstatus()
{
    ...
    if [ $? = 0 ]; then
        for pid in `cat ${DIR}/var/run/${pfile}-*.pid 2>/dev/null`; do
            ...
            if [ ! $? = 0 ]; then
                if [ $USE_JSON = false ]; then
                    echo "${pfile}: Process ${pid} not used by GuardBear, removing..."
                fi
...
stop_service()
{
    ...
    if [ $USE_JSON = true ]; then
        ...
        echo "GuardBear $VERSION Stopped"
    fi
}
...
info()
{
    if [ "X${1}" = "X" ]; then
        if [ $USE_JSON = true ]; then
            echo -n '{"error":0,"data":['
            echo -n '{"GUARDBEAR_VERSION":"'${VERSION}'"},'
            echo -n '{"GUARDBEAR_REVISION":"'${REVISION}'"},'
            echo -n '{"GUARDBEAR_TYPE":"'${TYPE}'"}'
            echo -n ']}'
        else
            echo "GUARDBEAR_VERSION=\"${VERSION}\""
            echo "GUARDBEAR_REVISION=\"${REVISION}\""
            echo "GUARDBEAR_TYPE=\"${TYPE}\""
        fi
    else
        case "${1}" in
            -v) echo "${VERSION}" ;;
...
restart)
    ...
    ;;
reload)
    DAEMONS=$(echo $DAEMONS | sed 's/guardbear-execd//')
    restart_service
    ;;
status)
    lock

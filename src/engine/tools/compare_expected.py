...
if __name__ == '__main__':
    try:
        arg_parser = ArgumentParser(
            description='Compare expected results of an output and guardbear output. Files should be a json consisting of an array of output objects, the script expects that the order of both files is the same')
        arg_parser.add_argument(
            'table', help='CSV table with metadata about the test')
        arg_parser.add_argument(
            'guardbear_output', type=Path, help='Path to the file with guardbear output')
        arg_parser.add_argument(
        ...
        table = args.table
        guardbear_output_path = Path(args.guardbear_output)
        ...
        # Read the files
        guardbear_expected = loads(guardbear_output_path.read_text())
        other_expected = loads(other_output_path.read_text())

        # Verify that the number of outputs is the same
        if len(guardbear_expected) != len(other_expected):
            raise Exception(
                f'Expected the same number of outputs, got {len(guardbear_expected)} guardbear outputs and {len(other_expected)} other outputs')

        # Compare each output following the table rules
        compare_results = []

        for guardbear_output, other_output in zip(guardbear_expected, other_expected):
            current_result = {}
            current_result['original'] = get_value(
                'event.original', guardbear_output)
            checked_fields = set()
            checked_fields.add('event.original')

            # Need to match
            for field in fields_needed_to_match(df):
                waz_value = get_value(field, guardbear_output)
            ...
            # Extra guardbear fields and missing guardbear fields
            for field in fields_mapped_by_waz(df):
                waz_value = get_value(field, guardbear_output)
                oth_value = get_value(field, other_output)

                # GuardBear missing fields
                ...
                # GuardBear extra fields
                if waz_value is not None and oth_value is None and show_extra:
                    if 'guardbear_extra' not in current_result:
                        current_result['guardbear_extra'] = []
                    current_result['guardbear_extra'].append(field)
        ...
        # Notify the user
        print(
            f'{guardbear_output_path.name} and {other_output_path.name} -> {output_path}')

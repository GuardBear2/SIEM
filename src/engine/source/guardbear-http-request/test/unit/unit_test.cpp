...
TEST_F(UrlRequestUnitTest, GetFileHttp)
{
    ...
    EXPECT_CALL(*request, setOption(optUrl, "http://www.guardbear.com/")).Times(1);
    ...
    GetRequest::builder(request).url("http://www.guardbear.com/").outputFile("/tmp/hello_world.html").execute();
}
...
TEST_F(UrlRequestUnitTest, GetFileWithUnixSocket)
{
    ...
    EXPECT_CALL(*request, setOption(optUnixSocketPath, "/tmp/guardbear-agent.sock")).Times(1);
    EXPECT_CALL(*request, setOption(optUrl, "http://www.guardbear.com/")).Times(1);
    EXPECT_CALL(*request, setOption(optCustomRequest, "GET")).Times(1);
    EXPECT_CALL(*request, setOption(optWriteData, SafeMatcherCast<void*>(_))).Times(1);
    EXPECT_CALL(*request, setOption(optWriteFunction, zero)).Times(1);
    ...
    GetRequest::builder(request)
        .url("http://www.guardbear.com/")
        .unixSocketPath("/tmp/guardbear-agent.sock")
...
TEST_F(UrlRequestUnitTest, GetApiRequest)
{
    ...
    EXPECT_CALL(*request, setOption(optUserAgent, "GuardBear-Agent/1.0")).Times(1);
...
TEST_F(UrlRequestUnitTest, PostApiRequestWithPostFields)
{
    ...
    EXPECT_CALL(*request, setOption(optCainfo, "cert.ca")).Times(1);
    EXPECT_CALL(*request, setOption(optTimeout, 10)).Times(1);
    EXPECT_CALL(*request, setOption(optPostFields, R"({"name":"guardbear"})")).Times(1);
    EXPECT_CALL(*request, setOption(optPostFieldSize, std::string(R"({"name":"guardbear"})").length())).Times(1);
    EXPECT_CALL(*request, execute()).Times(1);
...
TEST_F(UrlRequestUnitTest, HttpsCertExists)
{
    ...
    EXPECT_CALL(*request, setOption(optUrl, "https://www.guardbear.com/")).Times(1);
    ...
    EXPECT_NO_THROW(getRequest.url("https://www.guardbear.com/").execute());
}
...
TEST_F(UrlRequestUnitTest, HttpsCertExistsNoVerify)
{
    ...
    EXPECT_CALL(getRequest, exists("/etc/ssl/certs/ca-certificates.crt")).Times(1).WillOnce(Return(true));
    EXPECT_NO_THROW(getRequest.url("https://www.guardbear.com/", secureCommunication).execute());
}
...
TEST_F(UrlRequestUnitTest, HttpsNoCertNotExists)
{
    ...
    EXPECT_NO_THROW(getRequestNoCert.url("https://www.guardbear.com/").execute());
}

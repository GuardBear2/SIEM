#include "utils/guardbearProtocol/guardbearRequest.hpp"
 ...
 */
std::optional<std::string> GuardBearRequest::validate() const
{
...
GuardBearRequest GuardBearRequest::create(std::string_view command, std::string_view originName, const json::Json& parameters)
{
    ...
    jrequest.setString("guardbear-engine", "/origin/module");
    jrequest.setString(originName, "/origin/name");

    return GuardBearRequest(jrequest);
}

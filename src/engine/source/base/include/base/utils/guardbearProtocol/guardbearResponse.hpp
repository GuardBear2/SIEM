#ifndef _BASE_UTILS_GUARDBEAR_RESPONSE_HPP
#define _BASE_UTILS_GUARDBEAR_RESPONSE_HPP
...
public:
    ...
    /**
     * @brief  Construct a new GuardBear Response object
     *
     * @param data Data to be sent, it can be a json object or a string
     * @param error Error code (0 if no error)
     * @param message Optional message
     */
    explicit GuardBearResponse(const json::Json& data, int error = 0, std::string_view message = "") noexcept
        : m_data(data)
        , m_error(error)
    {
     ...
     */
    explicit GuardBearResponse(json::Json&& data, int error = 0, std::string_view message = "") noexcept
        : m_data(data)
        , m_error(error)
    {
        m_message = message.empty() ? std::nullopt : std::optional<std::string> {message};
     ...
     */
    explicit GuardBearResponse(std::string_view message) noexcept
        : m_data(json::Json {R"({})"})
     ...
     * @param message Messsage to be sent as a response
     */
    explicit GuardBearResponse(json::Json& data, std::string_view message) noexcept
        : m_data(data)
        , m_error(static_cast<int>(RESPONSE_ERROR_CODES::OK))
        , m_message(message) {};
     ...
     */
    explicit GuardBearResponse() noexcept
        : m_data(json::Json {R"({})"})
        , m_error(static_cast<int>(RESPONSE_ERROR_CODES::OK))
        , m_message() {};
    ...
    /**
     * @brief Create a GuardBearResponse object from a string
     *
     * @param response Response as a string.
     * @return GuardBearResponse object.
     ...
     * }
     */
    static GuardBearResponse fromStr(std::string_view response)
    {
        ...
        auto ret = GuardBearResponse {std::move(data.value()), error.value()};
     ...
     *
     */
    static GuardBearResponse invalidJsonRequest()
    {
        return GuardBearResponse(json::Json(R"({})"),
     ...
     *
     */
    static GuardBearResponse invalidSize()
    {
        return GuardBearResponse(
     ...
     *
     */
    static GuardBearResponse unknownError()
    {
        return GuardBearResponse(
     ...
     *
     */
    static GuardBearResponse invalidRequest(const std::string& message)
    {
        return GuardBearResponse(json::Json(R"({})"),
     ...
     * process  the respons
     */
    static GuardBearResponse internalError(const std::string& message)
    {
        return GuardBearResponse(json::Json(R"({})"),
     ...
     *
     */
    static GuardBearResponse busyServer()
    {
        return GuardBearResponse(
...
#endif // _API_GUARDBEAR_RESPONSE_HPP

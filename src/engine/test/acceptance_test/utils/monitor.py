...
if platform == "darwin":
    agentd_state_file = "/Library/Ossec/var/run/guardbear-agentd.state"
elif platform == "win32":
    agentd_state_file = "C:\\Program Files (x86)\\ossec-agent\\guardbear-agent.state"
    enable_disk_monitoring_win()
else:
    agentd_state_file = "/var/ossec/var/run/guardbear-agentd.state"


guardbear_binaries = {
    "manager": set(["guardbear-analysisd", "guardbear-authd", "guardbear-execd",
                "guardbear-integratord", "guardbear-logcollector", "guardbear-maild",
                "guardbear-monitord", "guardbear-syscheckd", "guardbear-remoted",
                "guardbear-modulesd", "guardbear-clusterd", "guardbear-db", ]),
    "agent": set(["guardbear-agentd", "guardbear-execd", "guardbear-logcollector",
              "guardbear-syscheckd", "guardbear-modulesd", ]),
    "winagent": set(["guardbear-agent.exe", ])
}


def check_binary(binary_list):
    for binary in binary_list:
        for _, binaries in guardbear_binaries.items():

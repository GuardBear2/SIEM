Feature: Tester API Management
  ...
  Scenario: Add a repeated session for testing via API
    ...
    And I create a "default" session that points to policy "policy/guardbear/0"
    When I send a request to the tester to add a new session called "default" with the data from policy:"policy/guardbear/0"
  ...
  Scenario: Add a new session for testing via API
    Given I have a policy "policy/guardbear/0" that has an integration called "guardbear-core-test" loaded
    And I create a "default" session that points to policy "policy/guardbear/0"
    When I send a request to the tester to add a new session called "test1" with the data from policy:"policy/guardbear/0"
    Then I should receive a success response

  Scenario: Remove specific session via API
    Given I have a policy "policy/guardbear/0" that has an integration called "guardbear-core-test" loaded
    And I create a "default" session that points to policy "policy/guardbear/0"
    When I send a request to the tester to add 5 sessions called "dummy" with policy "policy/guardbear/0"
  ...
  Scenario: Send events to specific session without debug session via API
    Given I have a policy "policy/guardbear/0" that has an integration called "other-guardbear-core-test" loaded
    And I create a "test" session that points to policy "policy/guardbear/0"

...
def setup_engine(engine_src_dir, environment_dir):
    schemas = ["guardbear-logpar-overrides", "engine-schema"]
    ...
    engine_bin_dest = Path(environment_dir) / 'guardbear-engine'
...
def main():
    ...
    if environment_directory is None:
        print("environment_directory is optional. For default is guardbear directory. Usage: python script.py -e <environment_directory>")

    SCRIPT_DIR = os.path.dirname(os.path.realpath(__file__))
    ENGINE_SRC_DIR = os.path.join(SCRIPT_DIR, '../')
    GUARDBEAR_DIR = os.path.realpath(os.path.join(SCRIPT_DIR, '../../../'))
    ENVIRONMENT_DIR = environment_directory or os.path.join(GUARDBEAR_DIR, 'environment')

...
Requires python 3.8, to install navigate where the GuardBear repository folder is located and run:

`pip install guardbear/src/engine/test/helper_tests/engine-helper-test`

If we want to install for developing and modifying the scripts, install in editable mode and the additional dev packages:

`pip install -e guardbear/src/engine/test/helper_tests/engine-helper-test[dev]`

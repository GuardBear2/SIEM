...
- Indicate the GuardBear version.
- Tell if it has failed on a manager, an agent, or both.
- Attach logs that illustrate the bug inside the <detail> tag below -you may want to set debug options `guardbear_modules.debug=2` and restart GuardBear (see https://documentation.guardbear.com/current/user-manual/reference/internal-options.html) to get verbose logs. This may help investigate the issue-.

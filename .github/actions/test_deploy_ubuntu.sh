...
GUARDBEAR_REGISTRATION_PASSWORD_PATH="/var/ossec/etc/authd.pass"

function install_guardbear(){

  echo "Testing the following variables $*"
  eval "${*} apt install -y ./guardbear-agent_${VERSION}-0.commit${SHA}_amd64.deb > /dev/null 2>&1"
  
}

function remove_guardbear () {

  apt purge -y guardbear-agent > /dev/null 2>&1
...
echo "Download package: https://s3.us-west-1.amazonaws.com/packages-dev.guardbear.com/warehouse/pullrequests/${MAJOR}.${MINOR}/deb/var/guardbear-agent_${VERSION}-0.commit${SHA}_amd64.deb"
wget "https://s3.us-west-1.amazonaws.com/packages-dev.guardbear.com/warehouse/pullrequests/${MAJOR}.${MINOR}/deb/var/guardbear-agent_${VERSION}-0.commit${SHA}_amd64.deb" > /dev/null 2>&1

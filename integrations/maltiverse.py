...
This integration enrichs any alert generated by GuardBear via the Maltiverse API,
...
And restart GuardBear Manager:

    /etc/init.d/guardbear-manager restart
...
#    5 - Invalid URL
#    6 - Unable to connect to GuardBear
#    7 - Unexpected error sending message to GuardBear
...
def process_args(args: list):
    ...
    # GuardBear Manager in case of positive match
...
def send_event(msg: str, agent: dict = None):
    """Send an event to the GuardBear Manager.
    ...
    try:
        ...
        if e.errno == 111:
            print('ERROR: GuardBear is not running.')
            sys.exit(6)
        elif e.errno == 90:
            print('ERROR: Message too long to send to GuardBear.')
            sys.exit(7)

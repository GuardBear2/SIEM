#!/usr/share/guardbear-server/framework/python/bin/python3
...
from guardbear.core.cluster.cluster import clean_up
from guardbear.core.cluster.unix_server.server import start_unix_server
from guardbear.core.cluster.utils import ClusterLogger, context_tag, ping_unix_socket, print_version, process_spawn_sleep
from guardbear.core.common import CONFIG_SERVER_SOCKET_PATH, GUARDBEAR_RUN, GUARDBEAR_SHARE, guardbear_gid, guardbear_uid
from guardbear.core.config.client import CentralizedConfig
from guardbear.core.config.models.server import NodeType, ServerConfig
from guardbear.core.exception import GuardBearDaemonError
from guardbear.core.task.order import get_orders
from guardbear.core.utils import clean_pid_files, create_guardbear_dir
from guardbear.core.wlogging import GuardBearLogger

SERVER_DAEMON_NAME = 'guardbear-server'
COMMS_API_DAEMON_NAME = 'guardbear-comms-apid'
COMMS_API_SCRIPT_PATH = GUARDBEAR_SHARE / 'apis' / 'scripts' / f'{COMMS_API_DAEMON_NAME}.py'
ENGINE_BINARY_PATH = GUARDBEAR_SHARE / 'bin' / 'guardbear-engine'
ENGINE_DAEMON_NAME = 'guardbear-engined'
MANAGEMENT_API_DAEMON_NAME = 'guardbear-server-management-apid'
MANAGEMENT_API_SCRIPT_PATH = GUARDBEAR_SHARE / 'apis' / 'scripts' / f'{MANAGEMENT_API_DAEMON_NAME}.py'
...
def set_logging(debug_mode=0) -> GuardBearLogger:
    """Set cluster logger.
...
def start_daemon(name: str, args: List[str]):
    ...
    try:
        ...
        raise GuardBearDaemonError(f'Error starting {name}: {e}')
...
# Master main
#
async def master_main(args: argparse.Namespace, server_config: ServerConfig, logger: GuardBearLogger):
    ...
    logger : GuardBearLogger
        Cluster logger.
    """
    from guardbear.core.cluster import local_server, master
...
# Worker main
#
async def worker_main(args: argparse.Namespace, server_config: ServerConfig, logger: GuardBearLogger):
    ...
    from guardbear.core.cluster import local_server, worker
    ...
    # Handle exception when the user running GuardBear cannot access /dev/shm
        ...
        main_logger.warning(
            "In order to take advantage of GuardBear 4.3.0 cluster improvements, the directory '/dev/shm' must be "
            "accessible by the 'guardbear' user. Check that this file has permissions to be accessed by all users. "
            'Changing the file permissions to 777 will solve this issue.'
        )
        main_logger.warning(
            'The GuardBear cluster will be run without the improvements added in GuardBear 4.3.0 and higher versions.'
        )
        task_pool = None
...
def get_script_arguments() -> argparse.Namespace:
    ...
    start_parser = subparsers.add_parser('start', help='Start GuardBear server')
    ####################################################################################################################
    ...
    stop_parser = subparsers.add_parser('stop', help='Stop GuardBear server')
    stop_parser.set_defaults(func=stop)

    status_parser = subparsers.add_parser('status', help='Show the GuardBear server status')
...
def start():
    """Start function of the guardbear-server script in charge of starting the server process."""
    try:
        server_pid = pyDaemonModule.get_guardbear_server_pid(SERVER_DAEMON_NAME)
    ...
    # Create /run/guardbear-server
    create_guardbear_dir(GUARDBEAR_RUN)

    # Check for unused PID files
    clean_pid_files(SERVER_DAEMON_NAME)

    # Drop privileges to guardbear
    if not args.root:
        os.setgid(guardbear_gid())
        os.setuid(guardbear_uid())
    ...
    try:
        ...
        main_logger.error("Directory '/tmp' needs read, write & execution permission for 'guardbear-server' user")
    except GuardBearDaemonError as e:
        main_logger.error(e)
    except RuntimeError:
...
def check_daemon(processes: list, proc_name: str, children_number: int):
    ...
    if not _check_children_number(child, children_number):
        raise GuardBearDaemonError(f'Daemon `{proc_name}` does not have the correct number of children process.')
...
async def monitor_server_daemons(loop: asyncio.BaseEventLoop, server_process: psutil.Process):
    ...
    while True:
        ...
        except GuardBearDaemonError as error:
...
def stop():
    """Stop the GuardBear server running in background."""
    try:
        server_pid = pyDaemonModule.get_guardbear_server_pid(SERVER_DAEMON_NAME)
    except StopIteration:
        main_logger.warning('GuardBear server is not running.')
        sys.exit(0)
...
def status():
    """Show the status of the GuardBear server."""

...
from guardbear.core.security import (
    REQUIRED_FIELDS,
    SORT_FIELDS,
...
from guardbear.core.utils import process_array
from guardbear.rbac.decorators import expose_resources
from guardbear.rbac.orm import (
    MAX_ID_RESERVED,
...
def get_user_me(token: dict) -> AffectedItemsGuardBearResult:
    ...
    AffectedItemsGuardBearResult
        User information.
    """
    result = AffectedItemsGuardBearResult(all_msg='Current user information was returned')
    affected_items = list()
    ...
    affected_items.append(user_info) if user_info else result.add_failed_item(
        id_=common.current_user.get(), error=GuardBearError(5001)
    )
...
async def get_users(
    ...
    with AuthenticationManager() as auth:
        for user_id in user_ids:
            ...
            affected_items.append(user) if user else result.add_failed_item(id_=user_id, error=GuardBearError(5001))
...
async def edit_run_as(user_id: str = None, allow_run_as: bool = False) -> AffectedItemsGuardBearResult:
    ...
    with AuthenticationManager() as auth:
        ...
        if query is False:
            result.add_failed_item(id_=user_id, error=GuardBearError(5001))
        elif query == SecurityError.INVALID:
            result.add_failed_item(id_=user_id, error=GuardBearError(5010))
        else:
...
async def create_user(username: str = None, password: str = None) -> AffectedItemsGuardBearResult:
    """Create a new user.
    ...
    if len(password) > 64 or len(password) < 8:
        raise GuardBearError(5009)
    elif not _user_password.match(password):
        raise GuardBearError(5007)

    result = AffectedItemsGuardBearResult(none_msg='User could not be created', all_msg='User was successfully created')
    with AuthenticationManager() as auth:
        if auth.add_user(username, password):
            ...
            if operation:
                ...
                result.add_failed_item(id_=username, error=GuardBearError(5000))
        else:
            result.add_failed_item(id_=username, error=GuardBearError(5000))
...
async def update_user(user_id: str = None, password: str = None, current_user: str = None) -> AffectedItemsGuardBearResult:
    ...
    if password:
        ...
        if int(user_id[0]) <= MAX_ID_RESERVED and current_user is not None:
            ...
            if current_user_id > MAX_ID_RESERVED:
                raise GuardBearError(5011)

    result = AffectedItemsGuardBearResult(all_msg='User was successfully updated', none_msg='User could not be updated')
    with AuthenticationManager() as auth:
        query = auth.update_user(int(user_id[0]), password)
        if query is False:
            result.add_failed_item(id_=int(user_id[0]), error=GuardBearError(5001))
        else:
...
async def remove_users(user_ids: list) -> AffectedItemsGuardBearResult:
    ...
    with AuthenticationManager() as auth:
        for user_id in user_ids:
            ...
            if not isinstance(current_user, bool) and user_id == int(current_user['id']) and user_id > MAX_ID_RESERVED:
                result.add_failed_item(id_=user_id, error=GuardBearError(5008))
                continue
            user = auth.get_user_id(user_id)
            ...
            if not query:
                ...
                result.add_failed_item(id_=user_id, error=GuardBearError(5004))
            elif query == SecurityError.RELATIONSHIP_ERROR:
                result.add_failed_item(id_=user_id, error=GuardBearError(4025))
            elif user:
                invalid_users_tokens(users=[user_id])
    ...
    with RolesManager() as rm:
        for r_id in role_ids:
            ...
            if role != SecurityError.ROLE_NOT_EXIST:
                ...
                result.add_failed_item(id_=int(r_id), error=GuardBearError(4002))
...
async def remove_roles(role_ids: list) -> AffectedItemsGuardBearResult:
    ...
    with RolesManager() as rm:
        for r_id in role_ids:
            ...
            if role_delete == SecurityError.ADMIN_RESOURCES:
                result.add_failed_item(id_=int(r_id), error=GuardBearError(4008))
            elif role_delete == SecurityError.RELATIONSHIP_ERROR:
                result.add_failed_item(id_=int(r_id), error=GuardBearError(4025))
...
async def add_role(name: str = None) -> AffectedItemsGuardBearResult:
    ...
    AffectedItemsGuardBearResult
        Result of the operation.
    """
    result = AffectedItemsGuardBearResult(none_msg='Role was not created', all_msg='Role was successfully created')
    with RolesManager() as rm:
        status = rm.add_role(name=name)
        if status == SecurityError.ALREADY_EXIST:
            result.add_failed_item(id_=name, error=GuardBearError(4005))
        elif status == SecurityError.INVALID:
            result.add_failed_item(id_=name, error=GuardBearError(4003))
        else:
...
async def update_role(role_id: str = None, name: str = None) -> AffectedItemsGuardBearResult:
    ...
    if name is None:
        raise GuardBearError(4001)
    result = AffectedItemsGuardBearResult(none_msg='Role was not updated', all_msg='Role was successfully updated')
    with RolesManager() as rm:
        status = rm.update_role(role_id=role_id[0], name=name)
        if status == SecurityError.ALREADY_EXIST:
            result.add_failed_item(id_=int(role_id[0]), error=GuardBearError(4005))
        elif status == SecurityError.INVALID:
            result.add_failed_item(id_=int(role_id[0]), error=GuardBearError(4003))
        elif status == SecurityError.ROLE_NOT_EXIST:
            result.add_failed_item(id_=int(role_id[0]), error=GuardBearError(4002))
        elif status == SecurityError.ADMIN_RESOURCES:
            result.add_failed_item(id_=int(role_id[0]), error=GuardBearError(4008))
        else:
    ...
    with PoliciesManager() as pm:
        for p_id in policy_ids:
            ...
            if policy != SecurityError.POLICY_NOT_EXIST:
                ...
                result.add_failed_item(id_=int(p_id), error=GuardBearError(4007))
...
async def remove_policies(policy_ids: list = None) -> AffectedItemsGuardBearResult:
    ...
    with PoliciesManager() as pm:
        for p_id in policy_ids:
            ...
            if policy_delete == SecurityError.ADMIN_RESOURCES:
                result.add_failed_item(id_=int(p_id), error=GuardBearError(4008))
            elif policy_delete == SecurityError.RELATIONSHIP_ERROR:
                result.add_failed_item(id_=int(p_id), error=GuardBearError(4025))
...
async def add_policy(name: str = None, policy: dict = None) -> AffectedItemsGuardBearResult:
    ...
    AffectedItemsGuardBearResult
        Result of the operation.
    """
    result = AffectedItemsGuardBearResult(none_msg='Policy was not created', all_msg='Policy was successfully created')
    sanitize_rbac_policy(policy)
    with PoliciesManager() as pm:
        status = pm.add_policy(name=name, policy=policy)
        if status == SecurityError.ALREADY_EXIST:
            result.add_failed_item(id_=name, error=GuardBearError(4009))
        elif status == SecurityError.INVALID:
            result.add_failed_item(id_=name, error=GuardBearError(4006))
        else:
...
async def update_policy(policy_id: str = None, name: str = None, policy: dict = None) -> AffectedItemsGuardBearResult:
    ...
    if name is None and policy is None:
        raise GuardBearError(4001)
    result = AffectedItemsGuardBearResult(none_msg='Policy was not updated', all_msg='Policy was successfully updated')
    policy is not None and sanitize_rbac_policy(policy)
    with PoliciesManager() as pm:
        status = pm.update_policy(policy_id=policy_id[0], name=name, policy=policy)
        if status == SecurityError.ALREADY_EXIST:
            result.add_failed_item(id_=int(policy_id[0]), error=GuardBearError(4013))
        elif status == SecurityError.INVALID:
            result.add_failed_item(id_=int(policy_id[0]), error=GuardBearError(4006))
        elif status == SecurityError.POLICY_NOT_EXIST:
            result.add_failed_item(id_=int(policy_id[0]), error=GuardBearError(4007))
        elif status == SecurityError.ADMIN_RESOURCES:
            result.add_failed_item(id_=int(policy_id[0]), error=GuardBearError(4008))
        else:
    ...
    with RulesManager() as rum:
        for ru_id in rule_ids:
            ...
            if rule != SecurityError.RULE_NOT_EXIST:
                ...
                result.add_failed_item(id_=ru_id, error=GuardBearError(4022))
...
@expose_resources(actions=['security:create'], resources=['*:*:*'])
async def add_rule(name: str = None, rule: dict = None) -> AffectedItemsGuardBearResult:
...
async def remove_rules(rule_ids: list = None) -> AffectedItemsGuardBearResult:
    ...
    with RulesManager() as rum:
        for r_id in rule_ids:
            ...
            if role_delete == SecurityError.ADMIN_RESOURCES:
                ...
                result.add_failed_item(id_=int(r_id), error=GuardBearError(4022))
            elif rule:
...
async def update_rule(rule_id: str = None, name: str = None, rule: dict = None) -> AffectedItemsGuardBearResult:
    ...
    with RulesManager() as rum:
        ...
        if status == SecurityError.ALREADY_EXIST:
            result.add_failed_item(id_=int(rule_id[0]), error=GuardBearError(4005))
        elif status == SecurityError.INVALID:
            result.add_failed_item(id_=int(rule_id[0]), error=GuardBearError(4003))
        elif status == SecurityError.RULE_NOT_EXIST:
            result.add_failed_item(id_=int(rule_id[0]), error=GuardBearError(4022))
        elif status == SecurityError.ADMIN_RESOURCES:
            result.add_failed_item(id_=int(rule_id[0]), error=GuardBearError(4008))
        else:
...
def get_username(user_id: str) -> AffectedItemsGuardBearResult:
...
async def set_user_role(user_id: list, role_ids: list, position: int = None) -> AffectedItemsGuardBearResult:
    ...
    with UserRolesManager() as urm:
        for role_id in role_ids:
            ...
            if user_role == SecurityError.ALREADY_EXIST:
                result.add_failed_item(id_=int(role_id), error=GuardBearError(4017))
            elif user_role == SecurityError.ROLE_NOT_EXIST:
                result.add_failed_item(id_=int(role_id), error=GuardBearError(4002))
            elif user_role == SecurityError.USER_NOT_EXIST:
                result.add_failed_item(id_=int(user_id[0]), error=GuardBearError(5001))
                break
            elif user_role == SecurityError.ADMIN_RESOURCES:
                result.add_failed_item(id_=int(user_id[0]), error=GuardBearError(4008))
            else:
                success = True
                result.total_affected_items += 1
...
async def remove_user_role(user_id: str, role_ids: list) -> AffectedItemsGuardBearResult:
    ...
    Raises
    ------
    GuardBearResourceNotFound(5001)
    ...
    username = get_username(user_id=user_id)
    if username == 'unknown':
        raise GuardBearResourceNotFound(5001)
    ...
    with UserRolesManager() as urm:
        for role_id in role_ids:
            ...
            if user_role == SecurityError.INVALID:
                result.add_failed_item(id_=int(role_id), error=GuardBearError(4016))
            elif user_role == SecurityError.ROLE_NOT_EXIST:
                result.add_failed_item(id_=int(role_id), error=GuardBearError(4002))
            elif user_role == SecurityError.ADMIN_RESOURCES:
                result.add_failed_item(id_=int(user_id[0]), error=GuardBearError(4008))
...
async def set_role_rule(role_id: str, rule_ids: list, run_as: bool = False) -> AffectedItemsGuardBearResult:
    ...
    with RolesRulesManager() as rrm:
        for rule_id in rule_ids:
            ...
            if role_rule == SecurityError.ALREADY_EXIST:
                result.add_failed_item(id_=int(rule_id), error=GuardBearError(4023))
            elif role_rule == SecurityError.ROLE_NOT_EXIST:
                result.add_failed_item(id_=int(role_id[0]), error=GuardBearError(4002))
            elif role_rule == SecurityError.RULE_NOT_EXIST:
                result.add_failed_item(id_=int(rule_id), error=GuardBearError(4022))
...
async def remove_role_rule(role_id: str, rule_ids: list) -> AffectedItemsGuardBearResult:
    ...
    Raises
    ------
    GuardBearResourceNotFound(4002)
    ...
    if not role:
        raise GuardBearResourceNotFound(4002)
    ...
    with RolesRulesManager() as rrm:
        for rule_id in rule_ids:
            ...
            if role_rule == SecurityError.INVALID:
                result.add_failed_item(id_=rule_id, error=GuardBearError(4024))
            elif role_rule == SecurityError.RULE_NOT_EXIST:
                result.add_failed_item(id_=rule_id, error=GuardBearError(4022))
            elif role_rule == SecurityError.ADMIN_RESOURCES:
                result.add_failed_item(id_=int(role_id[0]), error=GuardBearError(4008))
            else:
                success = True
...
async def set_role_policy(role_id: str, policy_ids: list, position: int = None) -> AffectedItemsGuardBearResult:
    ...
    with RolesPoliciesManager() as rpm:
        for policy_id in policy_ids:
            ...
            if role_policy == SecurityError.ALREADY_EXIST:
                result.add_failed_item(id_=policy_id, error=GuardBearError(4011))
            elif role_policy == SecurityError.ROLE_NOT_EXIST:
                result.add_failed_item(id_=int(role_id[0]), error=GuardBearError(4002))
            elif role_policy == SecurityError.POLICY_NOT_EXIST:
                result.add_failed_item(id_=policy_id, error=GuardBearError(4007))
...
async def remove_role_policy(role_id: str, policy_ids: list) -> AffectedItemsGuardBearResult:
    ...
    with RolesPoliciesManager() as rpm:
        for policy_id in policy_ids:
            ...
            if role_policy == SecurityError.INVALID:
                result.add_failed_item(id_=policy_id, error=GuardBearError(4010))
            elif role_policy == SecurityError.POLICY_NOT_EXIST:
                result.add_failed_item(id_=policy_id, error=GuardBearError(4007))
            elif role_policy == SecurityError.ADMIN_RESOURCES:
                result.add_failed_item(id_=int(role_id[0]), error=GuardBearError(4008))
...
def revoke_current_user_tokens() -> GuardBearResult:
    ...
    return GuardBearResult({'message': f'User {common.current_user.get()} was successfully logged out'})
...
async def wrapper_revoke_tokens() -> GuardBearResult:
    """Revoke all tokens.
    ...
    return GuardBearResult({'message': 'Tokens were successfully revoked'})
...
@lru_cache(maxsize=None)
def get_rbac_resources(resource: str = None) -> GuardBearResult:
    ...
    """
    if not resource:
        return GuardBearResult({'data': load_spec()['x-rbac-catalog']['resources']})
    else:
        if resource not in load_spec()['x-rbac-catalog']['resources'].keys():
            raise GuardBearError(4019)
        return GuardBearResult({'data': {resource: load_spec()['x-rbac-catalog']['resources'][resource]}})


@lru_cache(maxsize=None)
def get_rbac_actions(endpoint: str = None) -> GuardBearResult:
    ...
    if endpoint and endpoint not in endpoints_list:
        raise GuardBearError(4020, extra_remediation=endpoints_list)
    info_data = load_spec()
    data = dict()
    for path, path_info in info_data['paths'].items():
    ...
    return GuardBearResult({'data': data})


@expose_resources(actions=['security:read_config'], resources=['*:*:*'])
async def get_security_config() -> GuardBearResult:
    ...
    """
    return GuardBearResult({'data': get_security_conf()})

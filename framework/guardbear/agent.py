...
from guardbear.core.indexer.agent import DEFAULT_GROUP
from guardbear.core.indexer.base import IndexerKey
from guardbear.core.indexer.commands import create_restart_command, create_set_group_command
from guardbear.core.indexer.models.agent import Host as IndexerAgentHost
from guardbear.core.indexer.models.commands import ResponseResult
...
from guardbear.core.utils import get_group_file_path, get_hash, process_array
...
def build_agents_query(agent_list: list, filters: dict) -> dict:
    ...
    """
    # TODO: The query build should be improved in https://github.com/guardbear/guardbear/issues/25289
...
def reconnect_agents(agent_list: Union[list, str] = None) -> AffectedItemsGuardBearResult:
    ...
    with GuardBearQueue(common.AR_SOCKET) as wq:
...
async def restart_agents(agent_list: list) -> AffectedItemsGuardBearResult:
    ...
    async with get_indexer_client() as indexer_client:
        ...
        if len(agent_list) == 0:
            ...
            for not_found_id in set(agent_list) - set(available_agents):
                result.add_failed_item(not_found_id, error=GuardBearResourceNotFound(1701))
        ...
        if len(agent_list) > 0:
            ...
            if response.result in (ResponseResult.OK, ResponseResult.CREATED):
                ...
                for agent_id in agent_list:
                    result.add_failed_item(id_=agent_id, error=GuardBearError(1762, extra_message=response.result.value))
...
async def get_agents_in_group(
    ...
    if group_id not in system_groups:
        raise GuardBearResourceNotFound(1710)
...
async def add_agent(
    ...
    return GuardBearResult({'data': new_agent})
...
async def get_agent_groups(
    ...
    if group_list:
        ...
        for invalid_group in set(group_list) - system_groups:
            ...
            result.add_failed_item(id_=invalid_group, error=GuardBearResourceNotFound(1710))
...
async def create_group(group_id: str) -> GuardBearResult:
    """Create a group.
            ...
            async with get_indexer_client() as indexer_client:
                ...
                if len(agents) > 0:
                    ...
                    if response.result not in (ResponseResult.OK, ResponseResult.CREATED):
                        raise GuardBearError(1762, extra_message=response.result.value)
    ...
    async with get_indexer_client() as indexer_client:
        ...
        for agent in available_agents:
            ...
            if agent.groups is not None and group_id in agent.groups:
                result.add_failed_item(id_=agent.id, error=GuardBearError(1766))
                continue
...
async def remove_agents_from_group(agent_list: list = None, group_list: list = None) -> AffectedItemsGuardBearResult:
        ...
        if len(agent_list) > 0:
            ...
            for agent in agents:
                if agent.groups is None or group_id not in agent.groups:
                    result.add_failed_item(agent.id, error=GuardBearError(1734))
                    continue
                ...
                if response.result in (ResponseResult.OK, ResponseResult.CREATED):
                    ...
                    for agent_id in agent_list:
                        result.add_failed_item(
                            id_=agent_id, error=GuardBearError(1762, extra_message=response.result.value)
                        )
...
@expose_resources(actions=['group:read'], resources=['group:id:{group_list}'], post_proc_func=None)
async def get_group_conf(group_list: list = None) -> GuardBearResult:
...
async def update_group_file(group_list: list = None, file_data: str = None) -> GuardBearResult:
    ...
    -------
    GuardBearResult
        GuardBearResult object with the confirmation message.
    """
    # We access unique group_id from list, this may change if and when we decide to add option to update files for
    # a list of groups
    group_id = group_list[0]

    return GuardBearResult({'message': configuration.update_group_file(group_id, file_data)})

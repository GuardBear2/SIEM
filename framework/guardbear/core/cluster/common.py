...
from guardbear.core.cluster import cluster
from guardbear.core.cluster import utils as cluster_utils
from guardbear.core.config.models.server import ServerConfig
...
class ReceiveFileTask:
    ...
    def __init__(self, guardbear_common, logger, task_id: bytes = b''):
        """Class constructor.

        Parameters
        ----------
        guardbear_common : GuardBearCommon object
            Instance of GuardBearCommon.
        ...
        """
        self.guardbear_common = guardbear_common
    ...
    def done_callback(self, future=None):
        ...
        """
        if self.task_id in self.guardbear_common.sync_tasks:
            del self.guardbear_common.sync_tasks[self.task_id]
...
class Handler(asyncio.Protocol):
    ...
    def __init__(self, server_config: ServerConfig, logger: logging.Logger = None, tag: str = 'Handler'):
        ...
        self.logger = logging.getLogger('guardbear') if not logger else logger
        # Logging tag.
        self.tag = tag
    ...
    async def send_file(self, filename: str, task_id: bytes = None) -> int:
        ...
        relative_path = str(filename.relative_to(common.GUARDBEAR_RUN)).encode()
        try:
            # Tell to the destination node where (inside guardbear_path) the file has to be written.
    ...
    async def send_string(self, my_str: bytes) -> bytes:
        ...
        except exception.GuardBearException as e:
    ...
    async def forward_dapi_response(self, data: bytes):
        ...
        try:
            ...
            if isinstance(e, exception.GuardBearException):
                exc = json.dumps(e, cls=GuardBearJSONEncoder)
            else:
                exc = json.dumps(exception.GuardBearClusterError(1000, extra_message=str(e)), cls=GuardBearJSONEncoder)
    ...
    def dispatch(self, command: bytes, counter: int, payload: bytes) -> None:
        ...
        try:
            ...
            command, payload = b'err', json.dumps(e, cls=GuardBearJSONEncoder).encode()
        except Exception as e:
            self.logger.error(f"Unhandled error processing request '{command}': {e}", exc_info=True)
            command, payload = (
                b'err',
                json.dumps(exception.GuardBearInternalError(1000, extra_message=str(e)), cls=GuardBearJSONEncoder).encode(),
            )
    ...
    def receive_file(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        bytes
            Response message.
        """
        self.in_file[data] = {'fd': open(common.GUARDBEAR_RUN / data.decode(), 'wb'), 'checksum': hashlib.sha256()}
    ...
    def cancel_task(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        ----------
        data : bytes
            String containing task_id and GuardBearJSONEncoded object with the exception details.
        ...
        error_json = json.loads(error_details, object_hook=as_guardbear_object)
        if task_id != b'None':
    ...
    def process_error_from_peer(self, data: bytes) -> bytes:
        ...
        """
        try:
            exc = json.loads(data.decode(), object_hook=as_guardbear_object)
        ...
        try:
            ...
            with contextlib.suppress(Exception):
                await self.send_request(
                    command=b'cancel_task', data=f'{task_id} {json.dumps(exc, cls=GuardBearJSONEncoder)}'.encode()
                )
            raise exc
...
class GuardBearCommon:
    """Task implementing common methods for both clients and servers that are GuardBear specific."""
    ...
    def end_receiving_file(self, task_and_file_names: str, logger_tag: str = '') -> Tuple[bytes, bytes]:
        ...
        if task_id not in self.sync_tasks:
            ...
            if os.path.exists(os.path.join(common.GUARDBEAR_RUN, filename)):
                try:
                    os.remove(os.path.join(common.GUARDBEAR_RUN, filename))
                except Exception as e:
                    self.get_logger(logger_tag).error(
                        f'Attempt to delete file {os.path.join(common.GUARDBEAR_RUN, filename)} failed: {e}'
                    )
        ...
        self.sync_tasks[task_id].filename = os.path.join(common.GUARDBEAR_RUN, filename)
    ...
    def error_receiving_file(self, task_id_and_error_details: str, logger_tag: str = '') -> Tuple[bytes, bytes]:
        ...
        task_id_and_error_details : str
             GuardBearJSONEncoded object with the exception details.
        logger_tag : str
            Logger task to use. If empty, it will use main class logger.
        ...
        error_details_json = json.loads(error_details, object_hook=as_guardbear_object)
    ...
    async def sync(
        ...
        try:
            ...
            await self.server.send_request(
                ...
                data=task_id + b' ' + os.path.relpath(compressed_data, common.GUARDBEAR_RUN).encode(),
            )
        except Exception as e:
            self.logger.error(f'Error sending zip file: {e}')
            if isinstance(e, exception.GuardBearException):
                exc = json.dumps(e, cls=GuardBearJSONEncoder).encode()
            else:
                exc = json.dumps(exception.GuardBearClusterError(1000, extra_message=str(e)), cls=GuardBearJSONEncoder).encode()
            with contextlib.suppress(Exception):
...
class GuardBearJSONEncoder(json.JSONEncoder):
    """Define special JSON encoder for GuardBear."""
    ...
    def default(self, obj):
        if callable(obj):
            ...
            if hasattr(obj, '__self__'):
                if isinstance(obj.__self__, GuardBear):
                    attributes['__guardbear__'] = obj.__self__.to_dict()
            attributes['__type__'] = type(obj).__name__
            return result
        elif isinstance(obj, exception.GuardBearException):
            result = {'__guardbear_exception__': {'__class__': obj.__class__.__name__, '__object__': obj.to_dict()}}
            return result
        elif isinstance(obj, wresults.AbstractGuardBearResult):
            result = {'__guardbear_result__': {'__class__': obj.__class__.__name__, '__object__': obj.encode_json()}}
            return result
        elif isinstance(obj, (datetime.datetime, datetime.date)):
            return {'__guardbear_datetime__': obj.isoformat()}
...
def as_guardbear_object(dct: Dict):
    try:
        if '__callable__' in dct:
            encoded_callable = dct['__callable__']
            funcname = encoded_callable['__name__']
            if '__guardbear__' in encoded_callable:
                # Encoded GuardBear instance method.
                guardbear = GuardBear()
                return getattr(guardbear, funcname)
            else:
        ...
        elif '__guardbear_exception__' in dct:
            guardbear_exception = dct['__guardbear_exception__']
            return getattr(exception, guardbear_exception['__class__']).from_dict(guardbear_exception['__object__'])
        elif '__guardbear_result__' in dct:
            guardbear_result = dct['__guardbear_result__']
            return getattr(wresults, guardbear_result['__class__']).decode_json(guardbear_result['__object__'])
        elif '__guardbear_datetime__' in dct:
            return datetime.datetime.fromisoformat(dct['__guardbear_datetime__'])
        ...
        raise exception.GuardBearInternalError(
            1000, extra_message=f'GuardBear object cannot be decoded from JSON {dct}', cmd_error=True
        )

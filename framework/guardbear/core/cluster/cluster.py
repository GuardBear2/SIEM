...
from guardbear.core.utils import blake2b, get_date_from_timestamp, get_utc_now, mkdir_with_mode
...
def validate_haproxy_helper_config(config: dict):
    ...
    try:
        ...
        raise GuardBearError(3004, f'Invalid value for {error.path.pop()}. {error.message}')


def validate_file_path(config: dict, key: str):
    """Validate a file path is within GUARDBEAR_ETC and that the file exists.
    ...
    if not filepath.is_relative_to(common.GUARDBEAR_ETC):
        raise GuardBearError(3004, f'The {key} path ({filepath}) is not inside {common.GUARDBEAR_ETC}.')

    if '..' in str(filepath):
        raise GuardBearError(3004, f'The {key} path ({filepath}) contains "..".')

    if not filepath.exists():
        raise GuardBearError(3004, f'The {key} "{filepath}" does not exist.')
...
def check_cluster_config(config):
    ...
    if config['node_type'] != 'master' and config['node_type'] != 'worker':
        raise GuardBearError(3004, f'Invalid node type {config["node_type"]}. Correct values are master and worker')

    if not isinstance(config['port'], int):
        raise GuardBearError(3004, 'Port has to be an integer.')

    if not MIN_PORT < config['port'] < MAX_PORT:
        raise GuardBearError(3004, f'Port must be higher than {MIN_PORT} and lower than {MAX_PORT}.')

    cert_keys = ['cafile', 'certfile', 'keyfile']
    if len(cert_keys) > len(set(config[key] for key in cert_keys)):
        raise GuardBearError(3004, 'Paths to certificates and keys must be different.')
...
def walk_dir(
    ...
    full_dirname = path.join(common.GUARDBEAR_ETC, dirname)
                ...
                for file_ in files_:
                    ...
                    try:
                        ...
                        if files == ['all'] or file_ in files:
                            relative_file_path = path.join(path.relpath(root_, common.GUARDBEAR_ETC), file_)
...
def compress_files(name, list_path, cluster_control_json=None, max_zip_size=None):
    ...
    zip_file_path = path.join(common.GUARDBEAR_QUEUE, name, f'{name}-{get_utc_now().timestamp()}-{uuid4().hex}.zip')
    ...
    with open(zip_file_path, 'ab') as wf:
        for file in list_path:
            ...
            try:
                with open(path.join(common.GUARDBEAR_ETC, file), 'rb') as rf:
                    new_file = rf.read()
                    if len(new_file) > max_zip_size:
                        result_logs['warning'][file].append(
                            f'File too large to be synced: {path.join(common.GUARDBEAR_ETC, file)}'
                        )
                        update_cluster_control(file, cluster_control_json)
                        continue
                ...
                raise GuardBearError(3001, str(e))
            except Exception as e:
                result_logs['debug'][file].append('Exception raised: ' + str(GuardBearException(3001, str(e))))
...
def clean_up(node_name=''):
    ...
    try:
        rm_path = path.join(common.GUARDBEAR_QUEUE, node_name)
...
def merge_info(merge_type, node_name, files=None, file_type=''):
    ...
    ----------
    merge_type : str
        Directory inside {guardbear_path}/PATH where the files to merge can be found.
    node_name : str
    ...
    """
    merge_path = path.join(common.GUARDBEAR_QUEUE, merge_type)
    ...
    with open(path.join(common.GUARDBEAR_RUN, output_file), 'wb') as o_f:
...
def unmerge_info(merge_type, path_file, filename):
    ...
    ----------
    merge_type : str
        Name of the destination directory inside queue. I.e: {guardbear_path}/PATH/{merge_type}/<unmerge_files>.
    path_file : str

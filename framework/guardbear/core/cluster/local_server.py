...
from guardbear.core.cluster import client, server
from guardbear.core.cluster import common as c_common
from guardbear.core.cluster.dapi import dapi
from guardbear.core.cluster.utils import context_tag
    ...
    def process_request(self, command: bytes, data: bytes):
        ...
        if command == b'dapi':
            ...
            if node_name in self.server.node.clients:
                ...
                raise GuardBearClusterError(3022)
        else:
            return super().process_request(command, data)
    ...
    def process_request(self, command: bytes, data: bytes):
        ...
        if command == b'dapi':
            if self.server.node.client is None:
                raise GuardBearClusterError(3023)
            asyncio.create_task(

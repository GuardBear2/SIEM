...
from guardbear.core.cluster import cluster, server
from guardbear.core.cluster import common as c_common
from guardbear.core.cluster.dapi import dapi
from guardbear.core.cluster.utils import context_tag, log_subprocess_execution
from guardbear.core.common import DECIMALS_DATE_FORMAT
...
class ReceiveIntegrityTask(c_common.ReceiveFileTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.guardbear_common.integrity_check
    ...
    def done_callback(self, future=None):
        ...
        # if not self.guardbear_common.extra_valid_requested:
        #     self.guardbear_common.sync_integrity_free[0] = True
...
class ReceiveExtraValidTask(c_common.ReceiveFileTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.guardbear_common.sync_extra_valid
    ...
    def done_callback(self, future=None):
        ...
        """
        super().done_callback(future)
        self.guardbear_common.extra_valid_requested = False
        self.guardbear_common.sync_integrity_free[0] = True


class MasterHandler(server.AbstractServerHandler, c_common.GuardBearCommon):
    ...
    def hello(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        ----------
        data : bytes
            Node name, node type and guardbear version all separated by spaces.
        ...
        worker_dir = common.GUARDBEAR_QUEUE / self.name
    ...
    def get_manager(self) -> server.AbstractServer:
        """Get the Master object that created this MasterHandler. Used in the class GuardBearCommon.
        ...
        try:
            with utils.Timeout(timeout):
                for file_path, data in files_metadata.items():
                    full_path = common.GUARDBEAR_RUN / file_path
                    ...
                    if data['merged']:
                        for unmerged_file_path, file_data, file_time in cluster.unmerge_info(
                            ...
                            try:
                                # Destination path.
                                full_unmerged_name = common.GUARDBEAR_QUEUE / unmerged_file_path
                                # Path where to create the file before moving it to the destination path.
                                tmp_unmerged_path = (
                                    common.GUARDBEAR_QUEUE / worker_name / os.path.basename(unmerged_file_path)
                                )

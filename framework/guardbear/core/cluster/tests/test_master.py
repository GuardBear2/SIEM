...
with patch('guardbear.core.common.guardbear_uid'):
    with patch('guardbear.core.common.guardbear_gid'):
        with patch.object(ValidateFilePathMixin, '_validate_file_path', return_value=None):
            ...
            from guardbear.core.cluster import client, master
            from guardbear.core.cluster import common as cluster_common
            from guardbear.core.cluster.dapi import dapi
            from guardbear.core.cluster.master import DEFAULT_DATE
...
async def test_rit_init():
    ...
    with patch.object(master_common, 'integrity_check') as integrity_check_mock:
        receive_integrity_task = master.ReceiveIntegrityTask(
            guardbear_common=master_common, logger=logging.getLogger('guardbear')
        )
        assert isinstance(receive_integrity_task.guardbear_common, cluster_common.GuardBearCommon)
...
def test_rit_set_up_coro(create_task_mock):
    ...
    guardbear_common_mock = GuardBearCommonMock()
    receive_integrity_task = master.ReceiveIntegrityTask(
        guardbear_common=guardbear_common_mock, logger=logging.getLogger('guardbear')
    )
    assert receive_integrity_task.set_up_coro() == guardbear_common_mock.integrity_check
    create_task_mock.assert_called_once()


@patch('asyncio.create_task')
@patch('guardbear.core.cluster.common.ReceiveFileTask.done_callback')
def test_rit_done_callback(super_callback_mock, create_task_mock):
    ...
    assert guardbear_common_mock.sync_integrity_free[0] is True
...
@patch('asyncio.create_task')
@patch('guardbear.core.cluster.master.ReceiveExtraValidTask.set_up_coro')
def test_revt_init(set_up_coro_mock, create_task_mock):
    ...
    receive_extra_valid_task = master.ReceiveExtraValidTask(
        guardbear_common=cluster_common.GuardBearCommon(), logger=logging.getLogger('guardbear')
    )

    assert isinstance(receive_extra_valid_task.guardbear_common, cluster_common.GuardBearCommon)
...
def test_revt_set_up_coro(create_task_mock):
    ...
    assert receive_extra_valid_task.set_up_coro() == guardbear_common_mock.sync_extra_valid
    create_task_mock.assert_called_once()
...
def test_revt_done_callback(set_up_coro_mock, super_callback_mock, create_task_mock):
    ...
    set_up_coro_mock.assert_called_once()
    assert guardbear_common_mock.sync_integrity_free[0] is True
    assert guardbear_common_mock.extra_valid_requested is False
...
def test_master_handler_init():
    ...
    with patch('guardbear.core.cluster.master.context_tag', ContextVar('', default='')) as cv:
...
@patch.object(logging.getLogger('guardbear'), 'debug')
def test_master_handler_process_request(logger_mock):
    ...
    with patch('guardbear.core.cluster.master.MasterHandler.get_permission', return_value=b'ok') as get_permission_mock:
    ...
    with patch(
        'guardbear.core.cluster.master.MasterHandler.setup_sync_integrity', return_value=b'ok'
    ...
    with patch(
        'guardbear.core.cluster.master.MasterHandler.end_receiving_integrity_checksums', return_value=b'ok'
    ...
    with patch(
        'guardbear.core.cluster.master.MasterHandler.process_sync_error_from_worker', return_value=b'ok'
    ...
    with patch('guardbear.core.cluster.master.MasterHandler.process_dapi_res', return_value=b'ok') as process_dapi_res_mock:
    ...
    with patch('guardbear.core.cluster.master.MasterHandler.get_nodes', return_value=(['cmd', 'res'])) as get_nodes_mock:
    ...
    with patch('guardbear.core.cluster.master.MasterHandler.get_health', return_value=(['cmd', 'res'])) as get_health_mock:
        with patch('json.loads', return_value=b'ok') as json_loads_mock:
            with patch('json.dumps', return_value='ok') as json_dumps_mock:
    ...
    with patch(
        'guardbear.core.cluster.server.AbstractServerHandler.process_request', return_value=b'ok'
...
@patch('guardbear.core.cluster.master.uuid4', return_value=10101010)
async def test_master_handler_execute_ok(uuid4_mock, wait_for_mock):
    ...
    with patch('guardbear.core.cluster.master.MasterHandler.send_request', return_value=b'result') as send_request_mock:
    ...
    with patch(
        'guardbear.core.cluster.master.MasterHandler.process_request', return_value=[b'ok', b'']
    ) as process_request_mock:
        return_values = await asyncio.gather(
...
async def test_master_handler_execute_ko(uuid4_mock):
    ...
    with patch('guardbear.core.cluster.master.MasterHandler.send_request', return_value=b'result'):
    ...
    with patch('guardbear.core.cluster.master.MasterHandler.process_request', return_value=[b'error', b'']):
        with pytest.raises(exception.GuardBearClusterError, match=r'.* 3022 .*'):
            await master_handler.execute(command=b'random', data=b'client request', wait_for_complete=True)


@patch('guardbear.core.cluster.common.SyncFiles', return_value='SyncFilesMock')
@patch('os.path.exists', return_value=False)
@patch('guardbear.core.cluster.master.utils.mkdir_with_mode')
@patch('guardbear.core.cluster.master.metadata.__version__', 'version')
@patch('guardbear.core.cluster.server.AbstractServerHandler.hello', return_value=(b'ok', 'payload'))
def test_master_handler_hello_ok(super_hello_mock, mkdir_with_mode_mock, path_exists_mock, sync_files_mock):
    ...
    mkdir_with_mode_mock.assert_called_once_with(common.GUARDBEAR_RUN / 'cluster' / 'name')
    path_exists_mock.assert_called_once_with(common.GUARDBEAR_RUN / 'cluster' / 'name')
...
@patch('guardbear.core.cluster.master.metadata.__version__', 'random')
@patch('guardbear.core.cluster.server.AbstractServerHandler.hello', return_value=(b'ok', 'payload'))
...
def test_master_handler_get_permission():
    ...
    class MockServer:
        ...
        def __init__(self):
            self.integrity_already_executed = ['not guardbear']

    master_handler = get_master_handler()
    master_handler.server = MockServer()
    master_handler.name = 'guardbear'
...
@patch('guardbear.core.cluster.common.GuardBearCommon.setup_receive_file', return_value=b'ok')
...
@patch('guardbear.core.cluster.common.GuardBearCommon.error_receiving_file', return_value=b'ok')
...
@patch('guardbear.core.cluster.common.GuardBearCommon.end_receiving_file', return_value=b'ok')
...
@patch('shutil.rmtree')
@patch('asyncio.wait_for')
@patch('guardbear.core.cluster.cluster.decompress_files', return_value=('files_metadata', '/decompressed/files/path'))
@patch(
    'guardbear.core.cluster.master.cluster.run_in_pool',
...
async def test_master_handler_sync_worker_files_ok(run_in_pool_mock, decompress_files_mock, wait_for_mock, rmtree_mock):
    ...
    await asyncio.gather(
        master_handler.sync_worker_files('task_id', event, logging.getLogger('guardbear')), unlock_event(event)
    )
...
@pytest.mark.asyncio
@patch('shutil.rmtree')
@patch('guardbear.core.cluster.master.cluster.run_in_pool', side_effect=Exception)
@patch('guardbear.core.cluster.cluster.decompress_files', return_value=('files_metadata', '/decompressed/files/path'))
@patch('guardbear.core.cluster.master.MasterHandler.wait_for_file')
async def test_master_handler_sync_worker_files_ko(wait_for_mock, decompress_files_mock, run_in_pool_mock, rmtree_mock):
    ...
    with pytest.raises(Exception):
        await master_handler.sync_worker_files('task_id', asyncio.Event(), logging.getLogger('guardbear'))
...
@pytest.mark.asyncio
@patch('guardbear.core.cluster.master.MasterHandler.set_date_end_master')
@patch('guardbear.core.cluster.master.MasterHandler.sync_worker_files')
async def test_master_handler_sync_extra_valid(sync_worker_files_mock, set_date_end_master_mock):
    ...
    master_handler = get_master_handler()
    master_handler.task_loggers['Integrity sync'] = logging.getLogger('guardbear')
    await master_handler.sync_extra_valid('task_id', None)

    sync_worker_files_mock.assert_called_once_with('task_id', None, logging.getLogger('guardbear'))
    set_date_end_master_mock.assert_called_once_with(logging.getLogger('guardbear'))
...
@patch.object(logging.getLogger('guardbear'), 'info')
def test_set_date_end_master(info_mock):
    ...
    master_handler.set_date_end_master(logging.getLogger('guardbear'))
...
@patch.object(logging.getLogger('guardbear'), 'info')
@patch.object(logging.getLogger('guardbear'), 'debug')
@patch('guardbear.core.cluster.master.MasterHandler.integrity_sync')
@patch('guardbear.core.cluster.master.MasterHandler.wait_for_file')
@patch('guardbear.core.cluster.master.MasterHandler.send_request', return_value=b'ok')
@patch('guardbear.core.cluster.cluster.decompress_files', return_value=('files_metadata', '/decompressed/files/path'))
async def test_master_handler_integrity_check(
    ...
    master_handler.server = ServerMock()
    master_handler.sync_tasks = {'task_id': TaskMock()}
    master_handler.task_loggers['Integrity check'] = logging.getLogger('guardbear')

    with patch(
        'guardbear.core.cluster.cluster.compare_files', return_value=({'extra_valid': compare_result})
    ) as compare_mock:
...
@pytest.mark.asyncio
@patch('guardbear.core.cluster.master.MasterHandler.wait_for_file', return_value=Exception())
...
@patch('guardbear.core.cluster.master.utils.safe_move')
@patch('guardbear.core.common.guardbear_uid', return_value='guardbear_uid')
@patch('guardbear.core.common.guardbear_gid', return_value='guardbear_gid')
def test_master_handler_process_files_from_worker_ok(gid_mock, uid_mock, safe_move_mock, path_join_mock):
    ...
    with patch(
        'guardbear.core.cluster.cluster.unmerge_info',
        ...
        with patch('os.path.isfile', return_value=True) as isfile_mock:
            with patch('os.stat', return_value=StatMock()) as os_stas_mock:
                ...
                path_join_mock.assert_has_calls(
                    [
                        call(common.GUARDBEAR_PATH, 'data'),
                        call(common.GUARDBEAR_PATH, '/file/path'),
                        call(common.GUARDBEAR_PATH, 'queue', 'cluster', 'guardbear', 'path'),
                    ]
                )
    ...
    path_join_mock.assert_has_calls([call(common.GUARDBEAR_PATH, 'data'), call(decompressed_files_path, 'data')])
...
@patch.object(logging.getLogger('guardbear'), 'info')
@patch('guardbear.core.cluster.master.server.AbstractServerHandler.connection_lost')
@patch('guardbear.core.cluster.master.cluster.clean_up')
def test_master_handler_connection_lost(clean_up_mock, connection_lost_mock, logger_mock, worker_name):
    ...
    master_handler = get_master_handler()
    master_handler.logger = logging.getLogger('guardbear')
...
@patch.object(logging.getLogger('guardbear'), 'warning')
@patch('asyncio.get_running_loop', return_value=loop)
@patch('guardbear.core.cluster.master.ProcessPoolExecutor')
def test_master_init(pool_executor_mock, get_running_loop_mock, warning_mock):
    ...
    warning_mock.assert_has_calls(
        [
            ...
            call('The GuardBear cluster will be run without the improvements added in GuardBear 4.3.0 and higher versions.'),
        ]
    )
...
@patch('asyncio.get_running_loop', return_value=loop)
@patch('guardbear.core.cluster.master.metadata.__version__', '1.0.0')
...
@freeze_time('2021-11-02')
@patch('asyncio.sleep')
@patch('guardbear.core.cluster.master.cluster.run_in_pool', return_value={})
async def test_master_file_status_update(run_in_pool_mock, asyncio_sleep_mock):
    ...
    with patch(
        'guardbear.core.cluster.master.Master.setup_task_logger', return_value=logger_mock
...
@patch('asyncio.get_running_loop', return_value=loop)
@patch('guardbear.core.agent.Agent.get_agents_overview', return_value={'totalItems': 5})

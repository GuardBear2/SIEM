...
with patch('guardbear.common.guardbear_uid'):
    with patch('guardbear.common.guardbear_gid'):
        with patch.object(ValidateFilePathMixin, '_validate_file_path', return_value=None):
            ...
            from guardbear.core.cluster.local_server import *
...
async def test_LocalServerHandler_connection_made(event_loop):
    ...
    with patch.object(logger, 'debug') as logger_debug_mock:
        with patch('guardbear.core.cluster.local_server.context_tag', ContextVar('tag', default='')) as mock_contextvar:
...
@pytest.mark.asyncio
@patch('guardbear.core.cluster.local_server.server.AbstractServerHandler.process_request')
...
@pytest.mark.asyncio
@patch('guardbear.core.cluster.local_server.LocalServerHandler.send_request', return_value='changed')
...
async def test_LocalServerHandlerMaster_process_request(process_request_mock, event_loop):
    ...
    with pytest.raises(GuardBearClusterError, match='.* 3022 .*'):
...
@pytest.mark.asyncio
@patch('guardbear.core.cluster.local_server.LocalServerHandler.process_request')
async def test_LocalServerHandlerWorker_process_request(process_request_mock, event_loop):
    ...
    with pytest.raises(GuardBearClusterError, match='.* 3023 .*'):
    ...
    with patch.object(ls.node.client, 'send_request', return_value='') as send_request_mock:
        with patch.object(lshw, 'log_exceptions', return_value='') as log_exceptions_mock:
            with patch.object(lshw, 'get_api_response', return_value='') as callback_mock:
                assert lshw.send_request_to_master(command=b'test', arguments=b'guardbear') == (
                    b'ok',
                    b'Sent request to master node',
                )
                await wait_function_called(callback_mock)
                await wait_function_called(log_exceptions_mock)
                send_request_mock.assert_called_once_with(b'test', b'guardbear')

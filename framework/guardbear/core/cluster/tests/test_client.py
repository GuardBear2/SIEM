...
with patch('guardbear.common.guardbear_uid'):
    with patch('guardbear.common.guardbear_gid'):
        ...
        import guardbear.core.cluster.client as client
        from guardbear import GuardBearException

from guardbear.core.config.models.server import NodeConfig, NodeType, ServerConfig, SSLConfig, ValidateFilePathMixin
...
def test_acm_init():
    ...
    assert abstract_client_manager.logger == logging.getLogger('guardbear')
...
@patch('guardbear.core.cluster.client.AbstractClientManager.add_tasks', return_value=[])
async def test_acm_start(add_tasks_mock, starmap_mock, asyncio_sleep_mock):
    ...
    with patch.object(
        ...
        with (
            patch.object(logging.getLogger('guardbear'), 'info') as logger_info_mock,
            patch.object(logging.getLogger('guardbear'), 'error') as logger_error_mock,
...
def test_ac_connection_result():
    ...
    with patch.object(logging.getLogger('guardbear'), 'error') as logger_mock:
        ...
        with patch.object(abstract_client.transport, 'close') as close_mock:
            ...
            future.set_result([GuardBearClusterError(3020)])
            abstract_client.connection_result(future)
            logger_mock.assert_called_once_with(f'Could not connect to master: {str(GuardBearClusterError(3020))}.')
            ...
            future = asyncio.Future()
            future.set_exception(GuardBearClusterError(3020))
            abstract_client.connection_result(future)
            logger_mock.assert_called_once_with(f'Could not connect to master: {str(GuardBearClusterError(3020))}.')
            close_mock.assert_called_once()

    # Check second condition
    with patch.object(logging.getLogger('guardbear'), 'info') as logger_mock:
        abstract_client.transport = CloseMock()
        future = asyncio.Future()
...
@patch('guardbear.core.cluster.client.AbstractClient._cancel_all_tasks')
def test_ac_connection_lost(cancel_tasks_mock):
    ...
    with patch.object(logging.getLogger('guardbear'), 'error') as logger_mock:
        abstract_client.connection_lost(exc=GuardBearException(1001))
        logger_mock.assert_called_once_with(
            f'Connection closed due to an unhandled error: {GuardBearException(1001)}\n', exc_info=False
        )
...
def test_ac_process_response():
    ...
    # Test the second condition
    with patch('guardbear.core.cluster.common.Handler.process_response', return_value=b'ok') as pr_mock:
...
def test_ac_process_request():
    ...
    # Test the fist condition
    with patch('guardbear.core.cluster.client.AbstractClient.echo_client', return_value=b'ok') as echo_mock:
        assert abstract_client.process_request(command=b'echo-m', data=b'data') == b'ok'
        echo_mock.assert_called_once_with(b'data')

    # Test the second condition
    with patch('guardbear.core.cluster.common.Handler.process_request', return_value=b'ok') as pr_mock:
...
@patch('guardbear.core.cluster.client.AbstractClient.send_request', return_value=b'ok')
async def test_ac_client_echo_ok(send_request_mock, done_mock, asyncio_sleep_mock):
    ...
    with patch.object(logging.getLogger('guardbear'), 'info') as logger_mock:
        with patch(
            'guardbear.core.cluster.common.Handler.setup_task_logger', return_value=logging.getLogger('guardbear')
...
@patch.object(FutureMock, 'done', return_value=False)
@patch('guardbear.core.cluster.client.perf_counter', return_value=0)
@patch('guardbear.core.cluster.client.AbstractClient.send_request', return_value='ok')
...
@freeze_time('2022-01-01')
@patch('guardbear.core.cluster.client.perf_counter', return_value=0)
@patch('guardbear.core.cluster.client.AbstractClient.send_file', return_value='ok')
...
@freeze_time('2022-01-01')
@patch('guardbear.core.cluster.client.perf_counter', return_value=0)
@patch('guardbear.core.cluster.client.AbstractClient.send_string', return_value='ok')

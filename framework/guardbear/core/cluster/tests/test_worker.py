...
import pytest
import guardbear.core.exception as exception
...
with patch('guardbear.core.common.guardbear_uid'):
    with patch('guardbear.core.common.guardbear_gid'):
        with patch.object(ValidateFilePathMixin, '_validate_file_path', return_value=None):
            ...
            from guardbear.core import common as core_common
            from guardbear.core.cluster import client, worker
...
async def test_rit_set_up_coro(event_loop):
    ...
    with patch(
        'guardbear.core.cluster.worker.WorkerHandler.process_files_from_master', return_value=''
    ) as process_files_mock:
        receive_task = worker.ReceiveIntegrityTask(guardbear_common=get_worker_handler(event_loop), logger=None)
        receive_task.guardbear_common = cluster_common.GuardBearCommon()
...
async def test_rit_done_callback(event_loop):
    ...
    with patch('guardbear.core.cluster.worker.ReceiveIntegrityTask.set_up_coro', side_effect=return_coro):
        receive_task = worker.ReceiveIntegrityTask(guardbear_common=get_worker_handler(event_loop), logger=None)
        receive_task.guardbear_common = cluster_common.GuardBearCommon()
        ...
        assert receive_task.guardbear_common.check_integrity_free is True
...
@patch('guardbear.core.utils.mkdir_with_mode')
@patch('guardbear.core.cluster.worker.client.AbstractClient.connection_result')
async def test_worker_handler_connection_result(connection_result_mock, mkdir_with_mode_mock, exists_mock, event_loop):
    ...
    exists_mock.assert_called_once_with(core_common.GUARDBEAR_QUEUE / 'Testing')
    mkdir_with_mode_mock.assert_called_once_with(core_common.GUARDBEAR_QUEUE / 'Testing')
...
async def test_worker_handler_process_request_ok(logger_mock, event_loop):
    ...
    worker_handler = get_worker_handler(event_loop)
    worker_handler.logger = logging.getLogger('guardbear')
    ...
    with patch('guardbear.core.cluster.worker.WorkerHandler.sync_integrity_ok_from_master', return_value=b'ok') as ok_mock:
    ...
    with patch(
        'guardbear.core.cluster.worker.WorkerHandler.setup_receive_files_from_master', return_value=b'ok'
    ) as setup_mock:
    ...
    with patch('guardbear.core.cluster.worker.WorkerHandler.end_receiving_integrity', return_value=b'ok') as integrity_mock:
    ...
    with patch(
        'guardbear.core.cluster.worker.WorkerHandler.error_receiving_integrity', return_value=b'ok'
    ...
    with patch('guardbear.core.cluster.worker.WorkerHandler.forward_dapi_response', side_effect=b'ok') as forward_dapi_mock:
    ...
    with patch(
        'guardbear.core.cluster.worker.client.AbstractClient.process_request', return_value=True
...
@patch('guardbear.core.cluster.worker.client.AbstractClient.connection_lost')
@patch('guardbear.core.cluster.worker.cluster.clean_up')
...
@patch.object(logging.getLogger('guardbear.Integrity check'), 'info')
@patch('guardbear.core.cluster.common.GuardBearCommon.setup_receive_file', return_value='OK')
...
@pytest.mark.asyncio
@patch('guardbear.core.cluster.common.GuardBearCommon.end_receiving_file', return_value=(b'OK', b'OK'))
...
@pytest.mark.asyncio
@patch('guardbear.core.cluster.common.GuardBearCommon.error_receiving_file', return_value=(b'error', b'error'))
...
@freeze_time('1970-01-01')
@pytest.mark.asyncio
@patch.object(guardbear.core.cluster.worker.json, 'dumps', return_value='')
@patch.object(logging.getLogger('guardbear.Integrity check'), 'info')
@patch.object(logging.getLogger('guardbear.Integrity check'), 'error')
@patch('guardbear.core.cluster.common.SyncFiles.sync', return_value=True)
@patch('guardbear.core.cluster.cluster.get_files_status', return_value={})
@patch('guardbear.core.cluster.worker.client.common.Handler.send_request')
@patch('guardbear.core.cluster.common.SyncTask.request_permission', return_value=True)
async def test_worker_handler_sync_integrity(
    ...
    # Test the try
    with patch(
        'guardbear.core.cluster.worker.cluster.run_in_pool', side_effect=cluster_run_in_pool_mock
    ) as run_in_pool_mock:
        try:
        ...
        run_in_pool_mock.side_effect = exception.GuardBearException(1001)
        try:
            await asyncio.wait_for(worker_handler.sync_integrity(), 0.2)
        except asyncio.exceptions.TimeoutError:
            pass

        error_mock.assert_called_with(f'Error synchronizing integrity: {exception.GuardBearException(1001)}')
        json_dumps_mock.assert_called_with(exception.GuardBearException(1001), cls=cluster_common.GuardBearJSONEncoder)
        ...
        json_dumps_mock.assert_called_with(
            exception.GuardBearClusterError(code=1000, extra_message=str(Exception())), cls=cluster_common.GuardBearJSONEncoder
        )
...
@pytest.mark.asyncio
@freeze_time('1970-01-01')
@patch.object(logging.getLogger('guardbear.Integrity sync'), 'debug')
@patch('guardbear.core.cluster.common.SyncFiles.sync', return_value=True)
@patch('guardbear.core.cluster.worker.perf_counter', return_value=0)
@patch('guardbear.core.cluster.cluster.merge_info', return_value=('n_files', 'merged_file'))
async def test_worker_handler_sync_extra_valid(
    ...
    worker_handler = get_worker_handler(event_loop)
    worker_handler.server = ManagerMock()
    with patch.object(logging.getLogger('guardbear.Integrity sync'), 'info') as logger_info_mock:
    ...
    with patch('guardbear.core.cluster.worker.WorkerHandler.send_request') as send_request_mock:
        with patch.object(logging.getLogger('guardbear.Integrity sync'), 'error') as logger_error_mock:
            merge_info_mock.side_effect = exception.GuardBearException(1001)
            cls = cluster_common.GuardBearJSONEncoder
            await worker_handler.sync_extra_valid(extra_valid)
            ...
            logger_error_mock.assert_called_once_with(
                f'Error synchronizing extra valid files: {exception.GuardBearException(1001)}'
            )
            send_request_mock.assert_called_once_with(
                command=b'syn_i_w_m_r', data=b'None ' + json.dumps(exception.GuardBearException(1001), cls=cls).encode()
            )
...
@patch('guardbear.core.cluster.cluster.decompress_files')
@patch.object(logging.getLogger('guardbear.Integrity sync'), 'info')
@patch.object(logging.getLogger('guardbear.Integrity sync'), 'debug')
@patch('guardbear.core.cluster.worker.client.common.Handler.send_request')
@patch('guardbear.core.cluster.worker.WorkerHandler.update_master_files_in_worker')
async def test_worker_handler_process_files_from_master_ok(
    ...
    with patch('guardbear.core.cluster.cluster.run_in_pool', side_effect=cluster_run_in_pool_mock) as run_in_pool_mock:
        ...
        # Test first except
        worker_handler.sync_tasks['task_id'] = TaskMock()
        decompress_files_mock.side_effect = exception.GuardBearException(1001)
        event = asyncio.Event()
        await asyncio.gather(
            worker_handler.process_files_from_master(name='task_id', file_received=event), unlock_event(event)
        )
        ...
        json_dumps_mock.assert_called_once_with(exception.GuardBearException(1001), cls=cluster_common.GuardBearJSONEncoder)
...
@patch('builtins.open')
@patch('os.path.exists', return_value=False)
@patch('guardbear.core.cluster.worker.safe_move')
@patch('guardbear.core.cluster.worker.utils.mkdir_with_mode')
...
async def test_worker_handler_update_master_files_in_worker_ok(
    ...
    with patch('guardbear.core.cluster.cluster.unmerge_info', return_value=[('name', 'content', '_')]):
        with patch('os.remove') as os_remove_mock:
            ...
            path_join_mock.assert_has_calls(
                [
                    call(core_common.GUARDBEAR_PATH, 'filename1'),
                    call(core_common.GUARDBEAR_PATH, 'name'),
                    call(core_common.GUARDBEAR_PATH, 'filename1'),
                    call('/zip/path', 'filename1'),
                    call(core_common.GUARDBEAR_PATH, 'filename3'),
                ]
            )
            guardbear_uid_mock.assert_called_with()
            guardbear_gid_mock.assert_called_with()
    ...
    path_join_mock.assert_has_calls(
        [
            call(core_common.GUARDBEAR_PATH, 'filename1'),
            call(core_common.GUARDBEAR_PATH, 'filename2'),
            call(core_common.GUARDBEAR_PATH, 'filename3'),
        ]
    )
    guardbear_uid_mock.assert_not_called()
    guardbear_gid_mock.assert_not_called()
    ...
    with patch('os.listdir', return_value='dir_files') as listdir_mock:
        with patch('shutil.rmtree') as rmtree_mock:
            ...
            path_join_mock.assert_has_calls(
                [call(core_common.GUARDBEAR_PATH, 'filename3'), call(core_common.GUARDBEAR_PATH, '')]
            )
            guardbear_uid_mock.assert_not_called()
            guardbear_gid_mock.assert_not_called()
            mkdir_with_mode_mock.assert_not_called()
    ...
    path_join_mock.assert_has_calls([call(core_common.GUARDBEAR_PATH, 'filename3'), call(core_common.GUARDBEAR_PATH, '')])
    guardbear_uid_mock.assert_not_called()
    guardbear_gid_mock.assert_not_called()
    mkdir_with_mode_mock.assert_not_called()
    safe_move_mock.assert_not_called()
...
@pytest.mark.asyncio
@patch('guardbear.core.cluster.worker.metadata.__version__', '1.0.0')
@patch('guardbear.core.cluster.worker.dapi.APIRequestQueue', return_value='APIRequestQueue object')
...
@pytest.mark.asyncio
@patch('guardbear.core.cluster.client.AbstractClientManager.add_tasks', return_value=['task'])
@patch('guardbear.core.cluster.worker.dapi.APIRequestQueue', return_value='APIRequestQueue object')

...
with patch('guardbear.common.guardbear_uid'):
    with patch('guardbear.common.guardbear_gid'):
        ...
        from guardbear.core.cluster.server import *
        from guardbear.core.exception import GuardBearClusterError, GuardBearError, GuardBearResourceNotFound
...
async def test_AbstractServerHandler_init(event_loop):
    ...
    with patch('guardbear.core.cluster.server.context_tag', ContextVar('tag', default='')) as mock_contextvar:
...
@pytest.mark.asyncio
@patch('guardbear.core.cluster.server.AbstractServerHandler.hello')
@patch('guardbear.core.cluster.server.AbstractServerHandler.echo_master')
@patch('guardbear.core.cluster.common.Handler.process_request')
async def test_AbstractServerHandler_process_request(mock_process_request, mock_echo_master, mock_hello, event_loop):
    ...
    abstract_server_handler.process_request(command=b'echo-c', data=b'guardbear')
    mock_echo_master.assert_called_once_with(b'guardbear')
...
async def test_AbstractServerHandler_echo_master(event_loop):
    ...
    assert abstract_server_handler.echo_master(data=b'guardbear') == (b'ok-m ', b'guardbear')
    abstract_server_handler.echo_master(data=b'guardbear')
...
def test_AbstractServerHandler_hello(event_loop):
    ...
    with pytest.raises(GuardBearClusterError, match='.* 3029 .*'):
    ...
    with pytest.raises(GuardBearClusterError, match='.* 3028 .* if_test'):
        abstract_server_handler.hello(b'if_test')
    assert abstract_server_handler.name == ''


@pytest.mark.asyncio
@patch('guardbear.core.cluster.common.Handler.process_response')
...
@pytest.mark.asyncio
@patch('asyncio.Queue')
@patch('guardbear.core.cluster.server.functools')
...
@patch('guardbear.core.cluster.server.AbstractServer.check_clients_keepalive')
@patch('guardbear.core.cluster.server.AbstractServerHandler')
...
@patch('guardbear.core.cluster.server.uuid4', return_value='abc123')
@patch('asyncio.get_running_loop', new=Mock())
...
@patch('guardbear.core.cluster.server.utils.process_array')
@patch('asyncio.get_running_loop', new=Mock())
def test_AbstractServer_get_connected_nodes(mock_process_array):
    ...
    with patch.object(abstract_server, 'to_dict', return_value=basic_dict):
        abstract_server.get_connected_nodes(
            search={'value': 'guardbear', 'negation': True},
...
def test_AbstractServer_get_connected_nodes_ko(mock_process_array):
    ...
    with patch.object(abstract_server, 'to_dict', return_value=basic_dict):
        ...
        with pytest.raises(GuardBearError, match='.* 1724 .* Not a valid select field: no'):
            abstract_server.get_connected_nodes(select={'no': 'exists'})

        with pytest.raises(GuardBearError, match='.* 1728 .*'):
            abstract_server.get_connected_nodes(filter_type='None')

        abstract_server.server_config.node.name = 'master'
        with pytest.raises(GuardBearResourceNotFound, match='.* 1730 .*'):
...
async def test_AbstractServer_check_clients_keepalive(sleep_mock):
    ...
    with patch.object(logger, 'debug') as mock_debug:
        with patch.object(logger, 'error') as mock_error:
            with patch('guardbear.core.cluster.server.AbstractServer.setup_task_logger', return_value=logger):
                abstract_server = AbstractServer(
                    performance_test=1,
                    concurrency_test=2,
                    server_config=default_config.server,
...
@patch('asyncio.sleep', side_effect=IndexError)
@patch('asyncio.get_running_loop', new=Mock())
@patch('guardbear.core.cluster.server.perf_counter', return_value=0)
...
async def test_AbstractServer_start(keepalive_mock, mock_path_join):
    ...
    with (
        patch('guardbear.core.cluster.server.context_tag', ContextVar('tag', default='')) as mock_contextvar,

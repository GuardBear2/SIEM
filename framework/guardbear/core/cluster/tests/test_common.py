...
from guardbear import GuardBear
from guardbear.core import exception
from guardbear.core.cluster.tests.conftest import get_default_configuration
...
with patch('guardbear.common.guardbear_uid'):
    with patch('guardbear.common.guardbear_gid'):
        with patch.object(ValidateFilePathMixin, '_validate_file_path', return_value=None):
            ...
            import guardbear.core.cluster.common as cluster_common
            import guardbear.core.results as wresults
            from guardbear.core import common

# Globals
guardbear_common = cluster_common.GuardBearCommon()
...
@patch.object(logging.getLogger('guardbear'), 'error')
@patch('guardbear.core.cluster.common.ReceiveFileTask.set_up_coro')
@patch('guardbear.core.cluster.common.uuid4', return_value='e6c45993-b0ae-438f-8914-3b5175b2bbfd')
def test_rft_init(uuid_mock, setup_coro_mock, logger_mock, create_task_mock, event_mock):
    ...
    with patch.object(TaskMock, 'add_done_callback') as done_callback_mock:
        file_task = cluster_common.ReceiveFileTask(
            guardbear_common=cluster_common.GuardBearCommon(), logger=logger_mock, task_id=b'010'
        )
        assert isinstance(file_task.guardbear_common, cluster_common.GuardBearCommon)
        ...
        string_task = cluster_common.ReceiveFileTask(
            guardbear_common=cluster_common.GuardBearCommon(), logger=logger_mock, task_id=''
        )
...
@patch('logging.Logger')
@patch('guardbear.core.cluster.common.GuardBearCommon')
def test_rft_set_up_coro(guardbear_common_mock, logger_mock):
    """Test if the exception is being properly raised when an Exception takes place."""
    with pytest.raises(NotImplementedError):
        cluster_common.ReceiveFileTask(guardbear_common_mock, logger_mock, b'task')
...
def test_rft_done_callback(set_up_coro_mock, create_task_mock, event_mock):
    ...
    class GuardBearCommon:
        def __init__(self):
            self.sync_tasks = {'010': b'123456789', '011': b'123456789'}

    create_task_mock.return_value = TaskMock()
    guardbear_common_mock = GuardBearCommon()

    with patch.object(TaskMock, 'add_done_callback'):
        with patch.object(logging.getLogger('guardbear'), 'error') as logger_mock:
            file_task = cluster_common.ReceiveFileTask(
                guardbear_common=guardbear_common_mock, logger=logging.getLogger('guardbear'), task_id=b'010'
            )
            file_task.done_callback()
            assert file_task.guardbear_common.sync_tasks == {'011': b'123456789'}
...
def test_handler_init():
    ...
    with patch('guardbear.core.cluster.utils.context_tag', ContextVar('', default='')) as cv:
        ...
        assert handler.logger == logging.getLogger('guardbear')
...
def test_handler_get_messages_ok():
    ...
    with patch('guardbear.core.cluster.common.Handler.msg_parse', return_value=True) as handler_mock:
        # Test if
...
@pytest.mark.asyncio
@patch('guardbear.core.cluster.common.Handler.push')
@patch('guardbear.core.cluster.common.Handler.next_counter', return_value=30)
@patch('guardbear.core.cluster.common.Handler.msg_build', return_value=['some', 'messages'])
async def test_handler_send_request_ok(msg_build_mock, next_counter_mock, push_mock):
    ...
    with patch('guardbear.core.cluster.common.Response.read', return_value='some value') as read_mock:
    ...
    with patch('guardbear.core.cluster.common.Response.read', side_effect=delay):
...
async def test_handler_send_request_ko():
    ...
    with patch('guardbear.core.cluster.common.Handler.msg_build', side_effect=MemoryError):
...
@patch('guardbear.core.cluster.common.Handler.send_request', return_value=b'some data')
async def test_handler_send_file_ok(send_request_mock, os_path_exists_mock):
    ...
    with patch('hashlib.sha256', return_value=MockHash()):
        assert await handler.send_file(common.GUARDBEAR_RUN / 'some_file.txt', task_id=b'abcd') == 3
        ...
        os_path_exists_mock.assert_called_once_with(common.GUARDBEAR_RUN / 'some_file.txt')
...
async def test_handler_send_string():
    ...
    with patch('guardbear.core.cluster.common.Handler.send_request', return_value=b'some data'):
        assert await handler.send_string(b'something') == b'some data'

    with patch('guardbear.core.cluster.common.Handler.send_request', side_effect=exception.GuardBearClusterError(3020)):
        with patch.object(logging.getLogger('guardbear'), 'error') as logger_mock:
...
async def test_handler_forward_dapi_response_ok():
    ...
    with patch('guardbear.core.cluster.common.Handler.get_manager', return_value=mock_manager):
...
async def test_handler_forward_dapi_response_ko():
    ...
    with patch('guardbear.core.cluster.common.Handler.get_manager', return_value=mock_manager):
        ...
        with patch.object(
            mock_manager.local_server, 'send_string', side_effect=exception.GuardBearException(1001)
        ) as send_string_mock:
            with patch.object(logging.getLogger('guardbear'), 'error') as logger_mock:
                with patch('guardbear.core.cluster.common.Handler.send_request', return_value='some value'):
                    with patch('json.dumps', return_value='some value'):
                        await handler.forward_dapi_response(b'client string_id')
                        assert handler.in_str == {b'other_string': 'some value'}
                        logger_mock.assert_called_once_with(
                            f'Error sending API response to local client: {exception.GuardBearException(1001)}'
                        )
...
def test_handler_data_received_ok():
    ...
    # Test first if
    with patch('guardbear.core.cluster.common.Handler.get_messages', return_value=[(b'bytes1', 123, b'bytes2', b'd')]):
        # Test try
    ...
    with patch('guardbear.core.cluster.common.Handler.get_messages', return_value=[(b'bytes1', 123, b'bytes2', b'bytes3')]):
        ...
        with patch('guardbear.core.cluster.common.Handler.dispatch') as dispatch_mock:
            handler.data_received(b'message')
            dispatch_mock.assert_called_once_with(b'bytes1', 123, b'bytes2')


@patch('guardbear.core.cluster.common.Handler.msg_build', return_value=['msg'])
@patch('guardbear.core.cluster.common.Handler.push')
@patch('guardbear.core.cluster.common.Handler.process_request', return_value=(b'command', b'payload'))
def test_handler_dispatch(process_request_mock, push_mock, msg_build_mock):
    ...
    process_request_mock.side_effect = exception.GuardBearException(1001)
    with patch.object(logging.getLogger('guardbear'), 'error') as logger_mock:
        handler.dispatch(None, 123, b'payload')
        logger_mock.assert_called_with(f"Internal error processing request '{None}': {exception.GuardBearException(1001)}")
...
def test_handler_process_request():
    ...
    with patch('guardbear.core.cluster.common.Handler.echo') as echo_mock:
        handler.process_request(b'echo', b'data')
        echo_mock.assert_called_once_with(b'data')
    with patch('guardbear.core.cluster.common.Handler.receive_file') as receive_file_mock:
        handler.process_request(b'new_file', b'data')
        receive_file_mock.assert_called_once_with(b'data')
    with patch('guardbear.core.cluster.common.Handler.receive_str') as receive_str_mock:
        handler.process_request(b'new_str', b'data')
        receive_str_mock.assert_called_once_with(b'data')
    with patch('guardbear.core.cluster.common.Handler.update_file') as update_file_mock:
        handler.process_request(b'file_upd', b'data')
        update_file_mock.assert_called_once_with(b'data')
    with patch('guardbear.core.cluster.common.Handler.str_upd') as str_upd_mock:
        handler.process_request(b'str_upd', b'data')
        str_upd_mock.assert_called_once_with(b'data')
    with patch('guardbear.core.cluster.common.Handler.process_error_str') as process_error_str_mock:
        handler.process_request(b'err_str', b'data')
        process_error_str_mock.assert_called_once_with(b'data')
    with patch('guardbear.core.cluster.common.Handler.end_file') as end_file_mock:
        handler.process_request(b'file_end', b'data')
        end_file_mock.assert_called_once_with(b'data')
    with patch('guardbear.core.cluster.common.Handler.cancel_task') as cancel_task_mock:
        handler.process_request(b'cancel_task', b'data')
        cancel_task_mock.assert_called_once_with(b'data')
    with patch('guardbear.core.cluster.common.Handler.process_dapi_error') as process_dapi_err_mock:
    ...
    with patch('guardbear.core.cluster.common.Handler.process_unknown_cmd') as process_unknown_cmd_mock:
...
def test_handler_process_response():
    ...
    with patch('guardbear.core.cluster.common.Handler.process_error_from_peer', return_value=b'payload'):
...
def test_handler_receive_file():
    ...
    with patch('guardbear.core.cluster.common.open') as open_mock:
...
def test_handler_str_upd():
    ...
    with patch('guardbear.core.cluster.common.InBuffer.receive_data'):
...
def test_handler_setup_task_logger():
    ...
    with patch.object(logging.getLogger('guardbear'), 'getChild', return_value=TaskLoggerMock()) as get_child_mock:
...
@pytest.mark.asyncio
@patch('guardbear.core.cluster.common.Handler.send_request')
async def test_handler_wait_for_file_ko(send_request_mock):
    """Check if expected exception is raised.
    Condition 1: when event.wait() exceeds the timeout, GuardBearClusterError 3039 is raised
    Condition 2: when any other exception occurs, GuardBearClusterError 3040  is raised.
    """
...
# Test 'GuardBearCommon' class methods


def test_guardbear_common_init():
    """Test the '__init__' method correct functioning."""
    guardbear_common_test = cluster_common.GuardBearCommon()
    assert guardbear_common_test.sync_tasks == {}


def test_guardbear_common_get_logger():
    """Check if a Logger object is properly returned."""
    with pytest.raises(NotImplementedError):
        guardbear_common.get_logger()


def test_guardbear_common_setup_receive_file():
    ...
    with patch('guardbear.core.cluster.common.GuardBearCommon.get_logger'):
        first_output, second_output = guardbear_common.setup_receive_file(mock_object)
        assert first_output == b'ok'
        assert isinstance(second_output, bytes)
        assert MyTaskMock().task_id in guardbear_common.sync_tasks
        assert isinstance(guardbear_common.sync_tasks[MyTaskMock().task_id], MyTaskMock)


@patch('guardbear.core.cluster.common.GuardBearCommon')
@patch('logging.Logger')
def test_guardbear_common_end_receiving_file_ok(logger_mock, guardbear_common_mock):
    """Check if the full path to the received file is properly stored and availability is notified."""
    with patch('guardbear.core.cluster.common.ReceiveFileTask.set_up_coro'):
        with patch('asyncio.create_task'):
            file_task = cluster_common.ReceiveFileTask(guardbear_common_mock, logger_mock, b'task')

    guardbear_common.sync_tasks = {'task_ID': file_task}
    assert guardbear_common.end_receiving_file('task_ID filepath') == (b'ok', b'File correctly received')
    assert isinstance(guardbear_common.sync_tasks['task_ID'], cluster_common.ReceiveFileTask)


@patch('os.remove')
@patch('os.path.exists', return_value=True)
def test_guardbear_common_end_receiving_file_ko(path_exists_mock, os_remove_mock):
    ...
    with pytest.raises(exception.GuardBearClusterError, match=r'.* 3027 .*'):
        guardbear_common.end_receiving_file('not_task_ID filepath')
...
@patch('json.loads')
def test_guardbear_common_error_receiving_file_ok(json_loads_mock):
    """Check how error are handled by peer in the sent file process."""
    with patch('os.path.exists', return_value=True):
        with patch('os.remove'):
            # Test first condition and its nested condition
            assert guardbear_common.error_receiving_file('task_ID error_details') == (b'ok', b'Error received')

    # Test second condition
    with patch('guardbear.core.cluster.common.GuardBearCommon.get_logger'):
        assert guardbear_common.error_receiving_file('not_task_ID error_details') == (b'ok', b'Error received')


def test_guardbear_common_error_receiving_file_ko():
...
def test_guardbear_common_get_node():
    """Check if it is possible to obtain basic information about the node."""

    class MockClass(cluster_common.GuardBearCommon, cluster_common.Handler, abc.ABC):
        def __init__(self):
            super().__init__()
    ...
    with patch('guardbear.core.cluster.common.Handler.get_manager', return_value=mock_manager) as manager_mock:
        mock_class.get_node()
...
def test_guardbear_json_encoder_default():
    """Test if a special JSON encoder is defined for GuardBear."""
    guardbear_encoder = cluster_common.GuardBearJSONEncoder()

    # Test first condition
    assert isinstance(guardbear_encoder.default(object), dict)
    assert guardbear_encoder.default(object) == {
        '__callable__': {'__name__': 'object', '__module__': 'builtins', '__qualname__': 'object', '__type__': 'type'}
    }

    class GuardBearMock(json.JSONEncoder):
        __self__ = GuardBear()
        a = 0
        b = 0
    ...
    guardbear_encoder.default(GuardBearMock)

    with patch('builtins.callable', return_value=False):
        # Test second condition
        assert isinstance(guardbear_encoder.default(exception.GuardBearException(3012)), dict)
        assert guardbear_encoder.default(exception.GuardBearException(3012)) == {
            '__guardbear_exception__': {
                '__class__': 'GuardBearException',
                '__object__': {
                    'type': 'about:blank',
                    'title': 'GuardBearException',
                    'code': 3012,
                    'extra_message': None,
        ...
        abstract_guardbear_result = wresults.AffectedItemsGuardBearResult()

        assert isinstance(guardbear_encoder.default(abstract_guardbear_result), dict)
        assert guardbear_encoder.default(abstract_guardbear_result) == {
            '__guardbear_result__': {
                '__class__': 'AffectedItemsGuardBearResult',
                '__object__': {
                    'affected_items': [],
                    'sort_fields': None,
                    'sort_casting': ['int'],
        ...
        assert isinstance(guardbear_encoder.default(date), dict)
        assert guardbear_encoder.default(date) == {'__guardbear_datetime__': '2021-10-15T00:00:00'}

        # Test fifth condition
        exc = ValueError('test')
        assert isinstance(guardbear_encoder.default(exc), dict)
        assert guardbear_encoder.default(exc) == {'__unhandled_exc__': {'__class__': 'ValueError', '__args__': ('test',)}}

        # Test simple return
        with pytest.raises(TypeError):
            guardbear_encoder.default({'key': 'value'})


def test_as_guardbear_object_ok():
    """Test the different outputs taking into account the input values."""
    # Test the first condition and nested if
    assert cluster_common.as_guardbear_object({'__callable__': {'__name__': 'type', '__guardbear__': 'version'}}) == 'server'

    # Test the first condition and nested else
    assert isinstance(
        cluster_common.as_guardbear_object(
    ...
    assert isinstance(
        cluster_common.as_guardbear_object(
            {'__guardbear_exception__': {'__class__': 'GuardBearException', '__object__': {'code': 1001}}}
        ),
        exception.GuardBearException,
    )

    # Test the third condition
    with patch('guardbear.core.results.AbstractGuardBearResult.decode_json', return_value=wresults.AbstractGuardBearResult):
        assert isinstance(
            cluster_common.as_guardbear_object(
                {'__guardbear_result__': {'__class__': 'AbstractGuardBearResult', '__object__': {'code': 1001}}}
            ),
            abc.ABCMeta,
        )

    # Test the fourth condition
    assert isinstance(cluster_common.as_guardbear_object({'__guardbear_datetime__': '2021-10-14'}), datetime)
    assert cluster_common.as_guardbear_object({'__guardbear_datetime__': '2021-10-14'}) == datetime(2021, 10, 14)

    # Test the fifth condition
    result = cluster_common.as_guardbear_object({'__unhandled_exc__': {'__class__': 'ValueError', '__args__': ('test',)}})
    assert result == {'ValueError': ['test']}

    result = cluster_common.as_guardbear_object({'__unhandled_exc__': {'__class__': 'exit', '__args__': []}})
    assert result == {'exit': []}

    # No condition fulfilled
    assert isinstance(cluster_common.as_guardbear_object({'__guardbear_datetime_bad__': '2021-10-14'}), dict)
    assert cluster_common.as_guardbear_object({'__guardbear_datetime_bad__': '2021-10-14'}) == {
        '__guardbear_datetime_bad__': '2021-10-14'
    }


def test_as_guardbear_object_ko():
    """Test if the exceptions are correctly raised."""
    with pytest.raises(exception.GuardBearInternalError, match=r'.* 1000 .*'):
        cluster_common.as_guardbear_object({'__callable__': {'__name__': 'value', '__guardbear__': 'value'}})


def get_handler():
    """Return a Handler object. This is an auxiliary method."""
    return cluster_common.Handler(server_config=default_config.server, logger=logging.getLogger('guardbear'))
...
def test_sync_task_init():
    ...
    sync_task = cluster_common.SyncTask(b'cmd', logging.getLogger('guardbear'), get_handler())

    assert sync_task.cmd == b'cmd'
    assert sync_task.logger == logging.getLogger('guardbear')
    assert isinstance(sync_task.server, cluster_common.Handler)


@pytest.mark.asyncio
@patch('guardbear.core.cluster.common.Handler.send_request', side_effect=Exception())
async def test_sync_task_request_permission(send_request_mock):
    ...
    with patch.object(logging.getLogger('guardbear'), 'debug') as logger_mock:
        # Test second condition
...
@patch('guardbear.core.cluster.cluster.compress_files', return_value=('files/path/', {}))
@patch('guardbear.core.cluster.utils.log_subprocess_execution')
async def test_sync_files_sync_ok(log_subprocess_mock, compress_files_mock, unlink_mock, relpath_mock, json_dumps_mock):
    ...
    sync_files = cluster_common.SyncFiles(b'cmd', logging.getLogger('guardbear'), worker_mock)
    ...
    with patch.object(logging.getLogger('guardbear'), 'error') as logger_mock:
        ...
        json_dumps_mock.assert_called_once_with(
            exception.GuardBearClusterError(code=3020, extra_message='cmd'), cls=cluster_common.GuardBearJSONEncoder
        )
    ...
    with patch.object(WorkerMock, 'send_file', return_value=1000) as send_file_mock:
        ...
        with patch.object(logging.getLogger('guardbear'), 'debug') as logger_debug_mock:
            with patch.object(logging.getLogger('guardbear'), 'error') as logger_error_mock:
                await sync_files.sync(files_to_sync, files_metadata, 1, task_pool=None)
                ...
                relpath_mock.assert_called_once_with('files/path/', common.GUARDBEAR_RUN)
                ...
                logger_error_mock.assert_called_once_with(
                    f'Error sending zip file: {exception.GuardBearException(3016, "cmd_e")}'
                )
...
@pytest.mark.asyncio
@patch('guardbear.core.cluster.cluster.open')
@patch('guardbear.core.cluster.cluster.mkdir_with_mode')
@patch('guardbear.core.cluster.common.Handler.send_request', side_effect=Exception())
async def test_sync_files_sync_ko(send_request_mock, mkdir_with_mode_mock, open_mock):
    ...
    sync_files = cluster_common.SyncFiles(b'cmd', logging.getLogger('guardbear'), handler)
    ...
    with patch('os.unlink', side_effect=FileNotFoundError):
        ...
        with patch('guardbear.core.cluster.cluster.compress_files', return_value=(compressed_data, {})):
            send_request_mock.side_effect = None
...
def test_create_ssl_context(logger_mock, purpose, keyfile_password):
    ...
    ssl_mock = SSLMock()
    cafile = '/var/guardbear_server/etc/sslmanager.ca'
    certfile = '/var/guardbear_server/etc/sslmanager.cert'
    keyfile = '/var/guardbear_server/etc/sslmanager.key'

...
with patch('guardbear.common.getgrnam'):
    with patch('guardbear.common.getpwnam'):
        with patch('guardbear.common.guardbear_uid'):
            with patch('guardbear.common.guardbear_gid'):
                with patch.object(ValidateFilePathMixin, '_validate_file_path', return_value=None):
                    ...
                    from guardbear.core.cluster import control
...
async def test_get_nodes():
    ...
    with patch('guardbear.core.cluster.local_client.LocalClient.execute', side_effect=async_local_client):
    ...
    with patch('guardbear.core.cluster.local_client.LocalClient.execute', side_effect=[GuardBearClusterError(3020), 'error']):
        with pytest.raises(GuardBearClusterError):
            await control.get_nodes(lc=local_client)
...
async def test_get_system_nodes():
    ...
    with patch('guardbear.core.cluster.local_client.LocalClient.execute', side_effect=async_local_client):
        ...
        for expected in expected_result:
            with patch('guardbear.core.cluster.control.get_nodes', return_value=expected):

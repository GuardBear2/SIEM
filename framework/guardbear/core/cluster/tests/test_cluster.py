...
with patch('guardbear.common.guardbear_uid'):
    with patch('guardbear.common.guardbear_gid'):
        with patch.object(ValidateFilePathMixin, '_validate_file_path', return_value=None):
            ...
            import guardbear.core.cluster.cluster as cluster
...
@patch('guardbear.core.cluster.cluster.blake2b', return_value='hash')
@patch('guardbear.core.cluster.cluster.path.join', return_value='/mock/foo/bar')
@patch('guardbear.core.cluster.cluster.walk', return_value=[('foo/bar', (), ('spam', 'eggs', '.merged'))])
def test_walk_dir(walk_mock, path_join_mock, blake2b_mock, getmtime_mock):
    ...
    path_join_mock.assert_called_once_with(common.GUARDBEAR_ETC, '/foo/bar')
    ...
    path_join_mock.assert_has_calls(
        [
            call(common.GUARDBEAR_ETC, 'foo/bar'),
...
@patch('guardbear.core.cluster.cluster.walk', return_value=[('/foo/bar', (), ['spam'])])
...
def test_walk_dir_ko(mock_path_join, mock_walk):
    ...
    with patch('guardbear.core.cluster.cluster.walk', side_effect=OSError):
...
def test_get_files_status():
    ...
    with patch('guardbear.core.cluster.cluster.walk_dir', return_value=(test_dict, {})):
...
@patch('guardbear.core.cluster.cluster.path.dirname', return_value='/some/path')
@patch('guardbear.core.cluster.cluster.path.exists', return_value=False)
def test_compress_files_ok(mock_path_exists, mock_path_dirname, mock_mkdir_with_mode, mock_zlib):
    ...
    with patch('builtins.open', mock_open(read_data='test_content')) as open_mock:
        ...
        assert open_mock.call_args_list == [
            call(ANY, 'ab'),
            call(os.path.join(common.GUARDBEAR_ETC, 'some/path'), 'rb'),
            call(os.path.join(common.GUARDBEAR_ETC, 'another/path'), 'rb'),
        ]
...
def test_compress_files_ko(mock_path_exists, mock_path_dirname, mock_mkdir_with_mode):
    ...
    with patch('builtins.open', mock_open(read_data='test_content')):
        ...
        assert logs['warning']['some/path'] == [
            f'File too large to be synced: {os.path.join(common.GUARDBEAR_ETC, "some/path")}'
        ]

        CentralizedConfig._config.server.communications.zip.max_size = 15
        CentralizedConfig._config.server.communications.zip.compress_level = 0
        with patch('zlib.compress', side_effect=zlib.error):
            with pytest.raises(GuardBearError, match=r'.* 3001 .*'):
...
@pytest.mark.asyncio
@patch('guardbear.core.cluster.cluster.decompress_files', return_value='OK')
...
@patch('guardbear.core.cluster.cluster.remove')
@patch('guardbear.core.cluster.cluster.mkdir_with_mode')
        ...
        with patch('builtins.open', mock_open(read_data=f'path{cluster.PATH_SEP}content'.encode())):
            with patch('os.path.exists', return_value=False):
                with patch('os.makedirs', side_effect=PermissionError) as mock_makedirs:
                    with patch('guardbear.core.cluster.cluster.remove'):
...
def test_compare_files():
    ...
    # First condition
    with patch('guardbear.core.cluster.cluster.merge_info', return_values=[1, 'random/path/']):
...
@patch.object(guardbear.core.cluster.cluster.logger, 'error')
...
def test_clean_up_ok():
    ...
    with patch('os.path.join', return_value='some/path/'):
        with patch.object(guardbear.core.cluster.cluster.logger, 'debug') as mock_logger:
            with patch('os.path.exists', return_value=False) as path_exists_mock:
                ...
                with patch(
                    'guardbear.core.cluster.cluster.listdir', return_value=[common.LOCAL_SERVER_SOCKET, 'other_file.txt']
                ):
...
def test_clean_up_ko():
    ...
    with patch('os.path.join') as path_join_mock:
        with patch.object(guardbear.core.cluster.cluster.logger, 'error') as mock_error_logger:
            with patch.object(guardbear.core.cluster.cluster.logger, 'debug') as mock_debug_logger:
                ...
                with patch('os.path.exists', return_value=True):
                    with patch(
                        'guardbear.core.cluster.cluster.listdir',
...
@patch('guardbear.core.cluster.cluster.listdir', return_value=['005', '006'])
@patch('guardbear.core.cluster.cluster.stat')
def test_merge_info(stat_mock, listdir_mock):
    ...
    with patch('builtins.open', mock_open(read_data=agent_groups)) as open_mock:
        ...
        open_mock.assert_any_call(str(common.GUARDBEAR_RUN / 'cluster/worker1/testing-shared.merged'), 'wb')
        open_mock.assert_any_call(str(common.GUARDBEAR_RUN / 'cluster/testing/005'), 'rb')
        open_mock.assert_any_call(str(common.GUARDBEAR_RUN / 'cluster/testing/006'), 'rb')
...
def test_unmerge_info():
    ...
    with patch('builtins.open', mock_open(read_data=agent_info)):
        with patch('guardbear.core.cluster.cluster.stat') as stat_mock:
            ...
            with patch.object(guardbear.core.cluster.cluster.logger, 'warning') as mock_logger:
...
async def test_run_in_pool(event_loop):
    ...
    with patch('guardbear.core.cluster.cluster.wait_for', return_value='OK') as wait_for_mock:
        assert (
...
def test_validate_haproxy_helper_config_ko():
    """Verify that validate_haproxy_helper_config raises GuardBearError when there is a validation error."""
    config = {cluster.AGENT_CHUNK_SIZE: 120, cluster.AGENT_RECONNECTION_TIME: 10}

    with patch.object(cluster, 'validate', side_effect=cluster.ValidationError(message='Error test', path=['test'])):
        with pytest.raises(cluster.GuardBearError, match='.* 3004 .*'):
            cluster.validate_haproxy_helper_config(config)

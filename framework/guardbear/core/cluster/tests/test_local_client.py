...
with patch('guardbear.common.guardbear_uid'):
    with patch('guardbear.common.guardbear_gid'):
        with patch.object(ValidateFilePathMixin, '_validate_file_path', return_value=None):
            ...
            from guardbear.core.cluster.common import InBuffer
            from guardbear.core.cluster.local_client import *
            from guardbear.core.exception import GuardBearInternalError
...
@patch('guardbear.core.cluster.client.asyncio.get_running_loop')
...
@pytest.mark.asyncio
@patch('guardbear.core.config.client.CentralizedConfig.get_server_config')
async def test_localclient_start(mock_get_server_config):
    ...
    with patch('uvloop.Loop.create_unix_connection', side_effect=create_unix_connection) as mock_create_unix_connection:
        ...
        with patch('guardbear.core.cluster.client.asyncio.get_running_loop', return_value=mocked_loop):
            lc = LocalClient()
            await lc.start()
...
async def test_localclient_execute():
    ...
    with patch('guardbear.core.cluster.local_client.LocalClient.start'):
        with patch('guardbear.core.cluster.local_client.LocalClient.send_api_request', return_value='Test'):
...
async def test_localclient_send_file():
    ...
    """
    with patch('guardbear.core.cluster.local_client.LocalClient.start'):
        with patch('guardbear.core.cluster.local_client.LocalClient.send_api_request', return_value=b'guardbear/test python'):
            lc = LocalClient()
            assert await lc.send_file(path='guardbear/test', node_name='python') == b'guardbear/test python'

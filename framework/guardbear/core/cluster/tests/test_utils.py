...
with patch('guardbear.core.common.getgrnam'):
    with patch('guardbear.core.common.getpwnam'):
        with patch('guardbear.core.common.guardbear_uid'):
            with patch('guardbear.core.common.guardbear_gid'):
                with patch.object(ValidateFilePathMixin, '_validate_file_path', return_value=None):
                    ...
                    from guardbear.core.cluster import utils
                    from guardbear.core.exception import (
                        GuardBearError,
                        GuardBearException,
                        GuardBearHAPHelperError,
                        GuardBearInternalError,
                        GuardBearPermissionError,
                        GuardBearResourceNotFound,
                    )
...
default_cluster_config = {
    ...
    'nodes': ['127.0.0.1'],
    'hidden': 'no',
    'cafile': common.GUARDBEAR_ETC / 'server.ca',
    'certfile': common.GUARDBEAR_ETC / 'server.crt',
    'keyfile': common.GUARDBEAR_ETC / 'server.key',
    'keyfile_password': '',
}
...
def test_parse_haproxy_helper_config_ko(config: dict, exception_type: GuardBearException, expected_error_code: str):
...
def test_get_manager_status():
    ...
    with patch('guardbear.core.cluster.utils.glob', return_value=['ossec-0.pid']):
        with patch('re.match', return_value='None'):
            status = utils.get_manager_status()
            for value in status.values():
                assert value == 'failed'

        # with patch('guardbear.core.cluster.utils.join', return_value='failed') as join_mock:
        with patch('guardbear.core.cluster.utils.os.path.exists', side_effect=exist_mock):
            status = utils.get_manager_status()
            for value in status.values():
                assert value == 'failed'
...
def test_get_cluster_status():
    ...
    with patch('guardbear.core.cluster.utils.get_manager_status', side_effect=GuardBearInternalError(1913)):
        status = utils.get_cluster_status()
        assert {'running': 'no'} == status
...
def test_manager_restart():
    ...
    with patch('guardbear.core.cluster.utils.open', side_effect=None):
            ...
            with patch('os.path.exists', return_value=True):
                ...
                with patch('socket.socket.connect', side_effect=None):
                    ...
                    with patch('socket.socket.send', side_effect=None):
                        ...
                        assert GuardBearResult({'message': 'Restart request sent'}) == status
...
def test_ClusterLogger():
    """Verify that ClusterLogger defines the logger used by guardbear-server."""
...
@patch('guardbear.core.cluster.utils.pyDaemonModule.create_pid')
def test_process_spawn_sleep(pyDaemon_create_pid_mock, get_pid_mock):
    ...
    pyDaemon_create_pid_mock.assert_called_once_with(f'guardbear-server_child_{child}', get_pid_mock.return_value)


@pytest.mark.asyncio
@patch('concurrent.futures.ThreadPoolExecutor')
@patch('guardbear.core.cluster.dapi.dapi.DistributedAPI')

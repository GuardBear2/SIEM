...
from guardbear.core import cluster as metadata
from guardbear.core import common, exception, utils
from guardbear.core.cluster import client, cluster
from guardbear.core.cluster import common as c_common
from guardbear.core.cluster.dapi import dapi
from guardbear.core.cluster.utils import log_subprocess_execution
from guardbear.core.config.models.server import ServerConfig
from guardbear.core.utils import get_utc_now, safe_move
...
class ReceiveIntegrityTask(c_common.ReceiveFileTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.guardbear_common.process_files_from_master
    ...
    def done_callback(self, future=None):
        ...
        """
        self.guardbear_common.check_integrity_free = True
        super().done_callback(future)


class WorkerHandler(client.AbstractClient, c_common.GuardBearCommon):
    ...
    def connection_result(self, future_result):
        ...
        if self.connected:
            ...
            worker_tmp_files = common.GUARDBEAR_QUEUE / self.name
    ...
    def get_manager(self):
        """Get the Worker object that created this WorkerHandler. Used in the class GuardBearCommon.
    ...
    def error_receiving_integrity(self, data: str) -> Tuple[bytes, bytes]:
        ...
        ----------
        data : str
            Task ID and error formatted as GuardBearJSONEncoder.
        ...
        while True:
            ...
            # If exception is raised during sync process, notify the master so it removes the file if received.
                ...
                if isinstance(e, exception.GuardBearException):
                    exc = json.dumps(e, cls=c_common.GuardBearJSONEncoder)
                else:
                    exc = json.dumps(
                        exception.GuardBearClusterError(1000, extra_message=str(e)), cls=c_common.GuardBearJSONEncoder
                    )
    ...
    async def sync_extra_valid(self, extra_valid: Dict):
        ...
        # If exception is raised during sync process, notify the master, so it removes the file if received.
            ...
            if isinstance(e, exception.GuardBearException):
                ...
                exc = json.dumps(exception.GuardBearClusterError(1000, extra_message=str(e)), cls=c_common.GuardBearJSONEncoder)
            with contextlib.suppress(Exception):
                await self.send_request(command=b'syn_i_w_m_r', data=f'None {exc}'.encode())
    ...
    def update_master_files_in_worker(ko_files: Dict, zip_path: str, server_config: ServerConfig):
        ...
        def overwrite_or_create_files(filename_: str, data_: Dict):
            ...
            """
            full_filename_path = common.GUARDBEAR_ETC / filename
            ...
            if data_['merged']:  # worker nodes can only receive agent-groups files
                ...
                # and then move each one to the destination directory (<guardbear_path>/filename).
                # The TYPE string used in the 'unmerge_info' function is a placeholder. It corresponds to the
                # directory inside '{guardbear_path}/queue/' path.
                for name, content, _ in cluster.unmerge_info('TYPE', zip_path, filename_):
                    full_unmerged_name = common.GUARDBEAR_ETC / name
                    ...
                    safe_move(
                        ...
                        ownership=(common.guardbear_uid(), common.guardbear_gid()),
                    )
            else:
                # Create destination dir if it doesn't exist.
                if not os.path.exists(os.path.dirname(full_filename_path)):
                    utils.mkdir_with_mode(os.path.dirname(full_filename_path))
                # Move the file from zipdir (directory containing unzipped files) to <guardbear_path>/filename.
                safe_move(
            ...
            # Remove local files marked as extra.
                ...
                for file_to_remove in files:
                    try:
                        ...
                        file_path = common.GUARDBEAR_ETC / file_to_remove
                        try:
                            os.remove(file_path)
                        except OSError as e:
        ...
        for directory in directories_to_check:
            try:
                full_path = common.GUARDBEAR_ETC / directory

...
from guardbear.core import common, pyDaemonModule
from guardbear.core.config.client import CentralizedConfig
from guardbear.core.exception import GuardBearError, GuardBearHAPHelperError, GuardBearInternalError
from guardbear.core.results import GuardBearResult
from guardbear.core.utils import temporary_cache
from guardbear.core.guardbear_socket import create_guardbear_socket_message
...
execq_lockfile = common.GUARDBEAR_RUN / '.api_execq_lock'

# TODO(25554) - Delete HAPROXY Config
HELPER_DEFAULTS = {
    HAPROXY_PORT: 5555,
    HAPROXY_PROTOCOL: 'http',
    HAPROXY_BACKEND: 'guardbear_reporting',
    ...
    for field in [
        ...
        if helper_config.get(field):
            try:
                ...
                raise GuardBearError(3004, extra_message=f'HAProxy Helper {field} must be an integer.')
    ...
    for field in [IMBALANCE_TOLERANCE]:
        if helper_config.get(field):
            try:
                ...
                raise GuardBearError(3004, extra_message=f'HAProxy Helper {field} must be a float.')
    return helper_config
...
def parse_haproxy_helper_config(helper_config: dict) -> dict:
    ...
    if helper_config[HAPROXY_PROTOCOL].lower() == 'https' and type(helper_config[HAPROXY_CERT]) is bool:
        raise GuardBearHAPHelperError(3042, extra_message='HAProxy certificate file required in the haproxy_cert parameter')
...
def get_manager_status(cache=False) -> typing.Dict:
    ...
    processes = ['guardbear-server', 'guardbear-engined', 'guardbear-server-management-apid', 'guardbear-comms-apid']

    data, pidfile_regex, run_dir = {}, re.compile(r'.+\-(\d+)\.pid$'), common.GUARDBEAR_RUN
...
def get_cluster_status() -> typing.Dict:
    ...
    """
    try:
        cluster_status = {'running': 'yes' if get_manager_status()['guardbear-server'] == 'running' else 'no'}
...
def manager_restart() -> GuardBearResult:
    """Restart GuardBear manager.

    Send JSON message with the 'restart-guardbear' command to common.EXECQ_SOCKET socket.
    ...
    try:
        ...
        # json msg for restarting GuardBear manager
        msg = json.dumps(
            create_guardbear_socket_message(
                origin={'module': common.origin_module.get()},
                command=common.RESTART_GUARDBEAR_COMMAND,
    ...
    return GuardBearResult({'message': 'Restart request sent'})
...
class ClusterLogger(GuardBearLogger):
    """Define the logger used by guardbear-clusterd."""
    ...
    def setup_logger(self):
        ...
        * Sets log level based on the "debug_level" parameter received from guardbear-clusterd binary.
        """
        super().setup_logger()
...
def process_spawn_sleep(child):
    ...
    pyDaemonModule.create_pid(f'guardbear-server_child_{child}', pid)
...
async def forward_function(
    ...
    Returns
    -------
    Return either a dict or `GuardBearResult` instance in case the execution did not fail. Return an exception otherwise.
    """
    import concurrent
...
def print_version():
    from guardbear.core.cluster import __author__, __licence__, __version__, __guardbear_name__

    print('\n{} {} - {}\n\n{}'.format(__guardbear_name__, __version__, __author__, __licence__))

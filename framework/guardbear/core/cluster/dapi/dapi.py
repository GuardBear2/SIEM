...
import guardbear.core.cluster.cluster
import guardbear.core.cluster.utils
import guardbear.core.manager
import guardbear.core.results as wresults
from sqlalchemy.exc import OperationalError
from guardbear import agent
from guardbear.cluster import get_node_wrapper, get_nodes_info
from guardbear.core import common, exception
from guardbear.core.cluster import common as c_common
from guardbear.core.cluster import local_client
from guardbear.core.config.client import CentralizedConfig
from guardbear.core.exception import GuardBearClusterError, GuardBearError, GuardBearException
...
class DistributedAPI:
    ...
    def __init__(
        ...
        self.debug = debug
        self.node_info = guardbear.core.cluster.cluster.get_node() if node is None else node.get_node()
        ...
        if not basic_services:
            self.basic_services = ('guardbear-server',)
        else:
    ...
    def debug_log(self, message):
        ...
        message : str
            Full log message.
        """
        if self.logger.name == 'guardbear-api':
            self.logger.debug2(message)
        else:
            self.logger.debug(message)

    async def distribute_function(self) -> [Dict, exception.GuardBearException]:
        """Distribute an API call.

        Returns
        -------
        dict or GuardBearException
            Dictionary with API response or GuardBearException in case of error.
        """
        try:
            ...
            try:
                response = (
                    json.loads(response, object_hook=c_common.as_guardbear_object)
            ...
            return (
                response
                if isinstance(response, (wresults.AbstractGuardBearResult, exception.GuardBearException))
                else wresults.GuardBearResult(response)
            )
            ...
            return e
        except exception.GuardBearError as e:
            e.dapi_errors = await self.get_error_info(e)
            return e
    ...
    def check_guardbear_status(self):
        """There are some services that are required for guardbear to correctly process API requests. If any of those services
        is not running, the API must raise an exception indicating that:
            ...
            * There's an error in any of those services that must be addressed before using the API if any service is
              in failed status.
            * GuardBear must be started before using the API is the services are stopped.

        The basic service guardbear needs to be running is: guardbear-clusterd.
        """
        if self.f == guardbear.core.manager.status:
            return

        status = guardbear.core.manager.status()
    ...
    async def execute_local_request(self) -> str:
        ...
        try:
            ...
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.GuardBearJSONEncoder)
        except (exception.GuardBearError, exception.GuardBearResourceNotFound) as e:
            e.dapi_errors = await self.get_error_info(e)
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.GuardBearJSONEncoder)
            ...
            return json.dumps(
                ...
                cls=c_common.GuardBearJSONEncoder,
            )
    ...
    async def get_error_info(self, e) -> Dict:
        ...
        except exception.GuardBearException as rbac_exception:
        ...
        error_message = e.message if isinstance(e, exception.GuardBearException) else exception.GENERIC_ERROR_MSG
        ...
        if isinstance(e, exception.GuardBearInternalError):
            ...
            for h in self.logger.handlers or self.logger.parent.handlers:
                if hasattr(h, 'baseFilename'):
                    log_filename = os.path.join('GUARDBEAR_LOG', os.path.relpath(h.baseFilename, start=common.GUARDBEAR_LOG))
    ...
    async def send_tmp_file(self, node_name=None):
        ...
        json.loads(
            await client.send_file(os.path.join(common.GUARDBEAR_PATH, self.f_kwargs['tmp_file']), node_name),
            object_hook=c_common.as_guardbear_object,
        )
        os.remove(os.path.join(common.GUARDBEAR_PATH, self.f_kwargs['tmp_file']))
    ...
    async def execute_remote_request(self) -> Dict:
        ...
        node_response = await client.execute(
            command=b'dapi', data=json.dumps(self.to_dict(), cls=c_common.GuardBearJSONEncoder).encode()
        )
        return json.loads(node_response, object_hook=c_common.as_guardbear_object)

    async def forward_request(self) -> [wresults.AbstractGuardBearResult, exception.GuardBearException]:
        ...
        Returns
        -------
        wresults.AbstractGuardBearResult or exception.GuardBearException
        """

        async def forward(node_name: Tuple) -> [wresults.AbstractGuardBearResult, exception.GuardBearException]:
                ...
                try:
                    ...
                    result = json.loads(
                        await client.execute(
                            b'dapi_fwd',
                            '{} {}'.format(node_name, json.dumps(kcopy, cls=c_common.GuardBearJSONEncoder)).encode(),
                        ),
                        object_hook=c_common.as_guardbear_object,
                    )
                    ...
                    if e.code == 3022:
                        ...
                        raise e
                # Convert a non existing node into a GuardBearError exception
                if isinstance(result, GuardBearClusterError) and result.code == 3022:
                    common.rbac.set(self.rbac_permissions)
                    try:
                    ...
                    dikt['ids'] = {node_name}
                    result = GuardBearError.from_dict(dikt)

            return (
                result
                if isinstance(result, (wresults.AbstractGuardBearResult, exception.GuardBearException))
                else wresults.GuardBearResult(result)
            )
        ...
        if 'node_id' in self.f_kwargs or 'node_list' in self.f_kwargs:
            ...
            allowed_nodes = wresults.AffectedItemsGuardBearResult()
        ...
        if allowed_nodes.total_affected_items > 1:
            ...
            if isinstance(response, wresults.AbstractGuardBearResult):
        ...
        # It might be a GuardBearError after reducing
        if isinstance(response, wresults.AffectedItemsGuardBearResult):
...
class GuardBearRequestQueue:
    """Represents a queue of GuardBear requests."""
...
class APIRequestQueue(GuardBearRequestQueue):
    ...
    def __init__(self, server):
        ...
        self.logger = logging.getLogger('guardbear').getChild('dapi')
        self.logger.addFilter(guardbear.core.cluster.utils.ClusterFilter(tag='Cluster', subtag='D API'))
    ...
    async def run(self):
        while True:
            ...
            try:
                request = json.loads(request, object_hook=c_common.as_guardbear_object)
                self.logger.info(
                ...
                task_id = await node.send_string(json.dumps(result, cls=c_common.GuardBearJSONEncoder).encode())
            except Exception as e:

...
with patch('guardbear.common.guardbear_uid'):
    with patch('guardbear.common.guardbear_gid'):
        with patch.object(ValidateFilePathMixin, '_validate_file_path', return_value=None):
            ...
            from guardbear import GuardBearError, GuardBearInternalError, agent, cluster, manager
            from guardbear.core.cluster import local_client
            from guardbear.core.cluster.dapi.dapi import APIRequestQueue, DistributedAPI
            from guardbear.core.exception import GuardBearClusterError
            from guardbear.core.manager import get_manager_status
    ...
    [
        {
            ...
            'nodes': ['worker1'],
            'basic_services': ('guardbear-clusterd',),
...
def test_DistributedAPI_debug_log():
    ...
    logger_ = TestingLogger(logger_name='guardbear-api')
    ...
    logger_ = TestingLogger(logger_name='guardbear')
...
@patch(
    'guardbear.core.cluster.dapi.dapi.DistributedAPI.execute_local_request',
    new=AsyncMock(return_value=GuardBearResult({'result': 'local'})),
)
@patch(
    'guardbear.core.cluster.dapi.dapi.DistributedAPI.forward_request',
    new=AsyncMock(return_value=GuardBearResult({'result': 'forward'})),
)
@patch(
    'guardbear.core.cluster.dapi.dapi.DistributedAPI.execute_remote_request',
    new=AsyncMock(return_value=GuardBearResult({'result': 'remote'})),
)
@pytest.mark.parametrize(
...
async def test_DistributedAPI_distribute_function(api_request, request_type, node, expected, f_kwargs):
    ...
    expected : str
        Expected result.
    """
    with patch('guardbear.core.cluster.cluster.get_node', return_value={'type': node}):
...
@patch(
    'guardbear.core.cluster.dapi.dapi.DistributedAPI.get_solver_node',
    new=AsyncMock(return_value=GuardBearResult({'unknown': ['001', '002']})),
)
@pytest.mark.parametrize(
...
async def test_DistributedAPI_distribute_function_mock_solver(api_request, request_type, node, expected):
    ...
    expected : str
        Expected result
    """
    with patch('guardbear.core.cluster.cluster.get_node', return_value={'type': node, 'node': 'master'}):
...
async def test_DistributedAPI_distribute_function_exception():
    ...
    logger_ = logging.getLogger('guardbear')
    with patch('guardbear.core.cluster.dapi.dapi.get_node_wrapper', side_effect=GuardBearError(4000)):
        dapi = DistributedAPI(f=agent.get_agents, logger=logger_)
        get_error_result = await dapi.get_error_info(Exception('testing'))
        assert 'unknown-node' in get_error_result
        assert get_error_result['unknown-node']['error'] == 'GuardBear Internal Error. See log for more detail'

    with patch('guardbear.core.cluster.dapi.dapi.get_node_wrapper', side_effect=GuardBearError(4001)):
        dapi = DistributedAPI(f=agent.get_agents, logger=logger_)
        with pytest.raises(GuardBearError, match='.* 4001 .*'):
            await dapi.get_error_info(Exception('testing'))

    with patch('guardbear.core.cluster.dapi.dapi.get_node_wrapper', new=AsyncMock(return_value=NodeWrapper())):
        dapi = DistributedAPI(f=agent.get_agents, logger=logger_)
...
@patch(
    'guardbear.core.cluster.dapi.dapi.DistributedAPI.execute_local_request', new=AsyncMock(return_value="{wrong': json}")
)
...
async def test_DistributedAPI_local_request_errors():
    ...
    with patch('guardbear.cluster.get_nodes_info', side_effect=json.decoder.JSONDecodeError('test', 'test', 1)):
        with patch('guardbear.core.cluster.dapi.dapi.DistributedAPI.check_guardbear_status'):
...
@patch('guardbear.core.cluster.dapi.dapi.DistributedAPI.check_guardbear_status', side_effect=None)
...
async def test_DistributedAPI_local_request(mock_local_request):
    ...
    with patch('asyncio.wait_for', new=AsyncMock(side_effect=GuardBearError(1001))):
        dapi_kwargs = {'f': manager.status, 'logger': logger}
        await raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=1001)
    ...
    exc_code = 3000
    cluster_exc = GuardBearClusterError(exc_code)
    with patch('asyncio.wait_for', new=AsyncMock(side_effect=cluster_exc)):
        with patch.object(TestingLogger, 'error') as logger_error_mock:
            # Test GuardBearClusterError caught in execute_local_request and ProblemException raised
            ...
            # Test GuardBearClusterError is raised when using debug in execute_local_request and distribute_function
            dapi = DistributedAPI(f=manager.status, logger=testing_logger, debug=True)
            try:
                raise_if_exc(await dapi.distribute_function())
            except GuardBearClusterError as e:
...
@patch('guardbear.core.cluster.cluster.get_node', return_value={'type': 'worker'})
@patch('guardbear.core.cluster.local_client.LocalClient.execute', return_value='invalid_json')
...
@patch('guardbear.core.cluster.local_client.LocalClient.execute', new=AsyncMock(return_value='{"Testing": 1}'))
...
@patch('guardbear.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'master-node'})
@patch('guardbear.core.cluster.dapi.dapi.DistributedAPI.get_solver_node', return_value={'worker1': ['001', '002']})
@patch('guardbear.core.cluster.local_client.LocalClient.execute', return_value='invalid_json')
...
@patch('guardbear.core.cluster.local_client.LocalClient.send_file', new=AsyncMock(return_value='{"Testing": 1}'))
@patch('guardbear.core.cluster.local_client.LocalClient.execute', new=AsyncMock(return_value='{"Testing": 1}'))
@patch(
    'guardbear.core.cluster.dapi.dapi.DistributedAPI.get_solver_node',
    new=AsyncMock(return_value=GuardBearResult({'testing': ['001', '002']})),
)
async def test_DistributedAPI_tmp_file():
    ...
    with patch('guardbear.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'unknown'}):
        with patch(
            'guardbear.core.cluster.dapi.dapi.get_node_wrapper',
            return_value=AffectedItemsGuardBearResult(affected_items=[{'type': 'master', 'node': 'unknown'}]),
        ):
            dapi_kwargs = {
                'f': manager.status,
                'logger': logger,
    ...
    open('/tmp/dapi_file.txt', 'a').close()
    with patch('guardbear.core.cluster.cluster.get_node', return_value={'type': 'unk', 'node': 'master'}):
        await raise_if_exc_routine(dapi_kwargs=dapi_kwargs)
...
async def test_DistributedAPI_tmp_file_cluster_error():
    ...
    with patch('guardbear.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'unknown'}):
        with patch(
            ...
            with patch(
                'guardbear.core.cluster.local_client.LocalClient.execute',
                new=AsyncMock(side_effect=GuardBearClusterError(3022)),
            ):
                dapi_kwargs = {
                    'f': manager.status,
                    'logger': logger,
            ...
            with patch(
                'guardbear.core.cluster.local_client.LocalClient.execute',
                new=AsyncMock(side_effect=GuardBearClusterError(1000)),
            ):
                dapi_kwargs = {
                    'f': manager.status,
                    'logger': logger,
...
@patch(
    'guardbear.agent.Agent.get_agents_overview',
    return_value={
        'items': [
...
async def test_DistributedAPI_get_solver_node(mock_agents_overview):
    ...
    nodes_info_result = AffectedItemsGuardBearResult()
    nodes_info_result.affected_items.append({'name': 'master'})
    common.cluster_nodes.set(['master'])

    with patch('guardbear.core.cluster.dapi.dapi.get_nodes_info', new=AsyncMock(return_value=nodes_info_result)):
        with patch('guardbear.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'unknown'}):
            dapi_kwargs = {
                'f': manager.status,
                'logger': logger,
            ...
            expected = AffectedItemsGuardBearResult()
            expected.affected_items = [{'id': '001', 'node_name': 'master'}]
            with patch('guardbear.agent.get_agents_in_group', return_value=expected):
                dapi_kwargs = {
                    'f': manager.status,
                    'logger': logger,
                    'request_type': 'distributed_master',
...
@pytest.mark.parametrize('api_request', [agent.get_agents, guardbear.core.manager.status])
@patch('guardbear.core.manager.get_manager_status', return_value={process: 'running' for process in get_manager_status()})
def test_DistributedAPI_check_guardbear_status(status_mock, api_request):
    """Test `check_guardbear_status` method from class DistributedAPI."""
    dapi = DistributedAPI(f=api_request, logger=logger)
    data = dapi.check_guardbear_status()
    assert data is None


@pytest.mark.parametrize('status_value', ['failed', 'restarting', 'stopped'])
@patch('guardbear.core.cluster.cluster.get_node', return_value={'node': 'random_node'})
def test_DistributedAPI_check_guardbear_status_exception(node_info_mock, status_value):
    """Test exceptions from `check_guardbear_status` method from class DistributedAPI."""
    statuses = {process: status_value for process in sorted(get_manager_status())}
    with patch('guardbear.core.manager.get_manager_status', return_value=statuses):
        dapi = DistributedAPI(f=agent.get_agents, logger=logger)
        try:
            dapi.check_guardbear_status()
        except GuardBearInternalError as e:
            assert e.code == 1017
            assert statuses
...
@patch('guardbear.core.cluster.common.import_module', return_value='os.path')
...
async def test_APIRequestQueue_run(loop_mock, import_module_mock):
    ...
    with patch.object(logger, 'error', side_effect=Exception('break while true')) as logger_mock:
        ...
        logger_mock.assert_called_once_with(
            "Error in DAPI request. The destination node is not connected or does not exist: 'guardbear'."
        )

        node = NodeMock()
        with patch.object(node, 'send_request', side_effect=GuardBearClusterError(3020, extra_message='test')):
            with patch.object(node, 'send_string', return_value=b'noerror'):
                with patch('guardbear.core.cluster.dapi.dapi.DistributedAPI', return_value=DistributedAPI_mock()):
                    server.clients = {'guardbear': node}
                    with pytest.raises(Exception):
                        await apirequest.run()

            with patch.object(node, 'send_string', Exception('break while true')):
                with patch('guardbear.core.cluster.dapi.dapi.DistributedAPI', return_value=DistributedAPI_mock()):
                    with patch('guardbear.core.cluster.dapi.dapi.contextlib.suppress', side_effect=Exception()):

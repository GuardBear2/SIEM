...
import pytest
from guardbear.agent import get_agents, reconnect_agents
from guardbear.cluster import get_nodes_info
from guardbear.core.results import AffectedItemsGuardBearResult

from framework.guardbear.core.cluster.hap_helper.guardbear import GuardBearAgent, GuardBearDAPI


class TestGuardBearAgent:
    ...
    def test_can_reconnect(self, version: str, expected: bool):
        ...
        assert GuardBearAgent.can_reconnect(version) == expected
    ...
    def test_get_agents_able_to_reconnect(self):
        ...
        assert GuardBearAgent.get_agents_able_to_reconnect(agents_list=agents) == [2, 3]


@mock.patch('framework.guardbear.core.cluster.hap_helper.guardbear.DistributedAPI', autospec=True)
class TestGuardBearDAPI:
    guardbear_dapi = GuardBearDAPI(tag='test')

    @pytest.fixture
    def fixture_affected_items_result(self):
        return AffectedItemsGuardBearResult()
    ...
    @mock.patch('framework.guardbear.core.cluster.hap_helper.guardbear.get_system_nodes', return_value={})
    async def test_get_cluster_nodes(
        self,
        get_system_nodes_mock: mock.AsyncMock,
        dapi_mock: mock.MagicMock,
        fixture_affected_items_result: AffectedItemsGuardBearResult,
        nodes_data: list,
        excluded_nodes: list,
    ):
        """Check the correct output of `get_cluster_nodes` function."""
        self.guardbear_dapi.excluded_nodes = excluded_nodes
        ...
        ret_val = await self.guardbear_dapi.get_cluster_nodes()

        dapi_mock.assert_called_once_with(
            f=get_nodes_info,
            f_kwargs=None,
            logger=self.guardbear_dapi.logger,
    ...
    async def test_reconnect_agents(
        ...
        ret_val = await self.guardbear_dapi.reconnect_agents(agent_list=agent_list)
    ...
    async def test_get_agents_node_distribution(
        ...
        ret_val = await self.guardbear_dapi.get_agents_node_distribution()
        ...
        dapi_mock.assert_called_once_with(
            ...
            f_kwargs={
                ...
                'limit': self.guardbear_dapi.AGENTS_MAX_LIMIT,
            },
            logger=self.guardbear_dapi.logger,
            request_type='local_master',
        )
    ...
    async def test_get_agents_belonging_to_node(
        self, dapi_mock: mock.MagicMock, fixture_affected_items_result: AffectedItemsGuardBearResult, limit: Optional[int]
    ):
        ...
        ret_val = await self.guardbear_dapi.get_agents_belonging_to_node(node_name=node_name, limit=limit)
        ...
        dapi_mock.assert_called_once_with(
            ...
            f_kwargs={
                ...
                'limit': limit or self.guardbear_dapi.AGENTS_MAX_LIMIT,
            },
            logger=self.guardbear_dapi.logger,
            request_type='local_master',
        )

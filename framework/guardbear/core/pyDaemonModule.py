...
def create_pid(name: str, pid: int):
    ...
    """
    filename = common.GUARDBEAR_RUN / f'{name}-{pid}.pid'
...
def delete_child_pids(name: str, ppid: int, logger: logging.Logger):
    ...
    logger : logging.Logger
        Logger object.
    """
    filenames = [i for i in common.GUARDBEAR_RUN.glob(f'{name}*.pid')]
...
def get_guardbear_server_pid(server_daemon_name: str, pids_path: str = common.GUARDBEAR_RUN) -> int:
    """Get the PID of the running guardbear server process contained in the given path.
    ...
    pids_path : str, optional
        Path to search the PID, by default common.GUARDBEAR_RUN

    Returns
    -------
    int
        The PID of the guardbear server.
...
def get_running_processes(pids_path: str = common.GUARDBEAR_RUN) -> list:
    """Get the running processes based on the PID files contained in the given path.

    Parameters
    ----------
    pids_path : str, optional
        Path to search for the PIDs, by default `common.GUARDBEAR_RUN`.
    ...
    """
    pids_path = Path(pids_path)
    running_processes = [i.stem for i in pids_path.glob('guardbear-*-*') if '_' not in i.stem]

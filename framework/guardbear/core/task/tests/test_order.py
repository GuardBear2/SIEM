...
from guardbear.core.engine.base import APPLICATION_JSON
from guardbear.core.exception import GuardBearError, GuardBearIndexerError
from guardbear.core.indexer.models.commands import Command, Source, Status, Target, TargetType
from guardbear.core.indexer.utils import convert_enums
...
@patch('guardbear.core.task.order.httpx.AsyncClient.post')
@patch('guardbear.core.task.order.httpx.AsyncHTTPTransport')
@patch('asyncio.sleep')
@patch('guardbear.core.indexer.CentralizedConfig')
...
async def test_get_orders_ko(
    ...
    if exception is GuardBearIndexerError:
        message = f'Error {message} - Could not connect to the indexer'
    elif exception is GuardBearError:

...
def test_revoke_tokens(db_setup):
    """Checks that the return value of revoke_tokens is a GuardBearResult.
    ...
    db_setup: callable
        This function creates the rbac.db file.
    """
    security, GuardBearResult, _ = db_setup
    mock_current_user = ContextVar('current_user', default='guardbear')
    with patch('guardbear.core.common.current_user', new=mock_current_user):
        result = security.revoke_current_user_tokens()
        assert isinstance(result, GuardBearResult)
...
def test_invalid_roles_tokens(db_setup, role_list, expected_roles):
    ...
    expected_roles : set
        Expected roles.
    """
    with patch('guardbear.core.security.TokenManager.add_user_roles_rules') as TM_mock:
...
@patch('guardbear.core.security.TokenManager.add_user_roles_rules')
...
@patch('guardbear.core.security.revoke_tokens')
@patch('guardbear.core.security.check_database_integrity')
@patch('guardbear.core.security.os.remove')

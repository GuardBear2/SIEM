...
from guardbear.core.common import MAX_SOCKET_BUFFER_SIZE
from guardbear.core.wdb import AsyncGuardBearDBConnection, GuardBearDBConnection
...
def test_async_init():
    """Verify that AsyncGuardBearDBConnection attributes are correct."""
    async_wdb = AsyncGuardBearDBConnection('test')
...
async def test_async_open_connection(open_unix_connection_mock):
    ...
    async_wdb = AsyncGuardBearDBConnection(loop='test_loop')
...
def test_async_close():
    ...
    async_wdb = AsyncGuardBearDBConnection()
...
async def test_async_send_ko():
    ...
    # GuardBear-db error response.
    encoded_response = 'err Error message'.encode(encoding='utf-8')
    async_wdb._reader.readexactly.side_effect = [struct.pack('<I', len(encoded_response)), encoded_response]
    with pytest.raises(exception.GuardBearError, match=r'\b2003\b'):
        await async_wdb._send('test')


@pytest.mark.asyncio
async def test_run_wdb_command():
    """Test `GuardBearDBConnection.run_wdb_command` method."""
    send_result = ('status', '["data"]')
    command = 'any wdb command'

    wdb_con = AsyncGuardBearDBConnection()
    with patch('guardbear.core.wdb.AsyncGuardBearDBConnection._send', return_value=send_result) as wdb_send_mock:
...
async def test_run_wdb_command_ko(wdb_response):
    """Test `GuardBearDBConnection.run_wdb_command` method expected exceptions."""
    with patch('guardbear.core.wdb.AsyncGuardBearDBConnection._send', return_value=wdb_response):
        wdb_con = AsyncGuardBearDBConnection()
...
def test_failed_connection():
    ...
    with patch('guardbear.core.common.WDB_SOCKET', '/this/path/doesnt/exist'):
        with pytest.raises(exception.GuardBearException, match='.* 2005 .*'):
            GuardBearDBConnection()
...
def test_wrong_character_encodings_wdb(send_mock, connect_mock):
    """Tests receiving a text with a bad character encoding from guardbear db."""
...
def test_failed_send_private(send_mock, connect_mock):
    ...
    with patch('socket.socket.recv', side_effect=recv_mock):
        mywdb = GuardBearDBConnection()
        with pytest.raises(exception.GuardBearException, match='.* 2003 .*'):
            mywdb._send('test_msg')

    with patch('socket.socket.recv', return_value=b'a' * (MAX_SOCKET_BUFFER_SIZE + 1)):
        mywdb = GuardBearDBConnection()
        with pytest.raises(exception.GuardBearException, match='.* 2009 .*'):
            mywdb._send('test_msg')
...
def test_query_input_validation_private(send_mock, connect_mock, error_query):
    ...
    mywdb = GuardBearDBConnection()
    with pytest.raises(exception.GuardBearException, match='.* 2004 .*'):
...
@patch('socket.socket.connect')
@patch('socket.socket.send')
@patch('guardbear.core.wdb.GuardBearDBConnection._send')
def test_execute(send_mock, socket_send_mock, connect_mock):
    ...
    with patch('guardbear.core.wdb.GuardBearDBConnection._send', new=send_mock):
...
def test_execute_pagination(socket_send_mock, connect_mock):
    ...
    # Test pagination
    with patch(
        'guardbear.core.wdb.GuardBearDBConnection._send',
        side_effect=[
            [{'total': 5}],
    ...
    with patch(
        'guardbear.core.wdb.GuardBearDBConnection._send', side_effect=[[{'total': 5}], exception.GuardBearInternalError(2009)]
    ):
...
def test_failed_execute(send_mock, connect_mock, error_query, error_type, expected_exception, delete, update):
    ...
    if not error_type:
        ...
        with patch('guardbear.core.wdb.GuardBearDBConnection._send', return_value=[{'total': 5}]):
            with patch('guardbear.core.wdb.min', side_effect=error_type):
                with pytest.raises(exception.GuardBearException, match=f'.* {expected_exception} .*'):
                    mywdb.execute(error_query, delete=delete, update=update)
...
def test_GuardBearDBConnection_loads(string):
    """Test that the `loads` method from the class `GuardBearDBConnection` cleans empty objects from the result."""
    result = GuardBearDBConnection.loads(string)
    assert len(result) == 1

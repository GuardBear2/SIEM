...
with patch('guardbear.core.common.guardbear_uid'):
    with patch('guardbear.core.common.guardbear_gid'):
        with patch.object(ValidateFilePathMixin, '_validate_file_path', return_value=None):
            ...
            from guardbear.core.agent import *
            from guardbear.core.common import reset_context_cache
            from guardbear.core.exception import GuardBearException, GuardBearIndexerError
            from guardbear.core.indexer.agent import Agent as IndexerAgent
            from guardbear.core.utils import get_group_file_path
...
def get_GuardBearDBQuery_params(wdb_class):
    """Get default parameters for the specified GuardBearDBQuery class.

    Parameters
    ----------
    wdb_class : str
        Suffix of the GuardBearDBQuery class. Example: 'Agents' to get default parameters from `GuardBearDBQueryAgents` class.
    ...
    """
    with patch('guardbear.core.agent.GuardBearDBQuery.__init__') as wdbquery_mock:
        getattr(sys.modules[__name__], f'GuardBearDBQuery{wdb_class}')()
        return wdbquery_mock.call_args.kwargs


# list with GuardBear packages availables with their hash
wpk_versions = [
...
@patch('guardbear.core.agent.GuardBearDBBackend')
@patch('guardbear.core.wdb.GuardBearDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_GuardBearDBQueryAgents__init__(socket_mock, send_mock, backend_mock, value):
    """Tests if method __init__ of GuardBearDBQueryAgents works properly.
    ...
    if value:
        GuardBearDBQueryAgents()
        backend_mock.assert_called_once()
    else:
        with pytest.raises(GuardBearException, match='.* 2005 .*'):
            GuardBearDBQueryAgents()


@patch('socket.socket.connect')
def test_GuardBearDBQueryAgents_filter_date(mock_socket_conn):
    """Tests _filter_date of GuardBearDBQueryAgents returns expected query."""
    query_agent = GuardBearDBQueryAgents()
...
@patch('socket.socket.connect')
def test_GuardBearDBQueryAgents_sort_query(mock_socket_conn, field, expected_query):
    """Tests _sort_query of GuardBearDBQueryAgents returns expected result.
    ...
    """
    query_agent = GuardBearDBQueryAgents(sort={'order': 'asc'})
...
@patch('socket.socket.connect')
def test_GuardBearDBQueryAgents_add_search_to_query(mock_socket_conn):
    """Tests _add_search_to_query of GuardBearDBQueryAgents returns expected query."""
    query_agent = GuardBearDBQueryAgents(search={'value': 'test', 'negation': True})
...
@patch('socket.socket.connect')
def test_GuardBearDBQueryAgents_format_data_into_dictionary(mock_socket_conn, data):
    """Tests _format_data_into_dictionary of GuardBearDBQueryAgents returns expected data."""
    query_agent = GuardBearDBQueryAgents(
        offset=0,
        limit=1,
        sort=None,
...
@patch('socket.socket.connect')
def test_GuardBearDBQueryAgents_parse_legacy_filters(mock_socket_conn):
    """Tests _parse_legacy_filters of GuardBearDBQueryAgents returns expected query."""
    query_agent = GuardBearDBQueryAgents(filters={'older_than': 'test'})
...
@patch('socket.socket.connect')
def test_GuardBearDBQueryAgents_process_filter(mock_socket_conn, field_name, field_filter, q_filter):
    """Tests _process_filter of GuardBearDBQueryAgents returns expected query.
...
@patch('guardbear.core.wdb.GuardBearDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_GuardBearDBQueryGroup__init__(socket_mock, send_mock, backend_mock, value):
    """Test if method __init__ of GuardBearDBQueryGroup works properly.
...
@patch('guardbear.core.wdb.GuardBearDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_GuardBearDBQueryGroup_filters(socket_mock, send_mock, filters):
    """Test if parameter filters of GuardBearDBQueryGroup works properly.
    ...
    """
    query_group = GuardBearDBQueryGroup(filters=filters)
...
@patch('socket.socket.connect')
def test_GuardBearDBQueryGroupByAgents__init__(mock_socket_conn):
    """Tests if method __init__ of GuardBearDBQueryGroupByAgents works properly."""
    query_group = GuardBearDBQueryGroupByAgents(
...
@patch('socket.socket.connect')
def test_GuardBearDBQueryGroupByAgents_format_data_into_dictionary(mock_socket_conn):
    """Tests if method _format_data_into_dictionary of GuardBearDBQueryGroupByAgents works properly."""
    query_group = GuardBearDBQueryGroupByAgents(
        filter_fields=['name', 'os.name'],
        offset=0,
        limit=1,
    ...
    query_group._data = [{'count': 1, 'name': 'guardbear-master'}, {'count': 1, 'name': 'guardbear-agent1'}]
...
@patch('guardbear.core.wdb.GuardBearDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_GuardBearDBQueryGroupByAgents(mock_socket_conn, send_mock, filter_fields, expected_response):
    """Tests if GuardBearDBQueryGroupByAgents works properly."""
    query_group = GuardBearDBQueryGroupByAgents(
...
@patch('guardbear.core.wdb.GuardBearDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_GuardBearDBQueryGroup__add_sort_to_query(mock_socket_conn, send_mock):
    """Tests if _add_sort_to_query method of GuardBearDBQueryGroup works properly."""
    query_group = GuardBearDBQueryGroup()
...
@patch('guardbear.core.agent.Agent._add')
...
def test_agent_load_info_from_db_ko(socket_mock, send_mock):
    ...
    with pytest.raises(GuardBearResourceNotFound, match='.* 1701 .*'):
...
@patch('guardbear.core.agent.GuardBearQueue.send_msg_to_agent')
@patch('guardbear.core.wdb.GuardBearDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect(socket_mock, send_mock, mock_send_msg):
    ...
    agent.reconnect(GuardBearQueue(common.AR_SOCKET))

    # Assert send_msg method is called with correct params
    mock_send_msg.assert_called_with(GuardBearQueue.HC_FORCE_RECONNECT, agent_id)


@patch('guardbear.core.agent.GuardBearQueue')
@patch('guardbear.core.wdb.GuardBearDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect_ko(socket_mock, send_mock, mock_queue):
    ...
    with pytest.raises(GuardBearError, match='.* 1707 .*'):
        agent = Agent('003')
        agent.reconnect(mock_queue)


@patch('guardbear.core.agent.Agent._remove_authd', return_value='Agent was successfully deleted')
def test_agent_remove(mock_remove_authd):
    """Tests if method remove() works as expected."""
    with patch('guardbear.core.agent.get_manager_status', return_value={'guardbear-authd': 'running'}):
        agent = Agent('001')
        result = agent.remove()
...
def test_agent_remove_ko(mock_remove_authd):
    ...
    with pytest.raises(GuardBearError, match='.* 1726 .*'):
        agent = Agent('001')
        agent.remove()


@patch('guardbear.core.agent.GuardBearSocketJSON')
def test_agent_remove_authd(mock_guardbear_socket):
    """Tests if method remove_authd() works as expected."""
    agent = Agent('001')
    agent._remove_authd(purge=True)
    mock_guardbear_socket.return_value.send.assert_called_once_with(
        {'function': 'remove', 'arguments': {'id': str(1).zfill(3), 'purge': True}}
    )
    mock_guardbear_socket.return_value.receive.assert_called_once()
    mock_guardbear_socket.return_value.close.assert_called_once()
...
@patch('guardbear.core.agent.Agent._add_authd')
...
@patch('guardbear.core.agent.get_manager_status', return_value={'guardbear-authd': 'stopped'})
def test_agent_add_ko(mock_maganer_status):
    """Test if _add() method raises expected exception."""
    agent = Agent('001')

    with pytest.raises(GuardBearError, match='.* 1706 .*'):
...
@patch('guardbear.core.agent.GuardBearSocketJSON')
def test_agent_add_authd(mock_guardbear_socket, name, ip, id, key, force):
    ...
    mock_guardbear_socket.return_value.send.assert_called_once_with(socket_msg)
...
@pytest.mark.parametrize(
    ...
    [
        (None, None),
        (GuardBearError(9008, cmd_error=True), '.* 1705 .*'),
        (GuardBearError(9007, cmd_error=True), '.* 1706 .*'),
        (GuardBearError(9012, cmd_error=True), '.* 1708 .*'),
        (GuardBearError(9000, cmd_error=True), '.* None'),
    ],
)
@patch('guardbear.core.agent.GuardBearSocketJSON')
def test_agent_add_authd_ko(mock_guardbear_socket, mocked_exception, expected_exception):
    """Tests if method _add_authd() raises expected exception."""
    agent = Agent('001')

    if not mocked_exception:
        with pytest.raises(GuardBearError, match='.* 1709 .*'):
            agent._add_authd('test_add', '192.168.0.1', '2', 'adsiojew')
    else:
        mock_guardbear_socket.return_value.receive.side_effect = mocked_exception
        with pytest.raises(GuardBearError, match=expected_exception):
...
@pytest.mark.asyncio
@patch('guardbear.core.agent.remove')
@patch('guardbear.core.agent.path.exists', return_value=True)
@patch('guardbear.core.common.GUARDBEAR_GROUPS', new=os.path.join(test_data_path, 'etc', 'groups'))
@patch('guardbear.core.indexer.Indexer._get_opensearch_client')
@patch('guardbear.core.indexer.Indexer.connect')
@patch('guardbear.core.indexer.Indexer.close')
@patch('guardbear.core.indexer.agent.AgentsIndex.delete_group')
...
@pytest.mark.asyncio
@patch('guardbear.core.indexer.create_indexer')
...
@patch('guardbear.core.indexer.Indexer.connect')
@patch('guardbear.core.indexer.Indexer.close')
@patch('guardbear.core.indexer.agent.AgentsIndex.remove_agents_from_group')
@patch('guardbear.core.indexer.agent.AgentsIndex.add_agents_to_group')
async def test_agent_set_agent_group_relationship(
    ...
    with patch.object(CentralizedConfig, 'load', return_value=None):
        ...
        with patch('guardbear.core.indexer.agent.AgentsIndex.get', return_value=IndexerAgent(groups=','.join(groups))):
...
@pytest.mark.asyncio
@patch('guardbear.core.indexer.Indexer._get_opensearch_client', side_effect=GuardBearIndexerError(2200))
...
def test_get_groups():
    ...
    with patch('guardbear.core.common.GUARDBEAR_GROUPS', new=groups):
        try:
            os.makedirs(groups)

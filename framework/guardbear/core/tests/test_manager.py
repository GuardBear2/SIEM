...
with patch('guardbear.core.common.guardbear_uid'):
    with patch('guardbear.core.common.guardbear_gid'):
        ...
        from guardbear.core.manager import *
...
@patch('guardbear.core.cluster.utils.glob')
...
def test_get_ossec_log_fields():
    ...
    assert result[1] == 'guardbear-rootcheck'
...
def test_get_ossec_logs(log_format):
    ...
    with patch('guardbear.core.manager.get_guardbear_active_logging_format', return_value=log_format):
        with pytest.raises(GuardBearInternalError, match='.*1000.*'):
            get_ossec_logs()

        with patch('guardbear.core.manager.exists', return_value=True):
...
def test_get_logs_summary(mock_exists, mock_active_logging_format):
    ...
    with patch('guardbear.core.manager.tail', return_value=logs):
        ...
        assert result['guardbear-modulesd:database'] == {
            'all': 2,
            'info': 0,
            'error': 0,
            'critical': 0,
...
def test_validation_ko(mock_exists):
    ...
    with patch('socket.socket.bind'):
        ...
        with patch('guardbear.core.manager.exists', return_value=False):
            with pytest.raises(GuardBearInternalError, match='.* 1901 .*'):
                validate_ossec_conf()

        with patch('socket.socket.connect'):
            # Socket send raise socket.error
            with patch('guardbear.core.manager.GuardBearSocket.send', side_effect=socket.error):
                with pytest.raises(GuardBearInternalError, match='.* 1014 .*'):
                    validate_ossec_conf()

            with patch('socket.socket.send'):
                # Socket recv raise socket.error
                with patch('guardbear.core.manager.GuardBearSocket.receive', side_effect=socket.timeout):
                    with pytest.raises(GuardBearInternalError, match='.* 1014 .*'):
                        validate_ossec_conf()

                # _parse_execd_output raise KeyError
                with patch('guardbear.core.manager.GuardBearSocket'):
                    with patch('guardbear.core.manager.parse_execd_output', side_effect=KeyError):
...
def test_parse_execd_output(error_flag, error_msg):
    ...
    if not error_flag:
        ...
        with pytest.raises(GuardBearException, match='.* 1908 .*'):
...
def test_get_update_information_template(last_check_date, update_check, installation_uid):
    ...
    assert template['current_version'] == f'v{guardbear.__version__}'
...
async def test_query_update_check_service_request(installation_uid, client_session_get_mock):
    ...
    with patch.object(CentralizedConfig, 'load', return_value=None):
        CentralizedConfig._config = default_config
        with patch('framework.guardbear.core.manager.guardbear.__version__', version):
            await query_update_check_service(installation_uid)
            ...
            client_session_get_mock.assert_called_with(
                ...
                headers={
                    ...
                    GUARDBEAR_TAG_KEY: f'v{version}',
                    USER_AGENT_KEY: f'GuardBear UpdateCheckService/v{version}',
                },

...
with patch('guardbear.core.common.guardbear_uid'):
    with patch('guardbear.core.common.guardbear_gid'):
        from guardbear import GuardBearException
        from guardbear.core import exception, utils
        from guardbear.core.agent import GuardBearDBQueryAgents
        from guardbear.core.common import AGENT_NAME_LEN_LIMIT
...
@patch('guardbear.core.utils.chown')
@patch('guardbear.core.common.guardbear_uid')
@patch('guardbear.core.common.guardbear_gid')
def test_create_guardbear_dir(mock_gid, mock_uid, mock_chown, exists):
    """Test create_guardbear_dir function."""
    dirpath = MagicMock()
    dirpath.exists = MagicMock(return_value=exists)

    utils.create_guardbear_dir(dirpath)
...
@patch('guardbear.core.common.guardbear_uid')
@patch('guardbear.core.common.guardbear_gid')
def test_assign_guardbear_ownership(mock_gid, mock_uid, mock_chown):
    """Test assign_guardbear_ownership function."""
    with TemporaryDirectory() as tmp_dirname:
        tmp_file = NamedTemporaryFile(dir=tmp_dirname, delete=False)
        filename = os.path.join(tmp_dirname, tmp_file.name)
        utils.assign_guardbear_ownership(filename)
...
@patch('guardbear.core.common.guardbear_uid')
@patch('guardbear.core.common.guardbear_gid')
def test_assign_guardbear_ownership_write_file(mock_gid, mock_uid, mock_chown):
    """Test assign_guardbear_ownership function with a non-regular file."""
    with TemporaryDirectory() as tmp_dirname:
        tmp_file = NamedTemporaryFile(dir=tmp_dirname, delete=False)
        filename = os.path.join(tmp_dirname, tmp_file.name)
...
@patch('guardbear.core.utils.common.MAXIMUM_DATABASE_LIMIT', new=10)
...
def test_cut_array_ko(limit, offset, expected_exception):
    ...
    """
    with pytest.raises(exception.GuardBearException, match=f'.* {expected_exception} .*'):
...
@patch('guardbear.core.utils.len', return_value=1)
@patch('guardbear.core.utils.cut_array')
@patch('guardbear.core.utils.select_array', return_value=ANY)
@patch('guardbear.core.utils.filter_array_by_query', return_value=ANY)
@patch('guardbear.core.utils.search_array', return_value=ANY)
@patch('guardbear.core.utils.sort_array', return_value=ANY)
...
@patch('guardbear.core.utils.chmod')
...
@patch('guardbear.core.utils.chown')
@patch('guardbear.core.utils.chmod')
@patch('guardbear.core.utils.utime')
...
def test_safe_move_exception(mock_utime, mock_chmod, mock_chown):
    ...
    with TemporaryDirectory() as tmpdirname:
        tmp_file = NamedTemporaryFile(dir=tmpdirname, delete=False)
        target_file = os.path.join(tmpdirname, 'target')
        with patch('guardbear.core.utils.rename', side_effect=OSError(1)):
...
@patch('guardbear.core.utils.chmod')
@patch('guardbear.core.utils.mkdir')
@patch('guardbear.core.utils.curdir', new='var')
def test_mkdir_with_mode(mock_mkdir, mock_chmod, dir_name, path_exists):
    """Test mkdir_with_mode function."""
    with patch('guardbear.core.utils.path.exists', return_value=path_exists):
...
@patch('guardbear.core.utils.mkdir', side_effect=OSError)
def test_mkdir_with_mode_ko(mock_mkdir, dir_name, exists):
    """Test mkdir_with_mode function errors work."""
    with patch('guardbear.core.utils.path.exists', return_value=exists):
        with pytest.raises(OSError):
            utils.mkdir_with_mode(dir_name)


@patch('guardbear.core.utils.open')
@patch('guardbear.core.utils.iter', return_value=['1', '2'])
def test_md5(mock_iter, mock_open):
    """Test md5 function."""
    with patch('guardbear.core.utils.hashlib.md5') as md:
...
def test_blake2b(mock_iter, mock_open):
    """Test md5 function."""
    with patch('guardbear.core.utils.hashlib.blake2b') as blake2b_mock:
...
def test_protected_get_hashing_algorithm_ko():
    ...
    with pytest.raises(exception.GuardBearException, match='.* 1723 .*'):
...
def test_get_hash(mock_open):
    ...
    with patch('guardbear.core.utils.iter', return_value=['1', '2']):
        with patch('guardbear.core.utils.hashlib.new') as md:
            md.return_value.update.side_effect = None
    ...
    with patch('guardbear.core.utils.iter', return_value=[]):
...
def test_basic_load_guardbear_yaml():
    """Test basic `load_guardbear_yaml` functionality."""
    with patch('guardbear.core.utils.open') as f:
        f.return_value.__enter__.return_value = StringIO(test_yaml)
        result = utils.load_guardbear_yaml('test_file')

        assert isinstance(result, dict)


def test_load_guardbear_yaml_read_ko():
    """Test `load_guardbear_yaml` fails gracefully when reading invalid files."""
    file_path = os.path.join(test_data_path, 'test_load_guardbear_yaml_ko', 'invalid_utf8.yaml')
    with pytest.raises(GuardBearException, match=f'.*{1006}.*'):
        utils.load_guardbear_yaml(file_path)


@patch('guardbear.core.utils.yaml.safe_load', side_effect=yaml.YAMLError)
def test_load_guardbear_yaml_data_ko(safe_load_mock):
    """Test `load_guardbear_yaml` fails gracefully when parsing invalid data."""
    with pytest.raises(GuardBearException, match=f'.*{1132}.*'):
        utils.load_guardbear_yaml('', data='1')


@patch('guardbear.core.common.GUARDBEAR_GROUPS', new='/test')
...
@pytest.mark.parametrize(
    'version1, version2',
    [
        ('GuardBear v3.5.0', 'GuardBear v3.5.2'),
        ('GuardBear v3.6.1', 'GuardBear v3.6.3'),
        ('GuardBear v3.7.2', 'GuardBear v3.8.0'),
        ('GuardBear v3.8.0', 'GuardBear v3.8.1'),
        ('GuardBear v3.9.0', 'GuardBear v3.9.2'),
        ('GuardBear v3.9.10', 'GuardBear v3.9.14'),
        ('GuardBear v3.10.1', 'GuardBear v3.10.10'),
        ('GuardBear v4.10.10', 'GuardBear v4.11.0'),
        ('GuardBear v5.1.15', 'GuardBear v5.2.0'),
        ('v3.6.0', 'v3.6.1'),
        ('v3.9.1', 'v3.9.2'),
        ('v4.0.0', 'v4.0.1'),
        ('3.6.0', '3.6.1'),
...
def test_version_ok(version1, version2):
    """Test GuardBearVersion class."""
    current_version = utils.GuardBearVersion(version1)
    new_version = utils.GuardBearVersion(version2)
...
@pytest.mark.parametrize(
    ...
    [
        ('v3.6.0', 'v.3.6.1'),
        ('GuardBear v4', 'GuardBear v5'),
        ('GuardBear v3.9', 'GuardBear v3.10'),
        ('ABC v3.10.1', 'ABC v3.10.12'),
        ('GuardBearv3.9.0', 'GuardBearv3.9.2'),
        ('3.9', '3.10'),
        ('3.9.0', '3.10'),
        ('3.10', '4.2'),
        ('3', '3.9.1'),
...
def test_version_ko(version1, version2):
    """Test GuardBearVersion class."""
    try:
        utils.GuardBearVersion(version1)
        utils.GuardBearVersion(version2)
    except ValueError:
        return
...
@pytest.mark.parametrize(
    'version1, version2',
    [
        ('GuardBear v3.10.10', 'GuardBear v3.10.10'),
        ('GuardBear v5.1.15', 'GuardBear v5.1.15'),
        ('v3.6.0', 'v3.6.0'),
        ('v3.9.2', 'v3.9.2'),
    ],
)
...
def test_GuardBearVersion_to_array():
    """Test GuardBearVersion.to_array function."""
    version = utils.GuardBearVersion('GuardBear v3.10.0-alpha4')

    assert isinstance(version.to_array(), list)


def test_GuardBearVersion__str__():
    """Test GuardBearVersion.__str__ function."""
    version = utils.GuardBearVersion('GuardBear v3.10.0-alpha4')
...
@pytest.mark.parametrize(
    'version1, version2',
    [
        ('GuardBear v3.5.2', 'GuardBear v4.0.0'),
        ('GuardBear v3.10.0-alpha', 'GuardBear v3.10.0'),
        ('GuardBear v3.10.0-alpha4', 'GuardBear v3.10.0-beta4'),
        ('GuardBear v3.10.0-alpha3', 'GuardBear v3.10.0-alpha4'),
    ],
)
def test_GuardBearVersion__ge__(version1, version2):
    """Test GuardBearVersion.__ge__ function."""
    current_version = utils.GuardBearVersion(version1)
    new_version = utils.GuardBearVersion(version2)
...
def test_failed_test_get_timeframe_in_seconds():
    ...
    with pytest.raises(exception.GuardBearException, match='.* 1411 .*'):
...
@patch('guardbear.core.utils.path.exists', return_value=True)
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('guardbear.core.utils.common.MAXIMUM_DATABASE_LIMIT', new=10)
def test_GuardBearDBQuery_protected_clean_filter(
    mock_socket_conn, mock_conn_db, mock_exists, query_filter, expected_query_filter, expected_wef
):
    """Test GuardBearDBQuery._clean_filter function."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=500,
        table='agent',
        sort=None,
        ...
        query=None,
        backend=utils.GuardBearDBBackend(agent_id=1),
        count=5,
        get_data=None,
    )
...
@patch('socket.socket.connect')
@patch('guardbear.core.utils.common.MAXIMUM_DATABASE_LIMIT', new=10)
def test_GuardBearDBQuery_protected_add_limit_to_query(
    mock_socket_conn, mock_conn_db, mock_exists, limit, error, expected_exception
):
    """Test GuardBearDBQuery._add_limit_to_query function."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=limit,
        table='agent',
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardBearDBQuery_protected_sort_query(mock_socket_conn, mock_conn_db, mock_exists):
    """Tests GuardBearDBQuery._sort_query function works."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=1,
        table='agent',
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardBearDBQuery_protected_add_sort_to_query(
    mock_socket_conn, mock_conn_db, mock_exists, sort, expected_exception
):
    """Test GuardBearDBQuery._add_sort_to_query function."""
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardBearDBQuery_protected_add_search_to_query(mock_socket_conn, mock_conn_db, mock_exists):
    """Test GuardBearDBQuery._add_search_to_query function."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=1,
        table='agent',
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardBearDBQuery_protected_parse_select_filter(
    mock_socket_conn, mock_conn_db, mock_exists, selector_fields, error, expected_exception
):
    """Test GuardBearDBQuery._parse_select_filter function."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=1,
        table='agent',
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('guardbear.core.utils.GuardBearDBQuery._parse_select_filter')
def test_GuardBearDBQuery_protected_add_select_to_query(mock_parse, mock_socket_conn, mock_conn_db, mock_exists):
    """Test GuardBearDBQuery._add_select_to_query function."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=1,
        table='agent',
...
@patch('guardbear.core.utils.path.exists', return_value=True)
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
def test_GuardBearDBQuery_protected_parse_query_regex(mock_backend_connect, mock_exists, q, expected_query_filters):
    """Test GuardBearDBQuery._parse_query function."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=1,
        table='agent',
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardBearDBQuery_protected_parse_query(mock_socket_conn, mock_conn_db, mock_exists, q, error, expected_exception):
    """Test GuardBearDBQuery._parse_query function."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=1,
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardBearDBQuery_protected_parse_legacy_filters(mock_socket_conn, mock_conn_db, mock_exists, filter_):
    """Test GuardBearDBQuery._parse_legacy_filters function."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=1,
        table='agent',
        ...
        default_sort_field=None,
        query=None,
        backend=utils.GuardBearDBBackend(agent_id=0),
        count=5,
        get_data=None,
    )
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('guardbear.core.utils.GuardBearDBQuery._parse_legacy_filters')
@patch('guardbear.core.utils.GuardBearDBQuery._parse_query')
def test_GuardBearDBQuery_parse_filters(mock_query, mock_filter, mock_socket_conn, mock_conn_db, mock_exists, filter, q):
    """Test GuardBearDBQuery._parse_filters function."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=1,
        table='agent',
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('guardbear.core.utils.GuardBearDBQuery._filter_status')
@patch('guardbear.core.utils.GuardBearDBQuery._filter_date')
def test_GuardBearDBQuery_protected_process_filter(
    mock_date, mock_status, mock_socket_conn, mock_conn_db, mock_exists, field_name, field_filter, q_filter
):
    """Tests GuardBearDBQuery._process_filter."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=1,
        table='agent',
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('guardbear.core.utils.GuardBearDBQuery._process_filter')
def test_GuardBearDBQuery_protected_add_filters_to_query(mock_process, mock_socket_conn, mock_conn_db, mock_exists):
    """Test GuardBearDBQuery._add_filters_to_query function."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=1,
        table='agent',
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('guardbear.core.utils.path.exists', return_value=True)
def test_GuardBearDBQuery_protected_add_filters_to_query_final_query(
    mock_conn_db, mock_file_exists, filters, expected_query
):
    """Test GuardBearDBQuery._add_filters_to_query final query."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=1,
        table='agent',
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardBearDBQuery_protected_get_total_items(mock_socket_conn, mock_conn_db, mock_exists):
    """Test GuardBearDBQuery._get_total_items function."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=1,
        table='agent',
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardBearDBQuery_substitute_params(mock_socket_conn, mock_conn_db, mock_exists):
    """Test utils.GuardBearDBQuery._get_total_items function."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=1,
        table='agent',
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardBearDBQuery_protected_format_data_into_dictionary(mock_socket_conn, mock_conn_db, mock_exists):
    """Test utils.GuardBearDBQuery._format_data_into_dictionary."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=1,
        table='agent',
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardBearDBQuery_protected_filter_status(mock_socket_conn, mock_conn_db, mock_exists):
    """Test utils.GuardBearDBQuery._filter_status function."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=1,
        table='agent',
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardBearDBQuery_protected_filter_date(
    mock_socket_conn, mock_conn_db, mock_exists, date_filter, filter_db_name, time, error
):
    """Test utils.GuardBearDBQuery._filter_date function."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=1,
        table='agent',
    ...
    with patch('guardbear.core.utils.get_timeframe_in_seconds', return_value=time):
        if error:
            with pytest.raises(exception.GuardBearException, match='.* 1412 .*'):
...
@patch('socket.socket.connect')
def test_GuardBearDBQuery_general_run(mock_socket_conn, execute_value, expected_result):
    """Test utils.GuardBearDBQuery.general_run function."""
    with patch('guardbear.core.utils.GuardBearDBBackend.execute', return_value=execute_value):
        query = GuardBearDBQueryAgents(
            offset=0,
            limit=None,
            sort=None,
            search=None,
...
@patch('socket.socket.connect')
def test_GuardBearDBQuery_oversized_run(mock_socket_conn, execute_value, rbac_ids, negate, final_rbac_ids, expected_result):
    """Test utils.GuardBearDBQuery.oversized_run function."""
    with patch('guardbear.core.utils.GuardBearDBBackend.execute', side_effect=[execute_value, final_rbac_ids]):
        query = GuardBearDBQueryAgents(
            offset=0,
            limit=None,
            sort=None,
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('guardbear.core.utils.GuardBearDBQuery._default_query')
def test_GuardBearDBQuery_reset(mock_query, mock_socket_conn, mock_conn_db, mock_exists):
    """Test utils.GuardBearDBQuery.reset function."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=1,
        table='agent',
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardBearDBQuery_protected_default_query(mock_socket_conn, mock_conn_db, mock_exists):
    """Test utils.GuardBearDBQuery._default_query function."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=1,
        table='agent',
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardBearDBQuery_protected_default_count_query(mock_socket_conn, mock_conn_db, mock_exists):
    """Test utils.GuardBearDBQuery._default_count_query function."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=1,
        table='agent',
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardBearDBQuery_protected_pass_filter(mock_socket_conn, mock_conn_db, mock_exists, value):
    """Test utils.GuardBearDBQuery._pass_filter function."""
    query = utils.GuardBearDBQuery(
        offset=0,
        limit=1,
        table='agent',
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardBearDBQueryDistinct_protected_default_query(mock_socket_conn, mock_conn_db, mock_exists):
    """Test utils.GuardBearDBQueryDistinct._default_query function."""
    query = utils.GuardBearDBQueryDistinct(
        offset=0,
        limit=1,
        sort=None,
        search=None,
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardBearDBQueryDistinct_protected_default_count_query(mock_socket_conn, mock_conn_db, mock_exists):
    """Test utils.GuardBearDBQueryDistinct._default_count_query function."""
    query = utils.GuardBearDBQueryDistinct(
        offset=0,
        limit=1,
        sort=None,
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('guardbear.core.utils.GuardBearDBQuery._add_filters_to_query')
def test_GuardBearDBQueryDistinct_protected_add_filters_to_query(mock_add, mock_socket_conn, mock_conn_db, mock_exists):
    """Test utils.GuardBearDBQueryDistinct._add_filters_to_query function."""
    query = utils.GuardBearDBQueryDistinct(
        offset=0,
        limit=1,
        sort=None,
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('guardbear.core.utils.GuardBearDBQuery._add_select_to_query')
def test_GuardBearDBQueryDistinct_protected_add_select_to_query(
    mock_add, mock_socket_conn, mock_conn_db, mock_exists, select
):
    """Test utils.GuardBearDBQueryDistinct._add_select_to_query function."""
    query = utils.GuardBearDBQueryDistinct(
        offset=0,
        limit=1,
        sort=None,
    ...
    if len(select) > 1:
        with pytest.raises(exception.GuardBearException, match='.* 1410 .*'):
            query._add_select_to_query()
    else:
        query._add_select_to_query()
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardBearDBQueryDistinct_protected_format_data_into_dictionary(mock_socket_conn, mock_conn_db, mock_exists):
    """Test utils.GuardBearDBQueryDistinct._format_data_into_dictionary function."""
    query = utils.GuardBearDBQueryDistinct(
        offset=0,
        limit=1,
        sort=None,
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_GuardBearDBQueryGroupBy__init__(mock_socket_conn, mock_conn_db, mock_exists):
    """Tests utils.GuardBearDBQueryGroupBy.__init__ function works."""
    utils.GuardBearDBQueryGroupBy(
        filter_fields=None,
        offset=0,
        limit=1,
        table='agent',
...
@patch('guardbear.core.utils.GuardBearDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('guardbear.core.utils.GuardBearDBQuery._get_total_items')
def test_GuardBearDBQueryGroupBy_protected_get_total_items(mock_total, mock_socket_conn, mock_conn_db, mock_exists):
    """Test utils.GuardBearDBQueryGroupBy._get_total_items function."""
    query = utils.GuardBearDBQueryGroupBy(
...
@patch('guardbear.core.utils.GuardBearDBQuery._add_select_to_query')
@patch('guardbear.core.utils.GuardBearDBQuery._parse_select_filter')
def test_GuardBearDBQueryGroupBy_protected_add_select_to_query(
    mock_parse, mock_add, mock_socket_conn, mock_conn_db, mock_exists
):
    """Test utils.GuardBearDBQueryGroupBy._add_select_to_query function."""
    query = utils.GuardBearDBQueryGroupBy(
        filter_fields={'fields': ['name']},
        offset=0,
        limit=1,
...
def test_filter_array_by_query(q, return_length):
    ...
    if return_length == -1:
        with pytest.raises(exception.GuardBearError, match='.* 1407 .*'):
...
def test_select_array(select, required_fields, expected_result):
    ...
    except utils.GuardBearError as e:
        assert e.code == 1724
...
@patch('guardbear.core.utils.copy2', new=copyfile)

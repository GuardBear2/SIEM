...
import pytest
from guardbear.core.exception import GuardBearError, GuardBearException
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            None,
            None,
            'Error 1017 - Some GuardBear daemons are not ready yet in node "Node Name" (not ready daemons)',
        ),
    ],
)
def test_guardbear_exception_to_string(
    code, extra_message, extra_remediation, cmd_error, dapi_errors, title, type, exc_string
):
    """Check object constructor."""
    exc = GuardBearException(code, extra_message, extra_remediation, cmd_error, dapi_errors, title, type)
    assert str(exc) == exc_string


def test_guardbear_exception__or__():
    """Check that GuardBearException's | operator performs the join of dapi errors properly."""
    excp1 = GuardBearException(1307)
    excp1._dapi_errors = {'test1': 'test error'}
    excp2 = GuardBearException(1307)
    excp2._dapi_errors = {'test2': 'test error'}
    excp3 = excp2 | excp1
    assert excp3._dapi_errors == {'test1': 'test error', 'test2': 'test error'}


def test_guardbear_exception__deepcopy__():
    """Check that GuardBearException's __deepcopy__ magic method works properly."""
    excp1 = GuardBearException(1307)
    excp2 = excp1.__deepcopy__()
    assert excp1 == excp2 and excp1 is not excp2


def test_guardbear_error__or__():
    """Check that GuardBearError's | operator performs the union of id sets properly."""
    error1 = GuardBearError(1307, ids={1, 2, 3})
    error2 = GuardBearError(1307, ids={4, 5, 6})

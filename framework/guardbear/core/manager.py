...
from guardbear.core.configuration import get_active_configuration, get_cti_url
from guardbear.core.utils import get_utc_now, get_utc_strptime, tail
from guardbear.core.guardbear_socket import GuardBearSocket
...
GUARDBEAR_UID_KEY = 'guardbear-uid'
GUARDBEAR_TAG_KEY = 'guardbear-tag'
...
def get_ossec_log_fields(log: str, log_format: LoggingFormat = LoggingFormat.plain) -> Union[tuple, None]:
    ...
    if 'rootcheck' in tag:  # Unify rootcheck category
        tag = 'guardbear-rootcheck'

    return get_utc_strptime(date, '%Y/%m/%d %H:%M:%S'), tag, level.lower(), description


def get_guardbear_active_logging_format() -> LoggingFormat:
    """Obtain the GuardBear active logging format.

    Returns
    -------
    LoggingFormat
        GuardBear active log format. Can either be `plain` or `json`. If it has both types, `plain` will be returned.
    """
...
def get_ossec_logs(limit: int = 2000) -> list:
    ...
    log_format = get_guardbear_active_logging_format()
    if log_format == LoggingFormat.plain and exists(common.GUARDBEAR_LOG):
        guardbear_log_content = tail(common.GUARDBEAR_LOG, limit)
    elif log_format == LoggingFormat.json and exists(common.GUARDBEAR_LOG_JSON):
        guardbear_log_content = tail(common.GUARDBEAR_LOG_JSON, limit)
    else:
        raise GuardBearInternalError(1000)

    for line in guardbear_log_content:
...
def validate_ossec_conf() -> str:
    ...
    # Message for checking GuardBear configuration
    wcom_msg = common.CHECK_CONFIG_COMMAND

    # Connect to wcom socket
    if exists(wcom_socket_path):
        try:
            wcom_socket = GuardBearSocket(wcom_socket_path)
        except GuardBearException as e:
            extra_msg = f'Socket: GUARDBEAR_PATH/queue/sockets/com. Error {e.message}'
...
def parse_execd_output(output: str) -> Dict:
    ...
    if error_flag != 0:
        ...
        raise GuardBearError(1908, extra_message=', '.join(errors))
    else:
        response = {'status': 'OK'}
...
def get_update_information_template(
    uuid: str,
    update_check: bool,
    current_version: str = f'v{guardbear.__version__}',
    ...
    current_version : str, optional
        Indicates the current version of GuardBear, by default guardbear.__version__.
...
async def query_update_check_service(installation_uid: str) -> dict:
    ...
    installation_uid : str
        GuardBear UID to include in the query.
    ...
    update_information : dict
        Updates information.
    """
    current_version = f'v{guardbear.__version__}'
    headers = {
        GUARDBEAR_UID_KEY: installation_uid,
        GUARDBEAR_TAG_KEY: current_version,
        USER_AGENT_KEY: f'GuardBear UpdateCheckService/{current_version}',
    }

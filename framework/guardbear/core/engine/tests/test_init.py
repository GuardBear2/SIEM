...
from guardbear.core.engine import Engine, get_engine_client
from guardbear.core.engine.tests.conftest import get_default_configuration
...
async def test_get_engine_client_ko(socket_path: str, error_number: int):
    """Check that the `get_engine_client` returns a GuardBearEngineError on an exception."""
    with patch.object(CentralizedConfig, 'load', return_value=None):
        CentralizedConfig._config = default_config

        with pytest.raises(GuardBearEngineError, match=f'.*{error_number}.*'):
            async with get_engine_client() as engine:

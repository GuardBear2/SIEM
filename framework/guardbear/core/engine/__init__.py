...
from guardbear.core.common import ENGINE_SOCKET
from guardbear.core.config.client import CentralizedConfig
from guardbear.core.engine.events import EventsModule
from guardbear.core.engine.vulnerability import VulnerabilityModule
from guardbear.core.exception import GuardBearEngineError

logger = getLogger('guardbear')


class Engine:
    """GuardBear Engine API client."""
...
async def get_engine_client() -> AsyncIterator[Engine]:
    ...
    try:
        ...
        raise GuardBearEngineError(2800)
    except UnsupportedProtocol:
        raise GuardBearEngineError(2801)
    except ConnectError:
        raise GuardBearEngineError(2802)
    finally:
        await client.close()

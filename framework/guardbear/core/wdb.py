...
class AsyncGuardBearDBConnection:
    ...
    async def _send(self, msg, raw=False):
        """Format and send message to guardbear-db socket without blocking event loop.

        Parameters
        ----------
        msg : str
            Message to be sent to guardbear-db.
        raw : bool
            If `True`, the status message from guardbear-db is included in the response.

        Returns
        -------
        str, list
            Result for the request sent to guardbear-db.
        """
        try:
            ...
            if raw:
                ...
                raise GuardBearError(2003, data[1])
            else:
                return json.loads(data[1], object_hook=GuardBearDBConnection.json_decoder)
    ...
    async def run_wdb_command(self, command):
        ...
        ----------
        command : str
            Command to be executed inside guardbear-db
...
class GuardBearDBConnection:
    ...
    def __init__(self, request_slice=500):
        ...
        ----------
        request_slice : int
            Maximum number of items to request from guardbear-db on the first call.
        """
        self.socket_path = common.WDB_SOCKET
    ...
    def __query_input_validation(self, query: str):
        ...
        for check, error_text in input_val_errors:
            if not check:
                raise GuardBearError(2004, error_text)
    ...
    def _send(self, msg: str, raw: bool = False) -> dict:
        ...
        GuardBearInternalError(2009)
            Pagination error. Response from guardbear-db was over the maximum socket buffer size.
        GuardBearError(2003)
        ...
        if data[0] == 'err':
            ...
            return GuardBearDBConnection.loads(data[1])
    ...
    def loads(string: str) -> dict:
        ...
        ----------
        string : str
            String response from `guardbear-db`. It must be a dumped JSON.
        ...
        dict
            JSON object.
        """
        data = json.loads(string, object_hook=GuardBearDBConnection.json_decoder)
        if '"(null)"' in string:
    ...
    def delete_agents_db(self, agents_id: List[str]) -> dict:
        """Delete agents db through guardbear-db service.
        ...
        -------
        dict
            Dict received from guardbear db in the form: {"agents": {"ID": "MESSAGE"}}, where MESSAGE may be one of the
            following:
                - Ok
                - Invalid agent ID
                ...
                - DB not found
        """
        return self._send(f'guardbeardb remove {" ".join(agents_id)}')
    ...
    def send(self, query: str, raw: bool = True) -> Union[str, dict]:
        ...
        ----------
        query : str
            Query to be executed in guardbear-db.
        raw : bool
    ...
    def execute(self, query, count=False, delete=False, update=False):
        ...
        def send_request_to_wdb(query_lower, step, off, response):
            try:
                ...
                response.extend(GuardBearDBConnection.loads(request_response))
        ...
        if delete:
            ...
            if regex.match(query_lower) is None:
                raise GuardBearError(2004, 'Delete query is wrong')
        ...
        if update:
            ...
            if regex.match(query_lower) is None:
                raise GuardBearError(2004, 'Update query is wrong')
            return self._send(query_lower)
        ...
        if not re.search(r'.?select count\([\w \*]+\)( as [^,]+)? from', query_without_where):
            ...
            try:
                ...
                raise GuardBearError(2006, str(e))
            except (GuardBearError, GuardBearInternalError) as e:
                raise e

...
from guardbear.core.common import GUARDBEAR_INDEXER_CA_BUNDLE
...
from guardbear.core.utils import assign_guardbear_ownership
...
class SSLConfig(GuardBearConfigBaseModel, ValidateFilePathMixin):
...
class IndexerSSLConfig(GuardBearConfigBaseModel, ValidateFilePathMixin):
    ...
    certificate_authorities_bundle: Path = GUARDBEAR_INDEXER_CA_BUNDLE
    ...
    def create_ca_bundle(cls, file_paths: List[str]):
        ...
        """
        try:
            with open(GUARDBEAR_INDEXER_CA_BUNDLE, 'w') as bundle_file:
                for file_path in file_paths:
                    with open(file_path, 'r') as file:
                        bundle_file.write(file.read())

            assign_guardbear_ownership(GUARDBEAR_INDEXER_CA_BUNDLE)
        except IOError as e:
            raise GuardBearError(1006, str(e))


class APISSLConfig(GuardBearConfigBaseModel, ValidateFilePathMixin):

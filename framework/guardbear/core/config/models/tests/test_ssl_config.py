...
from guardbear.core.config.models.ssl_config import APISSLConfig, IndexerSSLConfig, SSLConfig, SSLProtocol
...
@patch('os.path.isfile', return_value=True)
@patch('builtins.open')
@patch('guardbear.core.config.models.ssl_config.assign_guardbear_ownership')
...
def test_indexer_ssl_config_create_bundle_file(assign_ownership_mock, open_mock, file_exists_mock):
    ...
    open_mock.assert_has_calls(
        [
            call(GUARDBEAR_INDEXER_CA_BUNDLE, 'w'),
            call('root_ca.pem', 'r'),
            call('intermediate.pem', 'r'),
        ],
        any_order=True,
    )
    assign_ownership_mock.assert_called_once_with(GUARDBEAR_INDEXER_CA_BUNDLE)
    assert ssl_config.certificate_authorities_bundle == GUARDBEAR_INDEXER_CA_BUNDLE
...
def test_indexer_ssl_config_create_bundle_file_ko(open_mock, file_exists_mock):
    ...
    with pytest.raises(GuardBearError, match=r'1006'):

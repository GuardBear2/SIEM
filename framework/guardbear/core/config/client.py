...
from guardbear import GuardBearInternalError
from guardbear.core.common import GUARDBEAR_SERVER_YML
from guardbear.core.config.models.central_config import (
    CommsAPIConfig,
    Config,
    ConfigSections,
    EngineConfig,
...
from guardbear.core.config.models.management_api import RBACMode
from guardbear.core.config.models.server import ServerSyncConfig
...
class CentralizedConfig:
    ...
    def load(cls):
        ...
        if cls._config is None:
            if not os.path.exists(GUARDBEAR_SERVER_YML):
                raise FileNotFoundError(f'Configuration file not found: {GUARDBEAR_SERVER_YML}')
            with open(GUARDBEAR_SERVER_YML, 'r') as file:
    ...
    def update_security_conf(cls, config: dict):
        ...
        try:
            with open(GUARDBEAR_SERVER_YML, 'w') as file:

...
from guardbear.core.exception import GuardBearException, GuardBearInternalError

SOCKET_COMMUNICATION_PROTOCOL_VERSION = 1


class GuardBearSocket:
    MAX_SIZE = 65536
    ...
    def _connect(self):
        try:
            ...
            raise GuardBearException(1013, str(e))
    ...
    def send(self, msg_bytes, header_format='<I'):
        if not isinstance(msg_bytes, bytes):
            raise GuardBearException(1105, 'Type must be bytes')

        try:
            sent = self.s.send(pack(header_format, len(msg_bytes)) + msg_bytes)
            if sent == 0:
                raise GuardBearException(1014, 'Number of sent bytes is 0')
            return sent
        except Exception as e:
            raise GuardBearException(1014, str(e))
...
class GuardBearSocketJSON(GuardBearSocket):
    MAX_SIZE = 65536

    def __init__(self, path):
        GuardBearSocket.__init__(self, path)

    def send(self, msg, header_format='<I'):
        return GuardBearSocket.send(self, msg_bytes=dumps(msg).encode(), header_format=header_format)

    def receive(self, header_format='<I', header_size=4, raw=False):
        response = loads(GuardBearSocket.receive(self, header_format=header_format, header_size=header_size).decode())
        if not raw:
            if 'error' in response.keys():
                if response['error'] != 0:
                    raise GuardBearException(response['error'], response['message'], cmd_error=True)
            return response['data']
        else:
            return response


class GuardBearAsyncProtocol(asyncio.Protocol):
    """GuardBear implementation of asyncio.Protocol class."""
...
class GuardBearAsyncSocket:
    ...
    async def connect(self, path_to_socket: str):
        ...
        try:
            ...
            raise GuardBearException(1013, str(exc)) from exc
    ...
    async def send(self, msg_bytes: bytes, header_format: str = '<I'):
        ...
        Raises
        ------
        GuardBearException(1014)

        """
        if not isinstance(msg_bytes, bytes):
            raise GuardBearException(1014, 'Type must be bytes')
        ...
        try:
            ...
            raise GuardBearException(1014, 'Socket connection was closed') from exc
    ...
    async def receive(self, header_format: str = '<I', header_size: int = 4) -> bytes:
        ...
        try:
            ...
            raise GuardBearException(1014, str(exc)) from exc


class GuardBearAsyncSocketJSON(GuardBearAsyncSocket):
...
def create_guardbear_socket_message(origin=None, command=None, parameters=None):

...
from guardbear.core.common import origin_module
...
def create_guardbear_queue_socket_msg(flag: str, str_agent_id: str, msg: str, is_restart: bool = False) -> str:
    """Create message that will be sent to the GuardBearQueue socket.
    ...
    -------
    str
        Message that will be sent to the GuardBearQueue socket.
    """
    return (
...
class BaseQueue:
    ...
    def _connect(self):
        try:
            ...
            if length_send_buffer < GuardBearQueue.MAX_MSG_SIZE:
                self.socket.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, GuardBearQueue.MAX_MSG_SIZE)
        except Exception:
...
class GuardBearQueue(BaseQueue):
    """GuardBearQueue Object."""
    ...
    RESTART_AGENTS_JSON = json.dumps(
        create_guardbear_socket_message(
            origin={'module': origin_module.get()}, command='restart-guardbear0', parameters={'extra_args': [], 'alert': {}}
        )
    )
    ...
    def send_msg_to_agent(self, msg: str = '', agent_id: str = '', msg_type: str = '') -> str:
        ...
        msg_is_no_ar = msg in [GuardBearQueue.HC_SK_RESTART, GuardBearQueue.HC_FORCE_RECONNECT]
        msg_is_restart = msg in [GuardBearQueue.RESTART_AGENTS, GuardBearQueue.RESTART_AGENTS_JSON]
        ...
        # AR
        if msg_type == GuardBearQueue.AR_TYPE:
            socket_msg = create_guardbear_queue_socket_msg(flag, str_agent_id, msg)
            # Return message
        ...
        # Restart agents
            ...
            socket_msg = create_guardbear_queue_socket_msg(flag, str_agent_id, msg, is_restart=msg_is_restart)
            # Return message
            if msg == GuardBearQueue.HC_SK_RESTART:
                ret_msg = 'Restarting Syscheck on agent' if agent_id else 'Restarting Syscheck on all agents'
            elif msg == GuardBearQueue.HC_FORCE_RECONNECT:
                ret_msg = 'Reconnecting agent' if agent_id else 'Reconnecting all agents'
            else:  # msg == GuardBearQueue.RESTART_AGENTS or msg == GuardBearQueue.RESTART_AGENTS_JSON
        ...
        try:
            ...
            raise GuardBearError(1014, extra_message=f': GuardBearQueue socket with path {self.path}')

        return ret_msg


class GuardBearAnalysisdQueue(BaseQueue):
    """GuardBearAnalysisdQueue Object."""
    ...
    def send_msg(self, msg_header: str, msg: str):
        ...
        if len(socket_msg) > self.MAX_MSG_SIZE:
            raise GuardBearError(1012, f'The event is too large to be sent to analysisd (maximum is {self.MAX_MSG_SIZE}B)')

        try:
            # Send message
            self._send(socket_msg)
        except Exception as e:
            raise GuardBearError(1014, extra_message=f': GuardBearAnalysisdQueue socket with path {self.path}. {str(e)}')

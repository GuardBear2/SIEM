...
def create_guardbear_dir(dirpath: Path):
    ...
    if not dirpath.exists():
        dirpath.mkdir()
        chown(dirpath, common.guardbear_uid(), common.guardbear_gid())


def assign_guardbear_ownership(filepath: str):
    ...
    if not os.path.isfile(filepath):
        f = open(filepath, 'w')
        f.close()
    if os.stat(filepath).st_gid != common.guardbear_gid() or os.stat(filepath).st_uid != common.guardbear_uid():
        os.chown(filepath, common.guardbear_uid(), common.guardbear_gid())
...
def clean_pid_files(daemon: str) -> None:
    ...
    for pid_file in os.listdir(common.GUARDBEAR_RUN):
        if match := re.match(regex, pid_file):
            try:
                ...
                if daemon.replace('-', '_') in command:
                    ...
                    print(f'{daemon}: Process {pid} does not belong to {daemon}, removing from {common.GUARDBEAR_RUN}...')

            except (OSError, psutil.NoSuchProcess):
                print(f'{daemon}: Non existent process {pid}, removing from {common.GUARDBEAR_RUN}...')
            finally:
                os.remove(path.join(common.GUARDBEAR_RUN, pid_file))
...
def process_array(
    ...
    """Process a GuardBear framework data array.
...
def cut_array(array: list, offset: int = 0, limit: int = common.DATABASE_LIMIT) -> list:
    ...
    if limit is not None:
        if limit > common.MAXIMUM_DATABASE_LIMIT:
            raise GuardBearError(1405, extra_message=str(limit))
        elif limit == 0:
            raise GuardBearError(1406)
    ...
    if offset < 0:
        raise GuardBearError(1400)
    elif limit < 1:
        raise GuardBearError(1401)
    else:
...
def sort_array(
    ...
    if sort_by:  # array should be a dictionary or a Class
        ...
        if type(array) is set or (type(array[0]) is not dict and "class 'guardbear" not in str(type(array[0]))):
...
def select_array(
    ...
    if allowed_select_fields and not select_no_nested.issubset(allowed_select_fields):
        raise GuardBearError(1724, '{}'.format(', '.join(select_no_nested)))
    ...
    for item in array:
        ...
        if array and not allowed_select_fields and not selected_fields:
            raise GuardBearError(1724, '{}'.format(', '.join(select)))
...
def _get_hashing_algorithm(hash_algorithm):
    ...
    if hash_algorithm not in algorithm_list:
        raise GuardBearError(1723, 'Available algorithms are {0}.'.format(', '.join(algorithm_list)))
...
def validate_guardbear_configuration(data: str):
    """Check that the GuardBear configuration provided is valid.
...
def load_guardbear_yaml(filepath: str, data: str = None) -> dict:
    """Load GuardBear YAML configuration files.
    ...
    validate_guardbear_configuration(data)

    try:
        parsed_data = yaml.safe_load(data)
    except yaml.YAMLError as e:
        raise GuardBearError(1132, extra_message=str(e))
...
def get_group_file_path(group_id: str) -> str:
    ...
    """
    return path.join(common.GUARDBEAR_GROUPS, group_id + GROUP_FILE_EXT)


class GuardBearVersion:
    def __init__(self, version):
        pattern = r'(?:GuardBear )?v?(\d+)\.(\d+)\.(\d+)\-?(alpha|beta|rc)?(\d*)'
...
def get_timeframe_in_seconds(timeframe: str) -> int:
    ...
    if not timeframe.isdigit():
        if 'h' not in timeframe and 'd' not in timeframe and 'm' not in timeframe and 's' not in timeframe:
            raise GuardBearError(1411, timeframe)
        ...
        for or_clause in or_clauses:
            ...
            for and_clause in and_clauses:
                ...
                try:
                    ...
                    raise GuardBearError(1407, extra_message=f"Parameter 'q' is not valid: '{and_clause}'")
...
class GuardBearDBBackend(AbstractDatabaseBackend):
    """This class describes a guardbear db backend that executes database queries."""
    ...
    def connect_to_db(self):
        return GuardBearDBConnection(request_slice=self.request_slice)
    ...
    def execute(self, query, request, count=False):
        """Execute SQL query through GuardBearDB socket."""
        query = self._substitute_params(query, request)
        return self.conn.execute(query=self._render_query(query), count=count)


class GuardBearDBQuery(object):
    """This class describes a database query for guardbear."""
    ...
    def __init__(
        ...
        """GuardBear DB Query constructor.
        ...
        # For example, the following regex -> (name!=guardbear;id>5),group=webserver <- would return 3 different matches:
        #   (name != guardbear ;
        #    id   > 5      ),
        #    group=webserver
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        isinstance(self.backend, GuardBearDBBackend) and self.backend.close_connection()
    ...
    def _add_limit_to_query(self):
        if self.limit:
            if self.limit > common.MAXIMUM_DATABASE_LIMIT:
                raise GuardBearError(1405, extra_message=str(self.limit))
    ...
    def _parse_query(self):
        ...
        if not self.query_regex.match(self.q):
            raise GuardBearError(1407, self.q)

        level = 0
        for open_level, field, op, value, close_level, separator in self.query_regex.findall(self.q):
            if field not in self.fields.keys():
                raise GuardBearError(1408, 'Available fields: {}. Field: {}'.format(', '.join(self.fields), field))
    ...
    def _filter_date(self, date_filter, filter_db_name):
        ...
        if date_filter['value'].isdigit() or re.match(r'\d+[dhms]', date_filter['value']):
            ...
            raise GuardBearError(1412, date_filter['value'])
    ...
    def oversized_run(self) -> dict:
        ...
        final_ids = list()
        resources = list()
        if self.__class__.__name__ == 'GuardBearDBQueryAgents':
            resource = 'id'
        elif self.__class__.__name__ == 'GuardBearDBQueryGroups':
            resource = 'name'
        else:
...
class GuardBearDBQueryDistinct(GuardBearDBQuery):
    ...
    def _add_filters_to_query(self):
        GuardBearDBQuery._add_filters_to_query(self)
        self.query += ' WHERE ' if not self.q and 'WHERE' not in self.query else ' AND '
        self.query += ' AND '.join(
            ["{0} IS NOT null AND {0} != ''".format(self.fields[field]) for field in self.select]
        )
    ...
    def _add_select_to_query(self):
        if len(self.select) > 1:
            raise GuardBearError(1410)

        GuardBearDBQuery._add_select_to_query(self)

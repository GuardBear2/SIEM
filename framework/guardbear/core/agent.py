...
from guardbear.core import common, configuration
from guardbear.core.cluster.utils import get_manager_status
from guardbear.core.exception import GuardBearError, GuardBearException, GuardBearInternalError, GuardBearResourceNotFound
from guardbear.core.indexer import get_indexer_client
from guardbear.core.indexer.base import IndexerKey
from guardbear.core.indexer.models.agent import Agent as IndexerAgent
from guardbear.core.InputValidator import InputValidator
from guardbear.core.utils import (
    GROUP_FILE_EXT,
    GuardBearDBBackend,
    GuardBearDBQuery,
    GuardBearDBQueryDistinct,
    GuardBearDBQueryGroupBy,
    GuardBearVersion,
...
from guardbear.core.guardbear_queue import GuardBearQueue
from guardbear.core.guardbear_socket import GuardBearSocketJSON
from guardbear.core.wdb import GuardBearDBConnection


class GuardBearDBQueryAgents(GuardBearDBQuery):
    """Class used to query GuardBear agents."""
    ...
    def __init__(
        ...
        if filters is None:
            filters = {}
        unify_guardbear_version_format(filters)
        if min_select_fields is None:
            min_select_fields = {'id'}
        backend = GuardBearDBBackend(query_format='global')
        GuardBearDBQuery.__init__(
            self,
            offset=offset,
            limit=limit,
            table='agent',
    ...
    def _filter_date(self, date_filter: dict, filter_db_name: str):
        """Add date filter to the GuardBear query."""
        GuardBearDBQuery._filter_date(self, date_filter, filter_db_name)
        self.query += ' AND id != 0'
    ...
    def _sort_query(self, field: str) -> str:
        ...
        return GuardBearDBQuery._sort_query(self, field)

    def _add_search_to_query(self):
        """Add search to the GuardBear query with the specific implications.

        Since IDs are stored in the database as integers, id searches must be turned into integers to work as expected.
        """
        if self.search:
            del self.fields['id']
            GuardBearDBQuery._add_search_to_query(self)
            self.fields['id'] = 'id'
    ...
    def _process_filter(self, field_name: str, field_filter: str, q_filter: dict):
        ...
        if field_name == 'group' and q_filter['value'] is not None:
            ...
            GuardBearDBQuery._process_filter(self, field_name, field_filter, q_filter)


class GuardBearDBQueryGroup(GuardBearDBQuery):
    """Class used to query GuardBear groups."""
...
class GuardBearDBQueryDistinctAgents(GuardBearDBQueryDistinct, GuardBearDBQueryAgents):
    pass


class GuardBearDBQueryGroupByAgents(GuardBearDBQueryGroupBy, GuardBearDBQueryAgents):
    ...
    def __init__(self, filter_fields: dict, *args: dict, **kwargs: dict):
        ...
        filter_fields : dict
            Fields to filter by.
        """
        GuardBearDBQueryAgents.__init__(self, *args, **kwargs)
        GuardBearDBQueryGroupBy.__init__(
            self,
            *args,
            table=self.table,
            fields=self.fields,
    ...
    def _format_data_into_dictionary(self) -> str:
        ...
        return GuardBearDBQuery._format_data_into_dictionary(self)


class Agent:
    """GuardBear Agent object."""
    ...
    def load_info_from_db(self, select: list = None):
        ...
        Raises
        ------
        GuardBearResourceNotFound(1701)
            Agent does not exist.
        """
        with GuardBearDBQueryAgents(
            offset=0,
            limit=None,
            sort=None,
            search=None,
            ...
            try:
                ...
                raise GuardBearResourceNotFound(1701)
    ...
    def reconnect(self, wq: GuardBearQueue) -> str:
        """Force reconnect to the manager.

        Parameters
        ----------
        wq : GuardBearQueue
            GuardBearQueue used for the active response message.
        ...
        -------
        str
            Message generated by GuardBear.
        """
        # TODO(#25121): the behavior should be reviewed in the corresponding use case.
        # Check if agent is active
        self.get_basic_information()
        if self.status.lower() != 'active':
            raise GuardBearError(1707)

        # Send force reconnect message to the GuardBearQueue
        ret_msg = wq.send_msg_to_agent(GuardBearQueue.HC_FORCE_RECONNECT, self.id)
    ...
    def remove(self, purge: bool = False) -> str:
        ...
        str
            Message generated by GuardBear.
        """
        # Check that guardbear-authd is running
        try:
            manager_status = get_manager_status(cache=True)
        except GuardBearInternalError as e:
            # guardbear-authd is not running due to a problem with /proc availability
            raise GuardBearError(1726, extra_message=str(e))

        if manager_status.get('guardbear-authd') != 'running':
            # guardbear-authd is not running
            raise GuardBearError(1726)
    ...
    def _remove_authd(self, purge: bool = False) -> dict:
        ...
        authd_socket = GuardBearSocketJSON(common.AUTHD_SOCKET)
        ...
        if ip != 'any':
            if ip.find('/') > 0:
                try:
                    ...
                    raise GuardBearError(1706, extra_message=ip)
            else:
                try:
    ...
    def _add_authd(self, name: str, ip: str, id: str = None, key: str = None, force: bool = None):
        """Add an agent to GuardBear using authd.
        2 uses:
            - name and ip [force]: Add an agent like manage_agents (generate id and key).
            - name, ip, id, key [force]: Insert an agent with an existing id and key.
        ...
        try:
            ...
            if e.code == 9008:
                raise GuardBearError(1705, extra_message=name)
            elif e.code == 9007:
                raise GuardBearError(1706, extra_message=ip)
            elif e.code == 9012:
                raise GuardBearError(1708, extra_message=id)
            raise e
    ...
    def get_agent_os_name(self) -> str:
        ...
        query = GuardBearDBQueryAgents(select=['os.name'], filters={'id': [self.id]})
    ...
    def get_agents_overview(
        ...
        db_query = GuardBearDBQueryAgents(
            offset=offset,
            limit=limit,
            sort=sort,
            search=search,
    ...
    def get_config(self, component: str = '', config: str = '', agent_version: str = '') -> dict:
        ...
        agent_version : str
            Agent version to compare with the required version. The format is vX.Y.Z or GuardBear vX.Y.Z.
        ...
        """
        if GuardBearVersion(agent_version) < GuardBearVersion(common.ACTIVE_CONFIG_VERSION):
...
def unify_guardbear_upgrade_version_format(upgrade_version: str) -> str:
...
def unify_guardbear_version_format(filters: dict):
    """Verify and format the specified guardbear version into the 'guardbear vX.Y.Z' standard.
    ...
    if wv is not None:
        if re.match(r'^v?\d+\.\d+\.\d+$', wv, re.IGNORECASE):
            filters['version'] = f'guardbear {"v" if "v" not in wv else ""}{wv}'
        elif re.match(r'^guardbear \d+\.\d+\.\d+$', wv, re.IGNORECASE):
...
def get_groups() -> set:
    ...
    """
    groups = set()
    for group_file in listdir(common.GUARDBEAR_GROUPS):
...
def get_manager_name() -> str:
    ...
    wdb_conn = GuardBearDBConnection()

...
import guardbear.core.exception as wexception
from guardbear.core import utils
from guardbear.core.common import DATABASE_LIMIT

current_module = sys.modules[__name__]


class AbstractGuardBearResult(collections.abc.MutableMapping):
    ...
    def __init__(self, dct: Union[dict, object]):
        ...
        if isinstance(dct, dict):
            self.dikt = dct
        elif isinstance(dct, AbstractGuardBearResult):
            self.dikt = dct.dikt
        else:
            raise wexception.GuardBearInternalError(
                1000,
                extra_message=f'dct param must be a dict or an AbstractGuardBearResult subclass, not a {type(dct)}',
            )
    ...
    def __or__(self, other):
        """| operator used to merge two AbstractGuardBearResult objects. When merged with a GuardBearException, the result is
        always a GuardBearException.

        Parameters
        ----------
        other : AbstractGuardBearResult or wexception.GuardBearException

        Raises
        ------
        GuardBearInternalError(1000)
            GuardBear internal error when two object could not be merged.

        Returns
        -------
        AbstractGuardBearResult or wexception.GuardBearException
            Resultant object.
        """
        if isinstance(other, wexception.GuardBearException):
            return other
        elif not isinstance(other, (dict, AbstractGuardBearResult)):
    ...
    def _merge_dict(self, self_field: dict, other_field: dict, key: str = None) -> dict:
        ...
        AbstractGuardBearResult subclass. This method may be redefined in subclasses.
    ...
    def limit(self, limit: int = DATABASE_LIMIT, offset: int = 0):
        """Should take the first `limit` results starting from `offset`. To be redefined in GuardBearResult subclasses.

        Parameters
        ----------
        limit : int
            Default: the value specified in guardbear.core.common.DATABASE_LIMIT
        offset : int
            Default: 0.

        Returns
        -------
        AbstractGuardBearResult
            Filtered AbstractGuardBearResult
        """
        result = deepcopy(self)
    ...
    def sort(self, fields: list = None, order: str = 'asc'):
        """Sort according to `fields` in order `order`. To be redefined in GuardBearResult subclasses.
        ...
        -------
        AbstractGuardBearResult
            Sorted AbstractGuardBearResult
        """
...
class GuardBearResult(AbstractGuardBearResult):
...
class AffectedItemsGuardBearResult(AbstractGuardBearResult):
    ...
    def add_failed_item(self, id_: str = None, error: wexception.GuardBearException = None):
        ...
        error : wexception.GuardBearException
    ...
    def add_failed_items_from(self, other):
        ...
        Parameters
        ----------
        other : AffectedItemsGuardBearResult
        ...
        wexception.GuardBearInternalError(1000)
            If dct is of a wrong type.
        """
        if not isinstance(other, AffectedItemsGuardBearResult):
            raise wexception.GuardBearInternalError(
    ...
    def __or__(self, other):
        """Merge a AffectedItemsGuardBearResult with self.

        Parameters
        ----------
        other : AffectedItemsGuardBearResult
            AffectedItemsGuardBearResult object to merge.
        ...
        if isinstance(other, wexception.GuardBearError):
            if len(other.ids) > 0:
                ...
                return other
        elif isinstance(result, wexception.GuardBearException):
            return result
        elif not isinstance(other, AffectedItemsGuardBearResult):
            raise wexception.GuardBearInternalError(1000, extra_message=f'Cannot be merged with {type(other)} object')
    ...
    def to_dict(self) -> dict:
        """Return the AffectedItemsGuardBearResult as a dict.
    ...
    def decode_json(cls, obj: dict):
        ...
        -------
        AbstractGuardBearResult
            Instance of AbstractGuardBearResult.
        """
        result = cls()
    ...
    def render(self) -> dict:
        """Render AffectedItemsGuardBearResult object.

        Returns
        -------
        dict
            Return AffectedItemsGuardBearResult as a dictionary with fields data, message and error.
        """

...
from guardbear.core.cluster.utils import manager_restart
...
def get_status() -> AffectedItemsGuardBearResult:
    """Wrapper for status().
...
@expose_resources(actions=['cluster:read'], resources=[f'node:id:{node_id}'])
def ossec_log_summary() -> AffectedItemsGuardBearResult:
...
def restart() -> AffectedItemsGuardBearResult:
    ...
    AffectedItemsGuardBearResult
        Affected items.
    """
    result = AffectedItemsGuardBearResult(**_restart_default_result_kwargs)
    try:
        manager_restart()
...
def validation() -> AffectedItemsGuardBearResult:
    ...
    AffectedItemsGuardBearResult
        Affected items.
    """
    result = AffectedItemsGuardBearResult(**_validation_default_result_kwargs)
...
@expose_resources(actions=['cluster:read'], resources=[f'node:id:{node_id}'])
def get_basic_info() -> AffectedItemsGuardBearResult:
    """Wrapper for GuardBear().to_dict.
    ...
    try:
        result.affected_items.append(GuardBear().to_dict())
    except GuardBearError as e:
        result.add_failed_item(id_=node_id, error=e)
    result.total_affected_items = len(result.affected_items)
...
def update_ossec_conf(new_conf: str = None) -> AffectedItemsGuardBearResult:
...
def get_update_information(installation_uid: str, update_information: dict) -> GuardBearResult:
    """Process update information into a guardbear result.
    ...
    if not update_information:
        ...
        return GuardBearResult({'data': get_update_information_template(uuid=installation_uid, update_check=False)})
    ...
    return GuardBearResult({'data': update_information})

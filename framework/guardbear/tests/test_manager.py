...
with patch('guardbear.core.common.guardbear_uid'):
    with patch('guardbear.core.common.guardbear_gid'):
        with patch.object(ValidateFilePathMixin, '_validate_file_path', return_value=None):
            ...
            from guardbear.core.manager import LoggingFormat
            from guardbear.core.tests.test_manager import get_logs
            from guardbear.manager import *

test_data_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'data')


@pytest.fixture(scope='module', autouse=True)
def mock_guardbear_path():
    with patch('guardbear.core.common.GUARDBEAR_PATH', new=test_data_path):
        yield
...
manager_status = {
    'guardbear-agentlessd': 'running',
    'guardbear-analysisd': 'running',
    'guardbear-authd': 'running',
    'guardbear-csyslogd': 'running',
    'guardbear-dbd': 'running',
    'guardbear-monitord': 'running',
    'guardbear-execd': 'running',
    'guardbear-integratord': 'running',
    'guardbear-logcollector': 'running',
    'guardbear-maild': 'running',
    'guardbear-remoted': 'running',
    'guardbear-reportd': 'running',
    'guardbear-syscheckd': 'running',
    'guardbear-clusterd': 'running',
    'guardbear-modulesd': 'running',
    'guardbear-db': 'running',
    'guardbear-server-management-apid': 'running',
    'guardbear-comms-apid': 'running',
}


@patch('guardbear.core.manager.status', return_value=manager_status)
def test_get_status(mock_status):
    """Tests get_status() function works."""
    result = get_status()

    # Assert there are no errors and type returned
    assert isinstance(result, AffectedItemsGuardBearResult), 'No expected result type'
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('guardbear-modulesd:database', None, 2, None, None),
        ('guardbear-modulesd:syscollector', None, 2, None, None),
        ('guardbear-modulesd:syscollector', None, 2, None, None),
        ('guardbear-modulesd:aws-s3', None, 5, None, None),
        ('guardbear-execd', None, 1, None, None),
        ('guardbear-csyslogd', None, 2, None, None),
...
@patch('guardbear.core.manager.get_guardbear_active_logging_format', return_value=LoggingFormat.plain)
@patch('guardbear.core.manager.exists', return_value=True)
def test_ossec_log(mock_exists, mock_active_logging_format, tag, level, total_items, sort_by, sort_ascending):
    ...
    tag : str
        Filters by log category (i.e. guardbear-remoted).
    total_items : int
    ...
    """
    with patch('guardbear.core.manager.tail') as tail_patch:
        ...
        if tag is not None and level != 'guardbear-modulesd:syscollector':
...
def test_ossec_log_summary(mock_exists, mock_active_logging_format):
    ...
    expected_result = {
        'guardbear-csyslogd': {'all': 2, 'info': 2, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 0},
        'guardbear-execd': {'all': 1, 'info': 0, 'error': 1, 'critical': 0, 'warning': 0, 'debug': 0},
        'guardbear-modulesd:aws-s3': {'all': 5, 'info': 2, 'error': 1, 'critical': 0, 'warning': 2, 'debug': 0},
        'guardbear-modulesd:database': {'all': 2, 'info': 0, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 2},
        'guardbear-modulesd:syscollector': {'all': 2, 'info': 2, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 0},
        'guardbear-rootcheck': {'all': 1, 'info': 1, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 0},
    }

    logs = get_logs().splitlines()
    with patch('guardbear.core.manager.tail', return_value=logs):
...
@patch('socket.socket')
@patch('guardbear.core.cluster.utils.fcntl')
@patch('guardbear.core.cluster.utils.open')
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            1,
            '2019/02/27 11:30:07 guardbear-clusterd: ERROR: [Cluster] [Main] Error 3004 - Error in cluster configuration: '
            'Unspecified key',
        ),
        (
            1,
            '2019/02/27 11:30:24 guardbear-authd: ERROR: (1230): Invalid element in the configuration: '
            "'use_source_i'.\n2019/02/27 11:30:24 guardbear-authd: ERROR: (1202): Configuration error at "
...
def test_validation(mock_exists, error_flag, error_msg):
    ...
    """
    with patch('guardbear.core.manager.GuardBearSocket') as sock:
        # Mock sock response
...
@pytest.mark.parametrize('exception', [GuardBearInternalError(1013), GuardBearError(1013)])
@patch('guardbear.manager.validate_ossec_conf')

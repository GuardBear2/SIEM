...
with patch('guardbear.core.common.guardbear_uid'):
    with patch('guardbear.core.common.guardbear_gid'):
        with patch.object(ValidateFilePathMixin, '_validate_file_path', return_value=None):
            ...
            from guardbear import GuardBearError, GuardBearException, GuardBearInternalError
            from guardbear.agent import (
                add_agent,
                build_agents_query,
                create_group,
                delete_agents,
            ...
            from guardbear.core.agent import Agent
            from guardbear.core.exception import GuardBearResourceNotFound
            ...
            from guardbear.core.indexer.models.commands import CreateCommandResponse, ResponseResult
            from guardbear.core.results import AffectedItemsGuardBearResult, GuardBearResult
            from guardbear.core.tests.test_agent import InitAgent
            from guardbear.core.utils import GROUP_FILE_EXT
...
@patch('guardbear.core.agent.Agent.reconnect')
@patch('guardbear.agent.get_agents_info', return_value=short_agent_list)
...
def test_agent_reconnect_agents(
    ...
    assert isinstance(result, AffectedItemsGuardBearResult), 'The returned object is not an "AffectedItemsGuardBearResult".'
...
async def test_agent_restart_agents(create_indexer_mock, agent_list, expected_items, fail):
    ...
    assert isinstance(result, AffectedItemsGuardBearResult)
...
@patch('guardbear.agent.build_agents_query')
@patch('guardbear.core.indexer.create_indexer')
...
@patch('guardbear.agent.get_groups')
@patch('guardbear.core.indexer.create_indexer')
async def test_agent_get_agents_in_group(create_indexer_mock, mock_get_groups, group, group_exists, expected_agents):
    ...
    if group_exists:
        ...
        with pytest.raises(GuardBearResourceNotFound, match='.* 1710 .*'):
...
@patch('guardbear.core.indexer.create_indexer')
@patch('guardbear.core.agent.Agent.group_exists', return_value=True)
...
async def test_agent_add_agent_ko(mock_group_exists, create_indexer_mock, name, id, key, type, version):
    ...
    key : str
        The agent key.
    """
    with pytest.raises(GuardBearError, match='.* 1738 .*'):
...
@patch('guardbear.core.common.GUARDBEAR_GROUPS', new=test_groups_path)
...
@patch('guardbear.core.common.GUARDBEAR_GROUPS', new=test_groups_path)
@patch('guardbear.core.common.guardbear_gid', return_value=getgrnam('root'))
@patch('guardbear.core.common.guardbear_uid', return_value=getpwnam('root'))
@patch('guardbear.agent.chown')
async def test_create_group(chown_mock, uid_mock, gid_mock, group_id):
    """Test `create_group` function from agent module.

    When a group is created a folder with the same name is created in `common.GUARDBEAR_GROUPS`.
    ...
    try:
        ...
        assert isinstance(result, GuardBearResult), 'The returned object is not an "GuardBearResult" instance.'
...
@pytest.mark.parametrize(
    ...
    [
        ('default', GuardBearError, 1711),
        (f'group-1{GROUP_FILE_EXT}', GuardBearError, 1722),
        ('invalid!', GuardBearError, 1722),
        ('delete-me', GuardBearInternalError, 1005),
        ('agent-template', GuardBearError, 1713),
    ],
)
@patch('guardbear.core.common.GUARDBEAR_GROUPS', new=test_groups_path)
async def test_create_group_exceptions(group_id, exception, exception_code):
    ...
    exception_code : int
        Expected error code for the GuardBear Exception object raised by `create_group` with the given parameters.
    """
    try:
...
@patch('guardbear.agent.get_groups')
@patch('guardbear.agent.Agent.delete_single_group')
@patch('guardbear.core.indexer.create_indexer')
...
@pytest.mark.parametrize(
    ...
    [
        (['none-1'], [GuardBearResourceNotFound(1710)]),
        (['default'], [GuardBearError(1712)]),
        (['none-1', 'none-2'], [GuardBearResourceNotFound(1710)]),
        (['default', 'none-1'], [GuardBearError(1712), GuardBearResourceNotFound(1710)]),
    ],
)
@patch('guardbear.agent.get_groups')
async def test_agent_delete_groups_other_exceptions(mock_get_groups, group_list, expected_errors):
    ...
    group_list : List of str
        List of groups to be deleted.
    expected_errors : list of GuardBearError
        List of expected GuardBearError to be raised by delete_groups if a group is not valid. An exception will be returned
...
@patch('guardbear.core.indexer.create_indexer')
@patch('guardbear.agent.get_groups', return_value={'group-1'})
...
@pytest.mark.parametrize(
    ...
    [
        (['group-1'], ['0191c7fa-26d5-705f-bc3c-f54810d30d79'], GuardBearResourceNotFound(1701), False),
    ],
)
@patch('guardbear.core.indexer.create_indexer')
@patch('guardbear.agent.get_groups', return_value={'group-1'})
async def test_agent_remove_agents_from_group_exceptions(
    ...
    catch_exception : bool
        ...
        `AffectedItemsGuardBearResult` containing the exceptions in its 'failed_items'.
    """
    try:
    ...
    except (GuardBearError, GuardBearResourceNotFound) as error:
...
@patch('guardbear.core.common.GUARDBEAR_GROUPS', new=test_groups_path)
@patch('guardbear.core.configuration.update_group_configuration')
...
@patch('guardbear.agent.get_agents_info', return_value=['test', 'test2'])
@patch('guardbear.core.wdb.GuardBearDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
def test_get_agents_big_env(mock_conn, mock_send, mock_get_agents, insert_agents_db, agent_list, params, expected_ids):
    ...
    with patch('guardbear.agent.get_agents_info', return_value=set(agent_ids_format(range(1, 100000)))):

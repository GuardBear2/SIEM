...
from guardbear.core.agent import expand_group, get_agents_info, get_groups
from guardbear.core.common import broadcast, cluster_nodes, rbac
from guardbear.core.exception import GuardBearPermissionError
from guardbear.core.results import AffectedItemsGuardBearResult
from guardbear.rbac.orm import AuthenticationManager, PoliciesManager, RolesManager, RulesManager
    ...
    **post_proc_kwargs: dict,
        ...
        for res_id, target_param in target.items():
            ...
            for denied_item in denied:
                result.add_failed_item(
                    id_=denied_item,
                    error=GuardBearPermissionError(4000, extra_message=f'Resource type: {res_id}', ids=denied),
                )
    if not add_denied or post_proc_kwargs.get('force'):
        ...
        if 'default_result_kwargs' in post_proc_kwargs and result is None:
            return AffectedItemsGuardBearResult(**post_proc_kwargs['default_result_kwargs'])
...
def expose_resources(
    ...
    """Decorator to apply user permissions on a GuardBear framework function based on exposed action:resource pairs.
            ...
            for res_id, target_param in target_params.items():
                try:
                    ...
                    if add_denied:
                        ...
                        if res_id in integer_resources:
                            denied = {int(i) if i.isdigit() else i for i in denied}
                        raise GuardBearPermissionError(

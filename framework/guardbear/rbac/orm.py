...
from guardbear.core.common import DEFAULT_RBAC_RESOURCES, GUARDBEAR_LIB, guardbear_gid, guardbear_uid
from guardbear.core.config.client import CentralizedConfig
from guardbear.core.utils import get_utc_now, safe_move
from guardbear.rbac.utils import clear_cache
...
class AuthenticationManager(RBACManager):
    ...
    def migrate_data(
        ...
        for user in old_users:
            if user.id in (GUARDBEAR_USER_ID, GUARDBEAR_WUI_USER_ID):
...
def check_if_reserved_id(from_id: Optional[str], to_id: Optional[str]):
    ...
    bool
        Condition result
    """
    return from_id == GUARDBEAR_USER_ID and to_id == GUARDBEAR_WUI_USER_ID
...
def check_database_integrity():
    ...
    def _set_permissions_and_ownership(database: str):
        """Set GuardBear ownership and permissions.
        ...
        """
        chown(database, guardbear_uid(), guardbear_gid())
    ...
    try:
        ...
        if os.path.exists(DB_FILE):
            ...
            if current_version < expected_version:
                ...
                db_manager.migrate_data(
                    source=DB_FILE, target=DB_FILE_TMP, from_id=GUARDBEAR_USER_ID, to_id=GUARDBEAR_WUI_USER_ID
                )
                db_manager.migrate_data(
                ...
                safe_move(DB_FILE_TMP, DB_FILE, ownership=(guardbear_uid(), guardbear_gid()), permissions=0o640)
        ...
        logger.error('Error retrieving the current GuardBear RBAC database version. Aborting database integrity check')
        db_manager.close_sessions()
        raise e

...
from guardbear.rbac.tests.utils import MockedUserRole, MockRolePolicy, MockRoleRules, init_db
...
def db_setup():
    with patch('guardbear.core.common.guardbear_uid'), patch('guardbear.core.common.guardbear_gid'):
        with patch('sqlalchemy.create_engine', return_value=create_engine('sqlite://')):
            with patch('shutil.chown'), patch('os.chmod'):
                import guardbear.rbac.orm as rbac
...
def fresh_in_memory_db():
    ...
    with patch('guardbear.core.common.guardbear_uid'), patch('guardbear.core.common.guardbear_gid'):
        import guardbear.rbac.orm as orm
...
def test_database_init(db_setup):
    ...
    with db_setup.RolesManager() as rm:
        assert rm.get_role('guardbear') != db_setup.SecurityError.ROLE_NOT_EXIST
...
def test_delete_all_expired_rules(db_setup):
    """Check that rules are correctly deleted."""
    with patch('guardbear.rbac.orm.time', return_value=0):
        add_token(db_setup)
    with db_setup.TokenManager() as tm:
...
@patch('guardbear.rbac.orm.create_engine')
@patch('guardbear.rbac.orm.sessionmaker')
...
@patch('guardbear.rbac.orm._Base.metadata.create_all')
...
def test_databasemanager_get_table(fresh_in_memory_db):
    ...
    with patch('guardbear.rbac.orm._new_columns', new={'new_column'}):
        updated_columns = set(
...
@patch('guardbear.rbac.orm.safe_move')
@patch('guardbear.rbac.orm.os.remove')
@patch('guardbear.rbac.orm.chown')
@patch('guardbear.rbac.orm.os.chmod')
def test_check_database_integrity(chmod_mock, chown_mock, remove_mock, safe_move_mock, fresh_in_memory_db):
    ...
    """
    db_mock = MagicMock()
    with patch('guardbear.rbac.orm.db_manager', new=db_mock):
        with patch('guardbear.rbac.orm.os.path.exists', return_value=True):
            with patch('guardbear.rbac.orm.CURRENT_ORM_VERSION', new=99999):
                ...
                db_mock.assert_has_calls(
                    [
                        ...
                        call.migrate_data(
                            ...
                            from_id=fresh_in_memory_db.GUARDBEAR_USER_ID,
                            to_id=fresh_in_memory_db.GUARDBEAR_WUI_USER_ID,
                        ),
                        call.migrate_data(
                            source=fresh_in_memory_db.DB_FILE,
                            target=fresh_in_memory_db.DB_FILE_TMP,
        ...
        safe_move_mock.assert_called_once_with(
            ...
            ownership=(fresh_in_memory_db.guardbear_uid(), fresh_in_memory_db.guardbear_gid()),
            permissions=0o640,
        )
...
@patch('guardbear.rbac.orm.DatabaseManager.close_sessions')
@patch('guardbear.rbac.orm.os.remove')
def test_check_database_integrity_exceptions(remove_mock, close_sessions_mock, exception, fresh_in_memory_db):
    ...
    with patch('guardbear.rbac.orm.os.path.exists', side_effect=mocked_exists) as mock_exists:
        with patch('guardbear.rbac.orm.DatabaseManager.connect', side_effect=exception):
...
@pytest.mark.parametrize(
    ...
    [
        (
            GUARDBEAR_USER_ID,
            GUARDBEAR_WUI_USER_ID,
            [User('guardbear', 'test', user_id=GUARDBEAR_USER_ID), User('guardbear-wui', 'test2', user_id=GUARDBEAR_WUI_USER_ID)],
        ),
...
def test_migrate_data(db_setup, from_id, to_id, users):
    ...
    if to_id == GUARDBEAR_WUI_USER_ID:
        with patch('guardbear.rbac.orm.db_manager.get_data', return_value=users):
            with patch('guardbear.rbac.orm.AuthenticationManager.update_user') as mock_update_user:
        ...
        user_exists = False
        with patch(
            'guardbear.rbac.orm.AuthenticationManager.add_user', side_effect=[not user_exists, user_exists, not user_exists]
        ) as mock_add_user:
            with patch(
                'guardbear.rbac.orm.db_manager.get_data',
...
@pytest.mark.parametrize(
    'user_data',
    [
        {'user': User('guardbear', 'test', user_id=GUARDBEAR_USER_ID), 'status': True, 'expected_update': True},
        {'user': User('guardbear', 'test', user_id=GUARDBEAR_WUI_USER_ID), 'status': True, 'expected_update': True},
        {'user': User('guardbear', 'test', user_id=110), 'status': True, 'expected_update': False},
        {'user': User('guardbear', 'test', user_id=101), 'status': False, 'expected_update': False},
    ],
)
def test_authentication_manager_migrate_data(db_setup, user_data):
    ...
    with db_setup.AuthenticationManager() as auth_manager:
        with patch('guardbear.rbac.orm.db_manager.get_data', return_value=[user]):
            with patch('guardbear.rbac.orm.AuthenticationManager.update_user') as mock_update_user:
                with patch(
                    'guardbear.rbac.orm.AuthenticationManager.add_user', side_effect=[status, status]
                ) as mock_add_user:
...
@pytest.mark.parametrize(
    'role_data',
    [
        {'user': Roles(GUARDBEAR_USER_ID, 'guardbear'), 'status': True},
        {'user': Roles(GUARDBEAR_WUI_USER_ID, 'guardbear'), 'status': True},
        {'user': Roles(110, 'guardbear'), 'status': True},
        {'user': Roles(101, 'guardbear'), 'status': SecurityError.ALREADY_EXIST},
    ],
)
def test_roles_manager_migrate_data(db_setup, role_data):
    ...
    with db_setup.RolesManager() as role_manager:
        with patch('guardbear.rbac.orm.db_manager.get_data', return_value=[role]):
            with patch('guardbear.rbac.orm.RolesManager.add_role', side_effect=[status, status]) as mock_add_role:
...
def test_roles_manager_migrate_data_with_reserved_ids(db_setup):
    ...
    role = Roles(GUARDBEAR_USER_ID, 'guardbear')

    with db_setup.RolesManager() as role_manager:
        with patch('guardbear.rbac.orm.db_manager.get_data', return_value=[role]):
            with patch('guardbear.rbac.orm.RolesManager.add_role') as mock_add_role:
                role_manager.migrate_data(
                    db_setup.db_manager,
                    source=db_setup.DB_FILE,
                    target=db_setup.DB_FILE,
                    from_id=GUARDBEAR_USER_ID,
                    to_id=GUARDBEAR_WUI_USER_ID,
                )
...
def test_rules_manager_migrate_data(db_setup, rules_data):
    ...
    with db_setup.RulesManager() as rule_manager:
        with patch('guardbear.rbac.orm.db_manager.get_data', return_value=[rule]):
            with patch('guardbear.rbac.orm.db_manager.sessions', autospec=True) as mock_sessions:
                with patch('guardbear.rbac.orm.RulesManager.add_rule', side_effect=[status, status]) as mock_add_rule:
                    with patch('guardbear.rbac.orm.db_manager.get_table') as mock_get_table:
                        with patch('guardbear.rbac.orm.RolesRulesManager.add_rule_to_role') as mock_add_rule_to_role:
...
def test_rules_manager_migrate_data_with_reserved_ids(db_setup):
    ...
    with db_setup.RulesManager() as rule_manager:
        with patch('guardbear.rbac.orm.db_manager.get_data', return_value=[rule]):
            with patch('guardbear.rbac.orm.RulesManager.add_rule') as mock_add_rule:
                rule_manager.migrate_data(
                    db_setup.db_manager,
                    source=db_setup.DB_FILE,
                    target=db_setup.DB_FILE,
...
@pytest.mark.parametrize(
    'policy_data',
    [
        {'policy': Policies('guardbear', '{}', policy_id=GUARDBEAR_USER_ID), 'status': True},
        {'policy': Policies('guardbear', '{}', policy_id=GUARDBEAR_USER_ID), 'status': SecurityError.ALREADY_EXIST},
    ],
)
def test_policies_manager_migrate_data(db_setup, policy_data):
    ...
    with db_setup.PoliciesManager() as policy_manager:
        with patch('guardbear.rbac.orm.db_manager.get_data', return_value=[policy]):
            with patch('guardbear.rbac.orm.db_manager.sessions', autospec=True) as mock_sessions:
                with patch('guardbear.rbac.orm.db_manager.get_table') as mock_get_table:
                    mock_query = mock_sessions.return_value.query.return_value
                    mock_filter_by = mock_query.filter_by.return_value
                    ...
                    mock_first.id = 'id'
                    with patch(
                        'guardbear.rbac.orm.PoliciesManager.add_policy', side_effect=[status, status]
                    ) as mock_add_policy:
...
def test_policies_manager_migrate_data_with_reserved_ids(db_setup):
    ...
    policy = Policies('guardbear', 'some_policy')

    with db_setup.PoliciesManager() as policy_manager:
        with patch('guardbear.rbac.orm.db_manager.get_data', return_value=[policy]):
            with patch('guardbear.rbac.orm.PoliciesManager.add_policy') as mock_add_policy:
                policy_manager.migrate_data(
                    db_setup.db_manager,
                    source=db_setup.DB_FILE,
                    target=db_setup.DB_FILE,
...
def test_user_roles_manager_migrate_data(db_setup, user_role_data):
    ...
    with db_setup.UserRolesManager() as user_roles_manager:
        with patch('guardbear.rbac.orm.db_manager.get_data', return_value=[user_role]):
            with patch('guardbear.rbac.orm.UserRolesManager.add_role_to_user') as mock_add_role_to_user:
                with patch(
                    'guardbear.rbac.orm.AuthenticationManager.get_user', autospec=True, return_value={'id': None}
                ) as mock_get_user:
                    with patch(
                        'guardbear.rbac.orm.RolesManager.get_role', autospec=True, return_value={'id': None}
                    ) as mock_get_role:
                        with patch('guardbear.rbac.orm.db_manager.get_table') as mock_get_table:
...
def test_roles_policies_manager_migrate_data(db_setup, role_policies_data):
    ...
    with db_setup.RolesPoliciesManager() as auth_manager:
        with patch('guardbear.rbac.orm.db_manager.get_data', return_value=[role_policies]):
            with patch('guardbear.rbac.orm.RolesPoliciesManager.add_policy_to_role') as mock_add_policy_to_role:
                with patch(
                    'guardbear.rbac.orm.PoliciesManager.get_policy', autospec=True, return_value={'id': None}
                ) as mock_get_policy:
                    with patch(
                        'guardbear.rbac.orm.RolesManager.get_role', autospec=True, return_value={'id': None}
                    ) as mock_get_role:
...
def test_roles_rules_manager_migrate_data(db_setup, role_rules_data):
    ...
    with db_setup.RolesRulesManager() as roles_rules_manager:
        with patch('guardbear.rbac.orm.db_manager.get_data', return_value=[role_rules]):
            with patch('guardbear.rbac.orm.RolesRulesManager.add_rule_to_role') as mock_add_rule_to_role:
                with patch(
                    'guardbear.rbac.orm.RulesManager.get_rule_by_name', autospec=True, return_value={'id': None}

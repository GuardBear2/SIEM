...
def getGuardBearInfo(guardbear_home):
    guardbear_control = os.path.join(guardbear_home, "bin", "guardbear-control")
    guardbear_env_vars = {}
    try:
        proc = subprocess.Popen([guardbear_control, "info"], stdout=subprocess.PIPE)
        (stdout, stderr) = proc.communicate()
    except Exception as e:
        print("Seems like there is no GuardBear installation.")
        return None
    ...
    for env_variable in env_variables:
        ...
        guardbear_env_vars[key] = value.replace("\"", "")

    return guardbear_env_vars
...
def provisionDR():
    ...
    for file in os.listdir(rules_dir):
        ...
        if os.path.isfile(file_fullpath) and re.match(r'^test_(.*_)?rules.xml$', file):
            shutil.copy2(file_fullpath, args.guardbear_home + "/etc/rules")
    ...
    for file in os.listdir(decoders_dir):
        ...
        if os.path.isfile(file_fullpath) and re.match(r'^test_(.*_)?decoders.xml$', file):
            shutil.copy2(file_fullpath, args.guardbear_home + "/etc/decoders")



def cleanDR():
    rules_dir = args.guardbear_home + "/etc/rules"
    decoders_dir = args.guardbear_home + "/etc/decoders"
...
class OssecTester(object):
    ...
    def buildCmd(self, rule, alert, decoder):
        cmd = ['%s/guardbear-logtest' % (self._ossec_path), ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='This script tests GuardBear rules.')
    parser.add_argument('--path', '-p', default='/var/ossec', dest='guardbear_home',
                        help='Use -p or --path to specify GuardBear installation path')
    ...
    guardbear_info = getGuardBearInfo(args.guardbear_home)
    if guardbear_info is None:
        sys.exit(1)
    ...
    provisionDR()
    OT = OssecTester(args.guardbear_home)
